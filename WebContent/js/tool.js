...
function ifExecute(kind, id) {
	...
	if (kind == "nslookup") {
		...
		if (kind == "ping")
			DWRUtil.ifExecute("ping", ip, pingExecuteMethod);
		if (kind == "traceroute")
			DWRUtil.ifExecute("traceroute", ip, tracerouteExecuteMethod);
	} else
		alert("您输入的Ip地址有误");
}
...
function nslookupExecuteMethod(ifexecute) {
	if (ifexecute) {
		...
		alert("Nslookup命令正在执行，请稍候...");
}

function readTraceRoute() {
	DWRUtil.readTRRsStr(document.getElementById('ipaddress').value,
			replyPingStr);
}

function readPing() {
	DWRUtil.readPingRsStr(document.getElementById('ipaddress').value,
			replyPingStr);
}

function readNslookup() {
	DWRUtil.readNslookupRsStr(document.getElementById('ipaddress').value,
			replyPingStr);
}
...
function replyPingStr(data) {
	if (data == null) {
		if (pingreader != null) {
			if (document.getElementById("resultofping").value.length < 10)
				document.getElementById("resultofping").value = "设备无响应！";
		...
		document.getElementById("resultofping").value = document
				.getElementById("resultofping").value
				+ data;
	}
}
...
function closePage(kind, ip) {
	if (kind == "ping")
		DWRUtil.closePro("ping", ip);
	if (kind == "traceroute")
		DWRUtil.closePro("traceroute", ip);
	if (kind == "nslookup")
		DWRUtil.closePro("nslookup", ip);
}