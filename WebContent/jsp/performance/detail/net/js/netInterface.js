...
$(function() {
			...
			// grid
			grid = $("#netInterfaceGrid").ligerGrid({
				columns : [{
							display : '索引',
							name : 'ifIndex',
							minWidth : 60,
...
function contextMenuItemClick(item, i) {
	if (item.text == "详细信息") {
		openWindow(basePath + "jsp/performance/detail/net/goto/ifDetail.jsp?ip="
						+ getUrlParam("ip") + "&alias=" + alias + "&ifIndex="
						+ ifIndex + "&ifName=" + ifName, 450, 850, "端口详细");
	} else if (item.text == "流速实时") {
		openWindow(basePath + "jsp/performance/detail/net/goto/ifRealTimeFlux.jsp?ip="
						+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId") + "&ifIndex="
						+ ifIndex, 485, 685, "端口详细");
	} else if (item.text == "带宽实时") {
		openWindow(basePath + "jsp/performance/detail/net/goto/ifRealTimeBandWidth.jsp?ip="
						+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId") + "&ifIndex="
						+ ifIndex, 485, 685, "端口详细");
	} else if (item.text == "广播包实时") {
		openWindow(basePath + "jsp/performance/detail/net/goto/ifRealTimeBroadcastPkts.jsp?ip="
						+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId") + "&ifIndex="
						+ ifIndex, 485, 685, "端口详细");
	} else if (item.text == "多播包实时") {
		openWindow(basePath + "jsp/performance/detail/net/goto/ifRealTimeMulticastPkts.jsp?ip="
						+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId") + "&ifIndex="
						+ ifIndex, 485, 685, "端口详细");
	}
}
