...
function contextMenuItemClick(item, i) {
	if (item.text == "详细信息") {
		openWindow(basePath
						...
						+ getUrlParam("ip") + "&alias=" + alias
						+ "&ifIndex=" + ifIndex + "&ifName=" + ifName, 450,
				850, "端口详细");
	}else if(item.text == "实时流速"){
		openWindow(basePath
				+ "jsp/performance/detail/host/goto/ifRealTimeFlux.jsp?ip="
				+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId")
				+ "&ifIndex=" + ifIndex, 550,
		850, "端口详细");
	}else if(item.text == "带宽"){
		openWindow(basePath
				+ "jsp/performance/detail/host/goto/ifRealTimeBandWidth.jsp?ip="
				+ getUrlParam("ip") + "&id=" + getUrlParam("nodeId")
				+ "&ifIndex=" + ifIndex, 550,
		850, "端口详细");
	}
