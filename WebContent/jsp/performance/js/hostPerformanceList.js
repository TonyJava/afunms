	...
	grid = $("#hostGrid").ligerGrid({
		columns : [{
			...
			columns : [{
						...
						render : function(item) {
							return toDetail(basePath
											...
											+ item.nodeId + '&alias=' + item.alias + '&ip='
											+ item.ip, 320, 850, "连通率详细");
						}
					}]
		}, {
			display : 'CPU(%)',
			columns : [{
						...
						width : 100,
						render : function(item) {
							return createDiv(60, item.cpuValue, item.cpuColor);
						}
					}, {
						display : 'D',
						minWidth : 20,
			...
			columns : [{
						...
						width : 120,
						render : function(item) {
							return createDiv(60, item.physicalMemoryValue, item.physicalMemoryColor);
						}
					}, {
						display : '虚拟',
						name : 'virtualMemoryValue',
						minWidth : 80,
						width : 120,
						render : function(item) {
							return createDiv(60, item.virtualMemoryValue, item.virtualMemoryColor);
						}
					}, {
						display : 'D',
						minWidth : 20,
						...
						render : function(item) {
							return toDetail(
									basePath
											+ 'jsp/performance/pDetail/hostMemoryDetail.jsp?id=2&nodeid='
											+ item.nodeId + '&alias=' + item.alias + '&ip='
											+ item.ip, 480, 850, "内存详细");
						}
					}]
		}, {
			display : '流量(KB/s)',
		...
		toolbar : {
			items : [{
						...
						text : '增加',
						click : itemclick,
						href : basePath + 'jsp/resourceManage/device/add.jsp',
						icon : 'add'
					}, {
						line : true
					}, {
						text : '修改',
						click : itemclick,
						href : basePath + 'jsp/resourceManage/device/edit.jsp?id=',
						icon : 'modify'
					}, {
						line : true
					}, {
		...
		onDblClickRow : function(data, rowindex, rowobj) {
			var url = null;
			if (type == "windows") {
				url = basePath + "jsp/performance/detail/windowsPerformanceDetail.jsp?ip="
						+ data.ip + "&alias=" + data.alias + "&nodeId=" + data.nodeId+"&type=host&subType=windows";
			} else if (type == "linux") {
				url = basePath + "jsp/performance/detail/linuxPerformanceDetail.jsp?ip=" + data.ip
						+ "&alias=" + data.alias + "&nodeId=" + data.nodeId+"&type=host&subType=linux";
			} else if (type == "aix") {
				url = basePath + "jsp/performance/detail/aixPerformanceDetail.jsp?ip=" + data.ip
						+ "&alias=" + data.alias + "&nodeId=" + data.nodeId+"&type=host&subType=aix";
			}
			openFullWindow(url, "主机详细");
		},
...
function contextMenuItemClick(item, i) {
	if (item.text == "Ping") {
		openDlgWindow(basePath + "jsp/tool/ping.jsp?ip=" + nodeIp, 380, 650, "Ping");
	} else if (item.text == "SNMP") {
		openDlgWindow(basePath + "jsp/tool/snmpCheck.jsp?nodeId=" + nodeId, 280, 650, "SNMP测试");
	}
}

function sItemclick() {
	grid.options.data = $.extend(true, {}, getHostNodeDataByType());
	grid.showFilter();
}
function itemclick(item) {
	if (item.text == "增加") {
		openWindow(item.href, 450, 520, item.text + "网元");
...
function batchCancleMonitor(string) {
	...
	$.ajax({
				type : "POST",
				async : false,
				url : basePath + "nodeHelperAjaxManager.ajax?action=batchCancleMonitor",
				// 参数
				data : {
					string : string
				},
...
function getHostNodeDataByType() {
	var rs = null;
	$.ajax({
				type : "POST",
				async : false,
				url : basePath + "hostPerformanceAjaxManager.ajax?action=getHostNodeDataByType",
				// 参数
				data : {
					type : type
				},
