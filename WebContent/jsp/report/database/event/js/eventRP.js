...
function f_exportPingWord(id) {
	...
	$.ajax({
				...
				async : false,
				url : basePath
						+ "databaseReportAjaxManager.ajax?action=exportDatabaseEvent",
				// 参数
				data : {
					id : id,
...
function tbItemclick(item) {
	if (item.text == "生成报表") {
		...
		if (rows.length == 0) {
			...
			openDlgWindow(basePath + 'jsp/report/database/event/eventRPDetail.jsp?ids=' + idString + '&startDate=' + $("#startDate").val() + '&endDate=' + $("#endDate").val(), 450, 1000);
		}
	}
}
