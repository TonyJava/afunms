...
function f_exportMiddlewareReportForIis(id) {
	...
	$.ajax({
				...
				async : false,
				url : basePath
						+ "middlewareReportAjaxManager.ajax?action=exportMiddlewareReportForIis",
				// 参数
				data : {
					ids : id,
...
function tbItemclick(item) {
	if (item.text == "生成报表") {
		...
		if (rows.length == 0) {
			...
			var filename = f_exportMiddlewareReportForIis(idString);
...
function toDetail(href, h, w, t, type, str, ip) {
	...
	var imgString = "<img src='"
			+ basePath
			+ "css/icons/export_" + type + ".gif' style='margin-top:5px;' class='pDetail' onclick='exportIisReportForType(\"" + type + "\",\"" + str + "\",\"" + ip +"\")' />";
	return imgString;
}

function exportIisReportForType(type,str,ip) {
	var waitDlg = $.ligerDialog.waitting("正在生成报表,请稍等...");
	var filename = "";
	$.ajax({
				type : "POST",
				async : false,
				url : basePath
						+ "middlewareReportAjaxManager.ajax?action=exportIisReportForType",
				// 参数
				data : {
					str : str,
					ipaddress : ip,
