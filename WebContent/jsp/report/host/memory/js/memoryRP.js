...
// 导出word
function f_exportPingWord(id) {
	var rs = "";
	$.ajax({
				type : "POST",
				async : false,
				url : basePath
						+ "hostReportAjaxManager.ajax?action=exportHostPing",
				// 参数
				data : {
					id : id,
...
function tbItemclick(item) {
	if (item.text == "生成报表") {
		...
		if (rows.length == 0) {
			...
			openDlgWindow(basePath + 'jsp/report/host/memory/memoryRPDetail.jsp?ids=' + idString + '&startDate=' + $("#startDate").val() + '&endDate=' + $("#endDate").val(), 450, 1000);
		}
	}
}
