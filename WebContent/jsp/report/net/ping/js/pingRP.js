...
function f_exportPing(id,flag) {
	...
	$.ajax({
				...
				async : false,
				url : basePath
						+ "netReportAjaxManager.ajax?action=exportNetPing" + flag,
				// 参数
				data : {
					id : id,
...
function tbItemclick(item) {
	if (item.text == "预览") {
		...
		if (rows.length == 0) {
			...
			openDlgWindow(basePath + 'jsp/report/net/ping/pingDetail.jsp?ids=' + idString + '&startDate=' + $("#startDate").val() + '&endDate=' + $("#endDate").val(), 500, 950, "预览");
		}
