...
function initData() {
	$.ajax({
				type : "POST",
				async : false,
				url : basePath + "alarmWayAjaxManager.ajax?action=beforeEditAlarmWay",
				dataType : "json",
				data : {
					id : id
				},
				success : function(array) {
					if (array) {
						$.each(array, function(n, value) {
									var form = liger.get("baseFrom");
									form.setData({
												alarmWayId : this.alarmWay[0].alarmWayId,
												name : this.alarmWay[0].name,
												isDefault : this.alarmWay[0].isDefault,
												isSystem : this.alarmWay[0].isSystem,
												remark : this.alarmWay[0].remark
											});
									if (this.alarmWay[0].isMail == 1) {
										liger.get('mailCheck').setValue(true);
										$("#mailConfigDiv").css('display', 'block');
									}
									if (this.alarmWay[0].isSms == 1) {
										liger.get('smsCheck').setValue(true);
										$("#smsConfigDiv").css('display', 'block');
									}
									if (this.alarmWay[0].isSound == 1) {
										liger.get('soundCheck').setValue(true);
										$("#soundConfigDiv").css('display', 'block');
									}

									if (this.mailConfigJson) {
										mailGridManager.set({
													data : this.mailConfigJson[0]
												});
									}

									if (this.smsConfigJson) {
										smsGridManager.set({
													data : this.smsConfigJson[0]
												});
									}

									if (this.soundConfigJson) {
										soundGridManager.set({
													data : this.soundConfigJson[0]
												});
									}
								});
					}
...
function f_save() {
	$.ajax({
				type : "POST",
				async : false,
				url : basePath + "alarmWayAjaxManager.ajax?action=editAlarmWay",
				// 参数
				contentType : "application/x-www-form-urlencoded; charset=UTF-8",
				data : {
					alarmWayId : $('#alarmWayId').attr("value"),
