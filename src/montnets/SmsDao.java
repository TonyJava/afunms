...
public class SmsDao extends BaseDao implements DaoInterface {
	...
	public List findByEvent(String eventlist, String starttime, String endtime) { 
		List list = new ArrayList();
		try {
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				rs = conn
						.executeQuery("select * from sms_server where eventlist like '%" + eventlist + "' and eventtime >= '" + starttime + "' and eventtime <= '" + endtime + "'");
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				rs = conn.executeQuery("select * from sms_server where eventlist like '%" + eventlist + "' and eventtime >= to_date('" + starttime
						+ "','YYYY-MM-DD HH24:MI:SS') and eventtime <= to_date('" + endtime + "','YYYY-MM-DD HH24:MI:SS')");
			}
