...
/**
 * 此类为静态变量类所有需要在内存中的数据都必须在此类中定义 并且定义的变量需要做详细的注释，例如key是什么，里面保存的是什么类型的变量
 * 
 * 
 */
...
public class nmsmemorydate {
	...
	/**
	 * 运行的采集任务对象列表，key是nodeid，value 为nms_host_node 数据库表 以hashtable的方式保存
	 */

	/**
	 * 
	 * 内存中保持采集任务表的信息 nms_gather_indicators_node id为key value 为数据库记录
	 * 
	 */
