...
public class LoadAixFile {
	...
	SystemCollectEntity systemdata = new SystemCollectEntity();
	...
	CpuCollectEntity cpuData = new CpuCollectEntity();
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (errptconfig != null) {
						if (errptconfig.getErrpttype().contains(errptlog.getErrpttype().toLowerCase()) || errptconfig.getErrptclass().contains(errptlog.getErrptclass().toLowerCase())) {
							...
							eventlist.setContent("设备IP：" + host.getIpAddress() + " 设备名称：" + host.getAlias() + "， errpt告警信息    级别：" + ErrptlogUtil.getTypename(errptlog.getErrpttype()) + " 种类:"
									+ ErrptlogUtil.getClassname(errptlog.getErrptclass()));
	...
	public void handleProcessor(StringBuffer fileContent) {
		...
		for (int i = 0; i < processorconfigLineArr.length; i++) {
			...
			if (processorLineToStringArr == null || processorLineToStringArr.length < 3) {
				continue;
			}
			...
			if (lineArray != null && lineArray.length > 0) {
				for (int i = 1; i < lineArray.length; i++) {
					if (lineArray[i].trim().indexOf("System configuration:") >= 0) {
						...
						if (lineArray[i].trim().indexOf("ent=") >= 0) {
							memoryString = (lineArray[i].trim().substring(lineArray[i].trim().indexOf("mem="), lineArray[i].trim().indexOf("ent="))).trim();
							memoryString = memoryString.substring(memoryString.indexOf("mem="), memoryString.length() - 2);
						} else {
							memoryString = (lineArray[i].trim().substring(lineArray[i].trim().indexOf("mem="), lineArray[i].trim().length() - 2));
						}
	...
	public void processData() {
		...
		if (pagePerformanceHashtable != null && pagePerformanceHashtable.size() > 0) {
			returnHash.put("pagehash", pagePerformanceHashtable);
