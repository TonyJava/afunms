		...
		if (dbmonitorlist != null) {
			...
			try {
				...
				try {
					...
					rs.close();
					returndata.put("topSqlReadWriteVector", topSqlReadWriteVector);
			...
			try {
				...
				Vector<OracleTopSqlReadWrite> topSqlReadWriteVector = (Vector<OracleTopSqlReadWrite>) returndata.get("topSqlReadWriteVector");
				if (topSqlReadWriteVector != null && topSqlReadWriteVector.size() > 0) {
					...
					for (int i = 0; i < topSqlReadWriteVector.size(); i++) {
						OracleTopSqlReadWrite oracleTopSqlReadWrite = topSqlReadWriteVector.get(i);
						if (oracleTopSqlReadWrite != null) {
							...
							insertsql.append("insert into nms_oratopsql_readwrite(serverip,sqltext,totaldisk,totalexec,diskreads,mon_time) values ('");
							insertsql.append(serverip);
							insertsql.append("','");
							insertsql.append(sqltext);
							insertsql.append("','");
							insertsql.append(oracleTopSqlReadWrite.getTotaldisk());
							insertsql.append("','");
							insertsql.append(oracleTopSqlReadWrite.getTotalexec());
							insertsql.append("','");
							insertsql.append(oracleTopSqlReadWrite.getDiskreads());
							if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
								insertsql.append("','");
								...
								insertsql.append("',");
								insertsql.append("to_date('" + montime + "','YYYY-MM-DD HH24:MI:SS')");
								insertsql.append(")");
							}
				...
				// topSqlSortVector
				Vector<OracleTopSqlSort> topSqlSortVector = (Vector<OracleTopSqlSort>) returndata.get("topSqlSortVector");
				if (topSqlSortVector != null && topSqlSortVector.size() > 0) {
					...
					for (int i = 0; i < topSqlSortVector.size(); i++) {
						OracleTopSqlSort oracleTopSqlSort = topSqlSortVector.get(i);
						if (oracleTopSqlSort != null) {
							...
							insertsql = new StringBuffer();
							insertsql.append("insert into nms_oratopsql_sort(serverip,sqltext,sorts,executions,sortsexec,mon_time) values ('");
							insertsql.append(serverip);
							insertsql.append("','");
							insertsql.append(sqltext);
							insertsql.append("','");
