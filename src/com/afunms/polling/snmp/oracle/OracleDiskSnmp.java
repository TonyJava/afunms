		...
		if (dbmonitorlist != null) {
				...
				try {
					...
					try {
						...
						while (rs.next()) {
							...
							diskHash.put("DISK_NUMBER", rs.getString("DISK_NUMBER").toString());
							diskHash.put("GROUP_NUMBER", rs.getString("GROUP_NUMBER").toString());
							diskHash.put("MOUNT_STATUS", rs.getString("MOUNT_STATUS").toString());
							diskHash.put("MODE_STATUS", rs.getString("MODE_STATUS").toString());
							diskHash.put("STATE", rs.getString("STATE").toString());
							diskHash.put("TOTAL_MB", rs.getString("TOTAL_MB").toString());
							diskHash.put("FREE_MB", rs.getString("FREE_MB").toString());
							diskHash.put("NAME", rs.getString("NAME").toString());
			...
			if (diskVector != null && !diskVector.isEmpty()) {
				...
				try {
					...
					String deletesql = "delete from nms_oradisk where serverip='" + serverip + "'";
					GathersqlListManager.Addsql(deletesql);

					for (int k = 0; k < diskVector.size(); k++) {
						Hashtable diskValue = (Hashtable) diskVector.get(k);
						String DISK_NUMBER = diskValue.get("DISK_NUMBER").toString();
						String GROUP_NUMBER = diskValue.get("GROUP_NUMBER").toString();
						...
						String insertsql = "insert into nms_oradisk(serverip,DISK_NUMBER,MOUNT_STATUS,MODE_STATUS,STATE,TOTAL_MB,FREE_MB,NAME,GROUP_NUMBER,mon_time) " + "values('"
								+ serverip + "'," + DISK_NUMBER + ",'" + MOUNT_STATUS + "','" + MODE_STATUS + "','" + STATE + "','" + TOTAL_MB + "','" + FREE_MB + "','" + NAME
								+ "'," + GROUP_NUMBER;
					...
					try {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							if ("disk".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								String diskstatus = "1";
								...
								checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, diskstatus, "disk");
							}
						}
					} catch (Exception e) {
						e.printStackTrace();
