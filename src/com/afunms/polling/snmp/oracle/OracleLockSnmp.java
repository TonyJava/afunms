...
public class OracleLockSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		if (dbmonitorlist != null) {
			...
			try {
				...
				try {
					...
					String sql_1 = "select distinct substr(s.username,1,18) username,s.sid as sid,s.status status,s.MACHINE machine,"
							+ "s.type sessiontype,to_char(s.LOGON_TIME,'yyyy-mm-dd hh24-mi-ss') logontime," + "substr(s.program,1,15) program from v$session s";
			...
			if (lockinfo_v != null && !lockinfo_v.isEmpty()) {
				...
				try {
					...
					for (int k = 0; k < lockinfo_v.size(); k++) {
						...
						insertsql = "";
						insertsql = "insert into nms_oralock(serverip,username,status,machine,sessiontype,logontime,program,locktype,lmode,requeststr,mon_time) " + "values('"
								+ serverip + "','" + usernames + "','" + status + "','" + machine + "','" + sessiontype + "','" + logontime + "','" + program + "','" + locktype
								+ "','" + lmode + "','" + requeststr;
			...
			// oracleLockInfo
			OracleLockInfo oracleLockInfo = (OracleLockInfo) returndata.get("oracleLockInfo");
			if (oracleLockInfo != null) {
				...
				try {
					...
					GathersqlListManager.Addsql(deletesql);
					String[] fields = { "lockwaitcount", "deadlockcount", "processcount", "maxprocesscount", "currentsessioncount", "useablesessioncount", "useablesessionpercent",
							"lockdsessioncount", "rollbackcommitpercent", "rollbacks" };
					...
					for (String field : fields) {
						...
						insertsql.append("insert into nms_oralockinfo(serverip,entity,subentity,thevalue,mon_time) values ('");
