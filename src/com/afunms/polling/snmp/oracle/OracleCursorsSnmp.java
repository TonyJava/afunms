		...
		if (dbmonitorlist != null) {
			...
			if (cursorsHash != null && !cursorsHash.isEmpty()) {
				...
				try {
					...
					String deletesql = "delete from nms_oracursors where serverip='" + ip + "'";
					// 加入队列
					GathersqlListManager.Addsql(deletesql);

					String curconnect = String.valueOf(cursorsHash.get("curconnect"));
					String opencur = String.valueOf(cursorsHash.get("opencur"));
					String insertsql = "insert into nms_oracursors(serverip, opencur, curconnect, mon_time) " + "values('" + ip + "','" + opencur + "','" + curconnect;
					...
					try {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							if ("opencur".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								if (opencur != null) {
									checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, opencur);
								}
							}
						}
					} catch (Exception e) {
