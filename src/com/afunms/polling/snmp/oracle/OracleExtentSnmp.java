		...
		if (dbmonitorlist != null) {
			...
			try {
				...
				String sqlExtent = "select a.tablespace_name, sum(a.extents) from dba_segments a, dba_tablespaces b "
						+ "where a.tablespace_name = b.tablespace_name and b.extent_management = 'DICTIONARY' " + "group by a.tablespace_name order by sum(a.extents)";
				ResultSet rs = null;
				// 字典管理表空间中的Extent总数
			...
			if (extent_v != null && !extent_v.isEmpty()) {
				...
				try {
					...
					for (int k = 0; k < extent_v.size(); k++) {
						...
						insertsql = "insert into nms_oraextent(serverip,tablespace_name,extents,mon_time) " + "values('" + serverip + "','" + tablespace_name + "','" + extents;
