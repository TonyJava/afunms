...
public class OracleSysInfoProxy extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		if (dbmonitorlist != null) {
			...
			String[] args = new String[] { "INSTANCE_NAME", "HOST_NAME", "DBNAME", "VERSION", "STARTUP_TIME", "CREATEED_TIME", "STATUS", "ARCHIVER", "java_pool", "mon_time" };
			...
			if (logFile_v != null && !logFile_v.isEmpty()) {
				...
				if (returndata1 != null && !returndata1.isEmpty()) {
					...
					try {
						...
						String INSTANCE_NAME = returndata1.get("INSTANCE_NAME").toString();
						String HOST_NAME = returndata1.get("HOST_NAME").toString();
						String DBNAME = returndata1.get("DBNAME").toString();
						String VERSION = returndata1.get("VERSION").toString();
						String STARTUP_TIME = returndata1.get("STARTUP_TIME").toString();
						String CREATEED_TIME = returndata1.get("CREATEED_TIME").toString();
						String STATUS = returndata1.get("STATUS").toString();
						String ARCHIVER = returndata1.get("ARCHIVER").toString();
						String java_pool = "";
						...
						String insertsql = "insert into nms_orasys(serverip,INSTANCE_NAME,HOST_NAME,DBNAME,VERSION,STARTUP_TIME,created_time,status,ARCHIVER,BANNER,java_pool,mon_time) "
								...
								+ STARTUP_TIME
								+ "','"
								+ CREATEED_TIME + "','" + STATUS + "','" + ARCHIVER + "','" + "" + "','" + java_pool;
			...
			args = new String[] { ".*" };
			htKey = "baseInfoHash";
			Hashtable returndata2 = LogParser.parse(fileContent, htKey, LogParser.lineseperator, args, "");
			...
			if (logFile_v != null && !logFile_v.isEmpty()) {
				...
				try {
					...
					for (int k = 0; k < logFile_v.size(); k++) {
						...
						insertsql.append("insert into nms_orabaseinfo(serverip,entity,subentity,thevalue,mon_time) values ('");
