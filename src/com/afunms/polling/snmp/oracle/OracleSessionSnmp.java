			...
			try {
				...
				try {
					...
					String sqlSession = "select substr(s.username,1,18) username,s.status status,s.MACHINE machine,"
							+ "s.type sessiontype, to_char(s.LOGON_TIME,'yyyy-mm-dd hh24-mi-ss') logontime," + "substr(s.program,1,15) program,decode(s.command, 0,'No Command',1,"
							+ "'Create Table',2,'Insert',3,'Select',6,'Update',7,'Delete',9," + "'Create Index',15,'Alter Table',21,'Create View',23,'Validate Index',35,"
							...
							+ "where s.paddr = p.addr and   s.taddr = t.addr (+) and   t.xidusn = r.usn (+) and" + "  r.usn = n.usn (+) order by logon_time desc";
