	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		if (dbmonitorlist != null) {
			...
			try {
				...
				try {
					// 回滚段信息
					String sqlRollback = "select distinct name \"Rollback Segment\", " + "optsize \"Optimal\",shrinks, AveShrink \"Average Shrink\",Wraps, "
			...
			if (rollbackinfo_v != null && !rollbackinfo_v.isEmpty()) {
				...
				try {
					...
					for (int k = 0; k < rollbackinfo_v.size(); k++) {
						...
						insertsql = "";
						insertsql = "insert into nms_orarollback(serverip,rollback,wraps,shrink,ashrink,extend,mon_time) " + "values('" + serverip + "','" + rollback + "','"
								+ wraps + "','" + shrink + "','" + ashrink + "','" + extend;
