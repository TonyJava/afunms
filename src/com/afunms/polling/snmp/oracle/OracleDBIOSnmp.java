		...
		if (dbmonitorlist != null) {
			...
			try {
				...
				String sqldbio = "SELECT DF.TABLESPACE_NAME NAME,DF.FILE_NAME FILENAME,F.PHYRDS PYR,"
						+ "F.PHYBLKRD PBR,F.PHYWRTS PYW, F.PHYBLKWRT PBW FROM V$FILESTAT F, DBA_DATA_FILES DF WHERE F.FILE# = DF.FILE_ID " + " ORDER BY DF.TABLESPACE_NAME";
				ResultSet rs = null;
				try {
			...
			if (dbiohash != null && !dbiohash.isEmpty()) {
				...
				try {
					...
					while (ipidIterator.hasNext()) {
						...
						insertsql = "insert into nms_oradbio(serverip, name, filename, pyr, pbr, pyw, pbw,mon_time) " + "values('" + serverip + "','" + name + "','" + filename
								+ "','" + pyr + "','" + pbr + "','" + pyw + "','" + pbw;
