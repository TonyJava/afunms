...
public class OracleMemorySnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		if (dbmonitorlist != null) {
			...
			// 保存至数据库中
			Hashtable memPerfValueHash = (Hashtable) returndata.get("memoryPerf");
			if (memPerfValueHash != null && !memPerfValueHash.isEmpty()) {
				...
				try {
					...
					String deletesql = "delete from nms_oramemperfvalue where serverip='" + ip + "'";
					// 加入队列
					GathersqlListManager.Addsql(deletesql);

					String pctmemorysorts = String.valueOf(memPerfValueHash.get("pctmemorysorts"));
					String pctbufgets = String.valueOf(memPerfValueHash.get("pctbufgets"));
					String dictionarycache = String.valueOf(memPerfValueHash.get("dictionarycache"));
					String buffercache = String.valueOf(memPerfValueHash.get("buffercache"));
					String librarycache = String.valueOf(memPerfValueHash.get("librarycache"));
					String insertsql = "insert into nms_oramemperfvalue(serverip, pctmemorysorts, pctbufgets, dictionarycache, buffercache, librarycache,mon_time) " + "values('"
							+ ip + "','" + pctmemorysorts + "','" + pctbufgets + "','" + dictionarycache + "','" + buffercache + "','" + librarycache;
					...
					try {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							if ("buffercache".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								if (buffercache != null) {
									checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, buffercache);
								}
							}
							if ("dictionarycache".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								if (dictionarycache != null) {
									checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, dictionarycache);
								}
							}
							if ("pctmemorysorts".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								if (pctmemorysorts != null) {
									checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, pctmemorysorts);
								}
							}
							if ("pctbufgets".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								if (pctbufgets != null) {
									checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, pctbufgets);
								}
							}
						}
					} catch (Exception e) {
