		...
		if (dbmonitorlist != null) {
				...
				try {
					...
					try {
						...
						while (rs.next()) {
							...
							asmHash.put("GROUP_NUMBER", rs.getString("GROUP_NUMBER").toString());
							asmHash.put("DB_NAME", rs.getString("DB_NAME").toString());
							asmHash.put("STATUS", rs.getString("STATUS").toString());
							asmHash.put("SOFTWARE_VERSION", rs.getString("SOFTWARE_VERSION").toString());
							asmHash.put("COMPATIBLE_VERSION", rs.getString("COMPATIBLE_VERSION").toString());
			...
			if (asmClientVector != null && !asmClientVector.isEmpty()) {
				...
				try {
					...
					String deletesql = "delete from nms_oraasmclient where serverip='" + serverip + "'";
					GathersqlListManager.Addsql(deletesql);

					for (int k = 0; k < asmClientVector.size(); k++) {
						Hashtable asmClientValue = (Hashtable) asmClientVector.get(k);
						String GROUP_NUMBER = asmClientValue.get("GROUP_NUMBER").toString();
						...
						String insertsql = "insert into nms_oraasmclient(serverip,GROUP_NUMBER,DB_NAME,STATUS,SOFTWARE_VERSION,COMPATIBLE_VERSION,mon_time) " + "values('"
								+ serverip + "','" + GROUP_NUMBER + "','" + DB_NAME + "','" + STATUS + "','" + SOFTWARE_VERSION + "','" + COMPATIBLE_VERSION;
					...
					try {
						...
						for (int i = 0; i < list.size(); i++) {
							...
							if ("asmclient".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
								...
								checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, asmclientstatus, "asmclient");
							}
						}
					} catch (Exception e) {
						e.printStackTrace();
