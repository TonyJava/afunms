			...
			if (tablespace_v != null && !tablespace_v.isEmpty()) {
				...
				try {
					...
					for (int k = 0; k < tablespace_v.size(); k++) {
						...
						insertsql = "";
						insertsql = "insert into nms_oraspaces(serverip,tablespace,free_mb,size_mb,percent_free,file_name,status,chunks,mon_time) " + "values('" + serverip + "','"
								+ tablespace + "','" + free_mb + "','" + size_mb + "','" + percent_free + "','" + file_name + "','" + status + "','" + chunks;
