...
public class LoadHpNasFile {
			...
			try {
				...
				if (hpnasVector != null && hpnasVector.size() > 0) {
					...
					for (int i = 0; i < hpnasVector.size(); i++) {
						...
						String sql = "";
						try {
							sql = "insert into nms_hpnas(ipaddress,server_name,state,cpu,net_io,disk_io,backup,ha,mon_time) " + "values('" + ipaddress + "','"
									+ nashash.get("server_name").toString() + "','" + nashash.get("state").toString() + "','" + nashash.get("cpu").toString() + "','"
									+ nashash.get("net_io").toString() + "','" + nashash.get("disk_io").toString() + "','" + nashash.get("backup").toString() + "','"
									+ nashash.get("ha").toString() + "','" + nashash.get("collecttime").toString() + "')";
	...
	public Hashtable collect_Data(NodeGatherIndicators gatherIndicatorsNode) {
		...
		try {
			...
			NodeDTO nodeDTO = nodeUtil.conversionToNodeDTO(nas);
			// 判断是否存在此告警指标
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ("filestatus".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
					checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, logstatus);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (nasStr != null && nasStr.length > 0) {
			...
			try {
				...
				for (int i = 0; i < list.size(); i++) {
					AlarmIndicatorsNode alarmIndicatorsNode = (AlarmIndicatorsNode) list.get(i);
					if ("nasstatus".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
						for (int k = 0; k < nasVector.size(); k++) {
							...
							checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, status, sIndex);
						}
					}
				}
			} catch (Exception e) {
		...
		try {
			new LoadHpNasFile().addHpnas(returnHash);
		} catch (Exception e) {
			e.printStackTrace();
		}
