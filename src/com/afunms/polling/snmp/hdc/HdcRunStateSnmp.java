...
public class HdcRunStateSnmp extends SnmpMonitor {
	...
	public void CreateResultTosql(Hashtable dataresult, Host node) {
		if (dataresult != null && dataresult.size() > 0) {
			...
			String endsql = "')";
			String deleteSql = "delete from hdc_run_state where nodeid='" + node.getId() + "'";
			...
			if (sysInfoVector != null && sysInfoVector.size() > 0) {
				for (int i = 0; i < sysInfoVector.size(); i++) {
					...
					sbuffer.append("'").append(hdcVo.getDkcRaidListIndexSerialNumber()).append("',");
					sbuffer.append("'").append(hdcVo.getDkcHWProcessor()).append("',");
