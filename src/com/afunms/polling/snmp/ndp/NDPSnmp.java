	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		try {
			...
			try {
				...
				try {
					valueArray = SnmpUtils.walkTable(node.getIpAddress(), node.getCommunity(), oids, node.getSnmpversion(), node.getSecuritylevel(), node.getSecurityName(), node.getV3_ap(), node.getAuthpassphrase(), node.getV3_privacy(),
				...
				if (valueArray.length == 1) {
					if (valueArray[0][0] == null || valueArray[0][1] == null) {
						try {
							valueArray = SnmpUtils.walkTable(node.getIpAddress(), node.getCommunity(), oids, node.getSnmpversion(), node.getSecuritylevel(), node.getSecurityName(), node.getV3_ap(), node.getAuthpassphrase(), node
									.getV3_privacy(), node.getPrivacyPassphrase(), 3, 1000 * 30);
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
			...
			for (int i = 0; i < ndpVector.size(); i++) {
				...
				if (!formerNDP.containsKey(ndp.getDeviceId() + "|" + ndp.getPortName())) {
					...
					if (nodehash.containsKey(ndp.getDeviceId())) {
						...
						newNDP.add(ndp);
						// 计算链路关系 开始
						String ipaddress = (String) nodehash.get(ndp.getDeviceId()); // nodehash===>key:mac
						// value:ip
													...
													if (vo != null) {
														...
														try {
															...
															if (!existLinkHash.containsKey(link.getStartId() + ":" + link.getStartIndex() + ":" + link.getEndId() + ":" + link.getEndIndex())
																...
																lr.setLinkName(link.getLinkName());
																lr.setMaxSpeed(link.getMaxSpeed());
																lr.setMaxPer(link.getMaxPer());
						...
						if (ipmacHash.containsKey(ndp.getDeviceId())) {
							...
							try {
								...
								int addResult = 0;
								addResult = helper.addHost(node.getAssetid(), node.getLocation(), ip, ip, node.getSnmpversion(), node.getCommunity(), node.getWritecommunity(), node.getTransfer(), 2, node.getOstype(), 1, node.getBid(), node
										.getSendmobiles(), node.getSendemail(), node.getSendphone(), node.getSupperid(), true, node.getSecuritylevel(), node.getSecurityName(), node.getV3_ap(), node.getAuthpassphrase(),
										node.getV3_privacy(), node.getPrivacyPassphrase(), layer + 1);
												...
												if (_host != null) {
													...
													try {
														if (ipaliasHash.containsKey(ip)) {
															...
															String _nodeIp = "";
															if (nodelistHash.containsKey(ip)) {
																_nodeIp = (String) nodelistHash.get(ip);
															}
															if (_nodeIp == null || _nodeIp.trim().length() == 0) {
																if (ipaliasHash.containsKey(ip)) {
																	_nodeIp = (String) ipaliasHash.get(ip);
																}
															}
															Host p_host = null;
															if (_nodeIp != null && _nodeIp.trim().length() > 0) {
														...
														TopoHelper helper = new TopoHelper(); // 包括更新数据库和更新内存
														int addResult = 0;
														addResult = helper.addHost(node.getAssetid(), node.getLocation(), ip, ip, node.getSnmpversion(), node.getCommunity(), node.getWritecommunity(), node.getTransfer(), 2,
																node.getOstype(), 1, node.getBid(), node.getSendmobiles(), node.getSendemail(), node.getSendphone(), node.getSupperid(), true, node.getSecuritylevel(), node.getSecurityName(),
					...
					if (!ndpFromDbHash.containsKey(ndp.getDeviceId() + "|" + ndp.getPortName())) {
						...
						if (nodehash.containsKey(ndp.getDeviceId())) {
							...
							String nodeIp = "";
							if (nodelistHash.containsKey(ipaddress)) {
								nodeIp = (String) nodelistHash.get(ipaddress);
							}
							if (nodeIp == null || nodeIp.trim().length() == 0) {
								if (ipaliasHash.containsKey(ipaddress)) {
									nodeIp = (String) ipaliasHash.get(ipaddress);
								}
							}
							ipaddress = nodeIp;
							Host _host = (Host) PollingEngine.getInstance().getNodeByIp(ipaddress);
							...
							if (ipmacHash.containsKey(ndp.getDeviceId())) {
								...
								try {
									...
									if (ipaliasHash.containsKey(ip)) {
										...
										TopoHelper helper = new TopoHelper(); // 包括更新数据库和更新内存
										int addResult = 0;
										addResult = helper.addHost(node.getAssetid(), node.getLocation(), ip, ip, node.getSnmpversion(), node.getCommunity(), node.getWritecommunity(), node.getTransfer(), 2, node.getOstype(), 1, node
												.getBid(), node.getSendmobiles(), node.getSendemail(), node.getSendphone(), node.getSupperid(), true, node.getSecuritylevel(), node.getSecurityName(), node.getV3_ap(), node
												.getAuthpassphrase(), node.getV3_privacy(), node.getPrivacyPassphrase(), layer + 1);
										...
										if (ShareData.getMacNDP().containsKey(ip + ":" + node.getIpAddress())) {
											...
											if (macList.contains(ndp.getDeviceId())) {
												...
												try {
													TopoHelper helper = new TopoHelper(); // 包括更新数据库和更新内存
													int addResult = 0;
													addResult = helper.addHost(node.getAssetid(), node.getLocation(), ip, ip, node.getSnmpversion(), node.getCommunity(), node.getWritecommunity(), node.getTransfer(), 2, node.getOstype(), 1,
															node.getBid(), node.getSendmobiles(), node.getSendemail(), node.getSendphone(), node.getSupperid(), true, node.getSecuritylevel(), node.getSecurityName(), node.getV3_ap(), node
																	.getAuthpassphrase(), node.getV3_privacy(), node.getPrivacyPassphrase(), layer + 1);
