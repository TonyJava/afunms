	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		try {
			...
			try {
				...
				valueArray = SnmpUtils.getTemperatureTableData(node.getIpAddress(), node.getCommunity(), oids, node.getSnmpversion(), 1, "", 1, "", 1, "", 3, 1000 * 30);
				int flag = 0;
				if (valueArray != null) {
		...
		if (!(ShareData.getSharedata().containsKey(node.getIpAddress()))) {
			...
			if (fanVector != null && fanVector.size() > 0) {
				ipAllData.put("rbutter", fanVector);
			}
			ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		} else {
			if (fanVector != null && fanVector.size() > 0) {
				((Hashtable) ShareData.getSharedata().get(node.getIpAddress())).put("rbutter", fanVector);
			}
