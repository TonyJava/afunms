	...
	public static void createInformixSpaceSMS(DBVo dbmonitorlist, Informixspaceconfig informixspaceconfig) {
		...
		try {
			if (!sendeddata.containsKey(ipaddress + ":" + informixspaceconfig.getSpacename())) {
				...
				smscontent.setMessage(dbmonitorlist.getIpAddress() + "的数据库的" + informixspaceconfig.getSpacename() + "表空间超过" + informixspaceconfig.getAlarmvalue() + "%的阀值");
				...
				if (subvalue / (1000 * 60 * 60 * 24) >= 1) {
					...
					alarminfo.setContent(dbmonitorlist.getIpAddress() + "的数据库的" + informixspaceconfig.getSpacename() + "表空间超过" + informixspaceconfig.getAlarmvalue() + "%的阀值");
					...
					// //
					String message = dbmonitorlist.getIpAddress() + "的数据库的" + informixspaceconfig.getSpacename() + "表空间超过" + informixspaceconfig.getAlarmvalue() + "%的阀值";
					SmscontentDao content = new SmscontentDao();
					content.createEventWithReasion("poll", dbmonitorlist.getId() + "", dbmonitorlist.getAlias() + "(" + dbmonitorlist.getIpAddress() + ")", message, 2, "db",
							"informixspace", "表空间超过阀值");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
									...
									if (spaceList != null && spaceList.size() > 0) {
										...
										for (int k = 0; k < spaceList.size(); k++) {
											...
											if (informixspaces.containsKey(serverip + ":" + tablespace)) {
												...
												// 依据表空间告警配置判断是否告警
												Informixspaceconfig informixspaceconfig = (Informixspaceconfig) informixspaces.get(serverip + ":" + tablespace);
