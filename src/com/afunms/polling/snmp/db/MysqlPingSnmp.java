			...
			if (dbmonitorlist != null) {
				...
				for (int k = 0; k < dbs.length; k++) {
					...
					try {
						...
						String port=dbmonitorlist.getPort();
						mysqlIsOK = dbdao.getMySqlIsOk(serverip,port, username, passwords, dbStr);
					} catch (Exception e) {
						mysqlIsOK = false;
					} finally {
						dbdao.close();
				...
				if (!(ShareData.getSharedata().containsKey(dbmonitorlist.getIpAddress() + ":" + dbmonitorlist.getId()))) {
					...
					Hashtable mysqlHash = (Hashtable) ShareData.getSharedata().get(dbmonitorlist.getIpAddress() + ":" + dbmonitorlist.getId());
					mysqlHash.put("ping", returndata.get("ping"));
				}
				...
				try {
					...
					hostdata.setIpaddress(dbmonitorlist.getIpAddress() + ":" + dbmonitorlist.getId());
