...
public class SQLServerDataCollector {
	...
	public static void createSqldbSMS(DBVo dbmonitorlist, Sqldbconfig sqldbconfig) {
		...
		if (sqldbconfig.getLogflag() == 0) {
			// 库文件
			errorcontent = dbmonitorlist.getIpAddress() + "的" + dbmonitorlist.getDbName() + "的" + sqldbconfig.getDbname() + "的库空间超过" + sqldbconfig.getAlarmvalue() + "%的阀值";
		} else {
			// 日志文件
			errorcontent = dbmonitorlist.getIpAddress() + "的" + dbmonitorlist.getDbName() + "的" + sqldbconfig.getDbname() + "的日志超过" + sqldbconfig.getAlarmvalue() + "%的阀值";
		}
		...
		try {
			if (!sendeddata.containsKey(ipaddress + ":" + sqldbconfig.getDbname() + ":" + sqldbconfig.getLogflag())) {
				...
				Calendar formerdate = (Calendar) sendeddata.get(ipaddress + ":" + sqldbconfig.getDbname() + ":" + sqldbconfig.getLogflag());
	...
	public void collect_data(String dbid, Hashtable gatherHash) {
		...
		try {
			...
			if (dbnode.getCollecttype() == SystemConstant.DBCOLLECTTYPE_SHELL) {
				// 脚本采集方式
				String filename = ResourceCenter.getInstance().getSysPath() + "/linuxserver/" + serverip + ".sqlserver.log";
							...
							if (ipPingData != null) {
								...
								if (pvalue == 0) {
									...
									try {
										...
										eventdao.createEventWithReasion("poll", dbmonitorlist.getId() + "", dbmonitorlist.getAlias() + "(" + dbmonitorlist.getIpAddress() + ")",
									...
									for (int k = 0; k < names.size(); k++) {
										...
										if (database.get(dbname) != null) {
											...
											if (alarmdbs.containsKey(serverip + ":" + dbname + ":0")) {
												Sqldbconfig sqldbconfig = (Sqldbconfig) alarmdbs.get(serverip + ":" + dbname + ":0");
												if (sqldbconfig == null) {
													continue;
												}
												if (usedperc == null) {
										...
										if (logfile.get(dbname) != null) {
											...
											String usedperc = (String) db.get("usedperc");
											if (alarmdbs.containsKey(serverip + ":" + dbname + ":0")) {
												Sqldbconfig sqldbconfig = (Sqldbconfig) alarmdbs.get(serverip + ":" + dbname + ":1");
												if (sqldbconfig == null) {
													continue;
												}
												if (sqldbconfig.getAlarmvalue() < new Integer(usedperc)) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ("1".equals(alarmIndicatorsNode.getEnabled())) {
				...
				if ("buffercache".equals(indicators)) {
					value = (String) memeryHashtable.get("bufferCacheHitRatio");// key
					...
					value = (String) locksHashtable.get("deadLocks");// key
				} else if ("connections".equals(indicators)) {
					value = (String) connsHashtable.get("connections");// key
				} else {
					continue;
				}
				if (value == null) {
				...
				if (AlarmConstant.DATATYPE_NUMBER.equals(alarmIndicatorsNode.getDatatype())) {
					...
					try {
						double value_int = Double.valueOf(value);// 实际值
						...
						if (num_int >= alarmTimes_int) {// 实际告警次数 >= 告警阀值
							// ，则提示报警信息 和 发送短信
							// HONGLI
							// 告警
							...
							createSMS(alarmIndicatorsNode.getType(), alarmIndicatorsNode.getSubtype(), sqlserver.getAlias(), sqlserver.getId() + "", alarmIndicatorsNode
									.getAlarm_info()
									+ " 当前值为：" + value + alarmIndicatorsNode.getThreshlod_unit(), Integer.valueOf(level), 1, sqlserver.getAlias(), sqlserver.getBid(), sqlserver
									.getAlias()
									+ "(" + sqlserver.getAlias() + ")");
						} else {
							num_int = num_int + 1;
