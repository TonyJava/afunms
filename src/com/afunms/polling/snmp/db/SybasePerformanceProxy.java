...
public class SybasePerformanceProxy extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		if (dbmonitorlist != null) {
			...
			String[] args = new String[] { "cpu_busy", "idle", "version", "io_busy", "sent_rate", "received_rate", "write_rate", "read_rate", "serverName", "cpu_busy_rate",
					"io_busy_rate", "disk_count", "locks_count", "xact_count", "total_dataCache", "total_physicalMemory", "metadata_cache", "procedure_cache",
					"total_logicalMemory", "data_hitrate", "procedure_hitrate", // "nms_sybaseperformance"
			};
			...
			if (!ShareData.getSharedata().containsKey(dbmonitorlist.getIpAddress() + ":" + dbmonitorlist.getId())) {
				...
				Hashtable sybaseHash = (Hashtable) ShareData.getSharedata().get(dbmonitorlist.getIpAddress() + ":" + dbmonitorlist.getId());
			...
			if (extent_v != null && !extent_v.isEmpty()) {
				...
				if (infoValueHash != null && !infoValueHash.isEmpty()) {
					...
					try {
						...
						String deletesql = "delete from nms_sybaseperformance where serverip='" + ip + "'";
						// 加入队列
						...
						String insertsql = "insert into nms_sybaseperformance(serverip, cpu_busy, idle, version, io_busy, sent_rate,"
								...
								+ "total_dataCache,total_physicalMemory,metadata_cache,procedure_cache,total_logicalMemory," + "data_hitrate,procedure_hitrate,mon_time) "
								+ " values ('"
								+ ip
								+ "','"
								+ cpu_busy
								...
								+ total_logicalMemory
								+ "','"
								+ data_hitrate + "','" + procedure_hitrate;
