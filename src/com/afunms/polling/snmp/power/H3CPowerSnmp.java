...
public class H3CPowerSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		try {
			...
			if (node.getSysOid().startsWith("1.3.6.1.4.1.2011.") || node.getSysOid().equalsIgnoreCase("1.3.6.1.4.1.25506.")) {
				String[] oids = new String[] { "1.3.6.1.4.1.2011.2.23.1.9.1.2.1.1",// hwDevMPowerNum
				...
				if (valueArray != null && valueArray.length > 0) {
					for (int i = 0; i < valueArray.length; i++) {
						...
						interfacedata.setEntity(parseString(valueArray[i][2]));
						interfacedata.setSubentity(parseString(valueArray[i][0]));
						interfacedata.setRestype("dynamic");
						interfacedata.setUnit("");
						interfacedata.setThevalue(parseString(parseInt(valueArray[i][0])));
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (null != powerVector && powerVector.size() > 0) {
