	...
	public Hashtable collect_Data() {
		...
		if (dbContent != null && dbContent.trim().length() > 0) {
			...
			if (db_LineArr != null && db_LineArr.length > 0) {
				for (int i = 0; i < db_LineArr.length; i++) {
					if (db_LineArr[i].indexOf("=") != -1) {
						...
						if (db_tmpData != null && db_tmpData.length > 1 && db_tmpData[0].trim().equalsIgnoreCase("Time database waited on locks (ms)")) {
							Lock_waits_Times = db_tmpData[1].trim().equalsIgnoreCase("Not Collected") ? db_tmpData[1].trim()
									: (Double.parseDouble(db_tmpData[1].trim() + "") / (Double.parseDouble(Lock_waits) == 0 ? 1 : Double.parseDouble(Lock_waits))) + "";
						}
						// 可以使用的锁数量
				...
				// 平均表空间直接写时间
				if (!write_elapsed_time.trim().equalsIgnoreCase("Not Collected") && !Direct_writes.trim().equalsIgnoreCase("Not Collected")
						&& Double.parseDouble(Direct_writes) != 0) {
