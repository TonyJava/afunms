...
public class LoadVMWare {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		String dstime = ""; // 上次更新时间,格式：2012-10-17
							// 09:42:32,SUMMARY_RESOURCE_DS_TIME,
		...
		// try{
		// SysLogger.info(host.getIpAddress()+" username:"+username+"
		// password:"+password);
		// vmids = (HashMap)vmwareutil.SyncVM(host.getIpAddress(), username,
		// password);
		...
		// HashMap<String, Object> summaryresultMap = (HashMap<String,
		// Object>)VIMMgr.syncVIMObjs(url, username, enpassword);
		//			
		// if(!(ShareData.getSharedata().containsKey(host.getIpAddress())))
		// {
		// if(ipAllData == null)ipAllData = new Hashtable();
		// if(summaryresultMap != null &&
		// summaryresultMap.size()>0)ipAllData.put("summaryresultMap",summaryresultMap);
		// ShareData.getSharedata().put(host.getIpAddress(), ipAllData);
		// }else
		// {
		// if(summaryresultMap != null &&
		// summaryresultMap.size()>0)((Hashtable)ShareData.getSharedata().get(host.getIpAddress())).put("summaryresultMap",summaryresultMap);
		// }
		// Map<String, Object> summaryresultMap1 = null;
		// if(ipAllData != null){
		// try {
		// summaryresultMap1 = (Map<String,
		// Object>)ipAllData.get("summaryresultMap");
		// } catch (Exception e) {
		...
		// summaryresultMap.get("Datastore");
		// ArrayList<HashMap<String, Object>> wulist =
		// (ArrayList<HashMap<String, Object>>)
		// summaryresultMap.get("HostSystem");
		// ArrayList<HashMap<String, Object>> rplist =
		// (ArrayList<HashMap<String, Object>>)
		// summaryresultMap.get("ResourcePool");
		// ArrayList<HashMap<String, Object>> vmlist =
		// (ArrayList<HashMap<String, Object>>)
		// summaryresultMap.get("VirtualMachine");
		// ArrayList<HashMap<String, Object>> dclist =
		// (ArrayList<HashMap<String, Object>>)
		// summaryresultMap.get("Datacenter");
		// ArrayList<HashMap<String, Object>> crlist =
		// (ArrayList<HashMap<String, Object>>)
		// summaryresultMap.get("ComputeResource");//群集
		...
		String perfCounterKeys[] = { "cpu.usage.average", "cpu.ready.summation", "mem.vmmemctl.average", "mem.swapoutRate.average", "mem.swapinRate.average",
				"cpu.usagemhz.average", "mem.usage.average", "disk.usage.average", "net.usage.average" };
		...
		if (wu_list != null && wu_list.size() > 0) {
			for (int i = 0; i < wu_list.size(); i++) {
				try {
					resultMap1 = (HashMap<String, Object>) VIMMgr.getPerformances(url, username, enpassword, VIMConstants.RESOURCE_HO, wu_list.get(i).toString(), perfIntervalKey,
							perfCounterKeys, endTime);
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				// System.out.println(wulist.get(i).get("vid").toString()+"<------物理机性能--------->"+vm.getPerformances(url,
				// username, password, vtype,
				// wulist.get(i).get("vid").toString(), perfIntervalKey,
				// perfCounterKeys, endTime).get("info.result"));
				...
				if (vm_list != null && vm_list.size() > 0) {
					for (int j = 0; j < vm_list.size(); j++) {
						...
						// (PerfEntityMetricBase[]) vm.getPerformances(url,
						// username, enpassword, VIMConstants.RESOURCE_VM,
						// vm_list.get(j).toString(), perfIntervalKey,
						// perfCounterKeys, endTime).get("info.result");

						resultMap2 = (HashMap<String, Object>) VIMMgr.getPerformances(url, username, enpassword, VIMConstants.RESOURCE_VM, vm_list.get(j).toString(),
			...
			if (!(ShareData.getVmdata().containsKey(host.getIpAddress()))) {
				if (vmData == null) {
					vmData = new Hashtable();
				}
				if (wuliji != null && wuliji.size() > 0) {
					vmData.put("wuliji", wuliji);
				}
				vmData.put("vmware", vmware);
				ShareData.getVmdata().put(host.getIpAddress(), vmData);
			} else {
				if (wuliji != null && wuliji.size() > 0) {
					((Hashtable) ShareData.getVmdata().get(host.getIpAddress())).put("wuliji", wuliji);
				}
		...
		try {
			...
			String cunchu_perfCounterKeys[] = { "disk.used.latest", "disk.provisioned.latest", "disk.capacity.latest" };
			...
			if (ds_list != null && ds_list.size() > 0) {
				for (int i = 0; i < ds_list.size(); i++) {
					// PerfEntityMetricBase[] perfEntityMetricBases =
					// (PerfEntityMetricBase[]) vm.getPerformances(url,
					// username, password, VIMConstants.RESOURCE_DS,
					// dslist.get(i).get("vid").toString(), perfIntervalKey,
					// cunchu_perfCounterKeys, endTime).get("info.result");
					List valuelist = new ArrayList();
					resultMap = (HashMap<String, Object>) VIMMgr.getPerformances(url, username, enpassword, VIMConstants.RESOURCE_DC_DATASTORE, ds_list.get(i).toString(),
							perfIntervalKey, cunchu_perfCounterKeys, endTime);
					...
					if ("success".equals(resultMap.get(VIMConstants.INFO_STATE))) {
						...
						if (null != perfEntityMetricBases) {
							for (PerfEntityMetricBase perfEntityMetricBase : perfEntityMetricBases) {
								PerfEntityMetricCSV csvMetric = (PerfEntityMetricCSV) perfEntityMetricBase;
								PerfMetricSeriesCSV[] perfMetricSeriesCSVs = csvMetric.getValue();
								// ds_map.put(dslist.get(i).get("vid").toString(),
								// perfMetricSeriesCSVs);
								if (null != perfMetricSeriesCSVs) {
									for (PerfMetricSeriesCSV perfMetricSeriesCSV : perfMetricSeriesCSVs) {
										int counterId = perfMetricSeriesCSV.getId().getCounterId();
										PerfCounterInfo perfCounterInfo = perfCounterInfoMap.get(counterId);
										String value = perfMetricSeriesCSV.getValue();
										// System.out.println("=================perfCounterInfo="+
				...
				if (!(ShareData.getVmdata().containsKey(host.getIpAddress()))) {
					...
					if (ds_list != null && ds_list.size() > 0) {
						((Hashtable) ShareData.getVmdata().get(host.getIpAddress())).put("ds", ds_map);
					}
				}
			}
		} catch (Exception e) {
		...
		try {
			...
			if (cr_list != null && cr_list.size() > 0) {
				for (int i = 0; i < cr_list.size(); i++) {
					// PerfEntityMetricBase[] perfEntityMetricBases =
					// (PerfEntityMetricBase[]) vm.getPerformances(url,
					// username, password, VIMConstants.RESOURCE_CCR,
					// crlist.get(i).get("vid").toString(), perfIntervalKey,
					// quji_perfCounterKeys, endTime).get("info.result");
					// cr_map.put(crlist.get(i).get("vid").toString(),
					// perfEntityMetricBases);
					// SysLogger.info("----------采集集群-----------"+perfEntityMetricBases);
					List valuelist = new ArrayList();
					resultMap = (HashMap<String, Object>) VIMMgr.getPerformances(url, username, enpassword, VIMConstants.RESOURCE_CCR, cr_list.get(i).toString(), perfIntervalKey,
							quji_perfCounterKeys, endTime);
					if ("success".equals(resultMap.get(VIMConstants.INFO_STATE))) {
						PerfEntityMetricBase[] perfEntityMetricBases = (PerfEntityMetricBase[]) resultMap.get(VIMConstants.INFO_RESULT);
						// 将所有的性能指标放入Map,以perfCounterInfo.getKey为Map的key
						HashMap<Integer, PerfCounterInfo> perfCounterInfoMap = new HashMap<Integer, PerfCounterInfo>();
			...
			if (!(ShareData.getVmdata().containsKey(host.getIpAddress()))) {
				...
				if (cr_list != null && cr_list.size() > 0) {
					((Hashtable) ShareData.getVmdata().get(host.getIpAddress())).put("cr", cr_map);
				}
			}
		...
		try {
			...
			if (rp_list != null && rp_list.size() > 0) {
				for (int i = 0; i < rp_list.size(); i++) {
					// PerfEntityMetricBase[] perfEntityMetricBases =
					// (PerfEntityMetricBase[]) vm.getPerformances(url,
					// username, password, VIMConstants.RESOURCE_RP,
					// rplist.get(i).get("vid").toString(), perfIntervalKey,
					// ziyuanchi_perfCounterKeys, endTime).get("info.result");
					// rp_map.put(rplist.get(i).get("vid").toString(),
					// perfEntityMetricBases);
					resultMap = (HashMap<String, Object>) VIMMgr.getPerformances(url, username, enpassword, VIMConstants.RESOURCE_RP, rp_list.get(i).toString(), perfIntervalKey,
							ziyuanchi_perfCounterKeys, endTime);
					List valuelist = new ArrayList();
					if ("success".equals(resultMap.get(VIMConstants.INFO_STATE))) {
						PerfEntityMetricBase[] perfEntityMetricBases = (PerfEntityMetricBase[]) resultMap.get(VIMConstants.INFO_RESULT);
						// 将所有的性能指标放入Map,以perfCounterInfo.getKey为Map的key
			...
			if (!(ShareData.getVmdata().containsKey(host.getIpAddress()))) {
				...
				if (rp_list != null && rp_list.size() > 0) {
					((Hashtable) ShareData.getVmdata().get(host.getIpAddress())).put("rp", rp_map);
				}
			}
