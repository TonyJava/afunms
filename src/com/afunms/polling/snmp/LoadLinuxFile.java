			...
			for (int i = 0; i < memperfLineArr.length; i++) {
				...
				if (diskperf_tmpData != null && diskperf_tmpData.length >= 4) {
					if (diskperf_tmpData[0].trim().equalsIgnoreCase("Mem:")) {
						...
						float PhysicalMemUtilization = (Float.parseFloat(diskperf_tmpData[2]) - Float.parseFloat(diskperf_tmpData[5]) - Float.parseFloat(diskperf_tmpData[6]))
								* 100 / (Float.parseFloat(diskperf_tmpData[1]) );
				...
				if (result.length == 8) {
					try {
						...
						if (servicestatus.indexOf("on") >= 0 || servicestatus.indexOf("启用") >= 0) {
							service.put("state", "启用");
						} else {
							service.put("state", "未启用");
						}
