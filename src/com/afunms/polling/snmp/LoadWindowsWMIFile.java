...
public class LoadWindowsWMIFile {
	...
	public void copyFile(String ipAddress, String max) {
		try {
			String cmd = "cmd   /c   copy   " + ResourceCenter.getInstance().getSysPath() + "\\linuxserver\\" + ipAddress + ".log" + " "
					+ ResourceCenter.getInstance().getSysPath() + "\\linuxserver_bak\\" + ipAddress + ".log";
	...
	public Hashtable getTelnetMonitorDetail() {
		...
		try {
			...
			for (int j = 0; j < elements.size(); j++) {
								...
								if (endelements != null && endelements.size() > 0) {
									...
									for (int m = 0; m < endelements.size(); m++) {
										if ("StartMode".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											rValue.put(((Element) (endelements.get(m))).getName(), SnmpMibConstants.ServiceStartModeConfig.get(((Element) (endelements.get(m)))
													.getValue()));
										} else if ("State".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											rValue.put(((Element) (endelements.get(m))).getName(), SnmpMibConstants.ServiceStateConfig.get(((Element) (endelements.get(m)))
													.getValue()));
										} else if ("ServiceType".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											rValue.put(((Element) (endelements.get(m))).getName(), SnmpMibConstants.ServiceTypeConfig.get(((Element) (endelements.get(m)))
													.getValue()));
										} else {
											rValue.put(((Element) (endelements.get(m))).getName(), ((Element) (endelements.get(m))).getValue());
										}
								...
								if (endelements != null && endelements.size() > 0) {
									...
									for (int m = 0; m < endelements.size(); m++) {
										if ("LocalAccount".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											rValue.put(((Element) (endelements.get(m))).getName(), SnmpMibConstants.UserLocalAccountConfig.get(((Element) (endelements.get(m)))
													.getValue()));
										} else if ("Status".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											rValue.put(((Element) (endelements.get(m))).getName(), SnmpMibConstants.UserStatusConfig.get(((Element) (endelements.get(m)))
													.getValue()));
										} else {
											rValue.put(((Element) (endelements.get(m))).getName(), ((Element) (endelements.get(m))).getValue());
										}
						...
						for (int k = 0; k < subelements.size(); k++) {
							...
							if ("processinfo".equalsIgnoreCase(((Element) (subelements.get(k))).getName())) {
								...
								if (endelements != null && endelements.size() > 0) {
									...
									// cputime
									String cputime = Math
											.round((Float.parseFloat((String) rValue.get("KernelModeTime")) + Float.parseFloat((String) rValue.get("UserModeTime"))) / 10000000)
											+ "";
									...
									if (procsV != null && procsV.size() > 0) {
										if (procsV.contains(name)) {
											...
											if (sendeddata.containsKey(ipaddress + ":" + name)) {
				...
				/**
				 * cpu 配置信息 位数、二级缓存大小、cpu使用率、cpu的主频 需要技术cpu的 负载
				...
				/**
				 * 网卡流速信息 接收数据包、发送的数据包、接收的字节、发送的字节、网卡带宽、发送的错包数
				 * 
				 */
					...
					if (subelements != null && subelements.size() > 0) {
						for (int k = 0; k < subelements.size(); k++) {
							...
							if ("NetworkInterfaceinfo".equalsIgnoreCase(((Element) (subelements.get(k))).getName())) {
								...
								// subValue.put(((Element)(subelements.get(k))).getName(),
								// rValue);
							}
						}
					}
								...
								if (endelements != null && endelements.size() > 0) {
									...
									for (int m = 0; m < endelements.size(); m++) {
										if ("NetConnectionStatus".equalsIgnoreCase(((Element) (endelements.get(m))).getName())) {
											...
											// rValue.put(((Element)(endelements.get(m))).getName(),
											// SnmpMibConstants.NetworkStatusConfig.get(((Element)(endelements.get(m))).getValue()));
											rValue.put(((Element) (endelements.get(m))).getName(), ((Element) (endelements.get(m))).getValue());
							...
							if ("SystemMemoryinfo".equalsIgnoreCase(((Element) (subelements.get(k))).getName())) {
								...
								if (endelements != null && endelements.size() > 0) {
									...
									for (int m = 0; m < endelements.size(); m++) {
										...
										// memoryhash.put(((Element)(endelements.get(m))).getName(),
										// ((Element)(endelements.get(m))).getValue());
									}
		...
		if (networkconfiglist != null && networkconfiglist.size() > 0) {
			...
			// //得到该网卡的MAC地址和状态
			// rValue.put("NetConnectionStatus",
			// (String)statusvalue.get("NetConnectionStatus"));
			// break;
			// }
			// }
			// }
		...
		if (logicdiskperformancelist != null && logicdiskperformancelist.size() > 0) {
			...
			for (int i = 0; i < logicdiskperformancelist.size(); i++) {
				...
				if (olddisk.containsKey((String) newlogicdisk.get("Name"))) {
					...
					result = ((prevCountValue - newCountValue) / (prevCountBase - newCountBase)) * 100;
					...
					AvgDiskReadQueueLength = (Long.parseLong((String) newlogicdisk.get("AvgDiskReadQueueLength")) - Long.parseLong((String) prelogicdisk
							.get("AvgDiskReadQueueLength")));

					AvgDiskWriteQueueLength = (Long.parseLong((String) newlogicdisk.get("AvgDiskWriteQueueLength")) - Long.parseLong((String) prelogicdisk
							.get("AvgDiskWriteQueueLength")));
					...
					// disklist.put("PercentDiskTime_Base",
					...
					if (servicelist != null && servicelist.size() > 0) {
						...
						for (int j = 0; j < servicelist.size(); j++) {
							...
							if (obj instanceof Hashtable) {
								...
								ServiceCollectEntity servicecollectdata = new ServiceCollectEntity();
								try {
