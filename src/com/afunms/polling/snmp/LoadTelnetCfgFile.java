...
public class LoadTelnetCfgFile {
	...
	public void parseClass(String content, List<CfgBaseInfo> list) {
		Pattern tmpPt = null;
		Matcher mr = null;
		tmpPt = Pattern.compile("(-----------------begin\\(sh class-map\\)-----------------\n)(.*)(-----------------end\\(sh class-map\\)-----------------\n)", Pattern.DOTALL);
	...
	public void parsePolicy(String content, List<CfgBaseInfo> list) {
		Pattern tmpPt = null;
		Matcher mr = null;
		tmpPt = Pattern.compile("(-----------------begin\\(sh policy-map\\)-----------------\n)(.*)(-----------------end\\(sh policy-map\\)-----------------\n)", Pattern.DOTALL);
		...
		if (mr.find()) {
			...
			if (totalLines != null) {
				...
				for (int i = 0; i < totalLines.length; i++) {
					...
					if (totalLines[i].indexOf("(total queued/total drops/no-buffer drops)") > -1) {
						String[] lines = realBlock.toString().split("Class-map:");
						...
						if (lines != null && lines.length > 1) {
							...
							for (int j = 1; j < lines.length; j++) {
								...
								if (serNames != null) {
									...
									vo.setPolicyName(serNames[2].replace("Service-policy output:", "").trim());
								}
								String[] classLines = lines[j].split("\n");
								if (classLines != null && classLines.length > 0) {
									vo.setClassName(classLines[0].trim());
								}
								tmpPt = Pattern.compile("(5 minute offered rate)(.*)(bps,)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									vo.setOfferedRate(Integer.parseInt(match.trim()));
									// System.out.println(match.trim());
								} else {
									vo.setOfferedRate(-1);
								}
								tmpPt = Pattern.compile("(drop rate)(.*)(bps\n)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									vo.setDropRate(Integer.parseInt(match.trim()));
									// System.out.println(match.trim()+"///");
								} else {
									vo.setDropRate(-1);
								}
								tmpPt = Pattern.compile("(Match:)(.*)(\n      Queueing)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									vo.setMatchGroup(match.trim());
								}
								tmpPt = Pattern.compile("(\\(pkts matched\\/bytes matched\\))(.*)(\n        \\(total drops\\/bytes drops\\))", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									String[] values = match.split("\\/");
									if (values != null && values.length == 2) {
										vo.setMatchedPkts(Integer.parseInt(values[0].trim()));
										vo.setMatchedBytes(Integer.parseInt(values[1].trim()));
									}
								...
								tmpPt = Pattern.compile("(\\(total drops\\/bytes drops\\))(.*)(\n\n)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									String[] values = match.split("\\/");
									...
									if (values != null && values.length == 2) {
										vo.setDropsTotal(Integer.parseInt(values[0].trim()));
										vo.setDropsBytes(Integer.parseInt(values[1].trim()));
									}
								...
								// //////////////////
								tmpPt = Pattern.compile("(\\(depth\\/total drops\\/no-buffer drops\\))(.*)(\n\n)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									String[] values = match.split("\\/");
									...
									if (values != null && values.length == 3) {
										vo.setDepth(Integer.parseInt(values[0].trim()));
										vo.setDropsTotal(Integer.parseInt(values[1].trim()));
										vo.setNoBufferDrop(Integer.parseInt(values[2].trim()));
									}
								...
								// //////////////////
								tmpPt = Pattern.compile("(\\(total queued\\/total drops\\/no-buffer drops\\))(.*)(\n)", Pattern.DOTALL);
								mr = tmpPt.matcher(lines[j]);
								if (mr.find()) {
									String match = mr.group(2);
									String[] values = match.split("\\/");
									...
									if (values != null && values.length == 3) {
										vo.setTotalQueued(Integer.parseInt(values[0].trim()));
										vo.setDropsTotal(Integer.parseInt(values[1].trim()));
										vo.setNoBufferDrop(Integer.parseInt(values[2].trim()));
									}
	...
	public void parseQueue(String content, List<QueueInfo> list, String command) {
		...
		tmpPt = Pattern.compile("(-----------------begin\\(" + cmd + "\\)-----------------\n)(.*)(-----------------end\\(" + cmd + "\\)-----------------)", Pattern.DOTALL);
		mr = tmpPt.matcher(content);
		if (mr.find()) {
			String availBandwidth = "";
			String block = mr.group(2);
			tmpPt = Pattern.compile("(Available Bandwidth)(.*)(kilobits\\/sec)", Pattern.DOTALL);
			...
			tmpPt = Pattern.compile("(Input queue:)(.*)(\\(size\\/max\\/drops\\/flushes\\))", Pattern.DOTALL);
			mr = tmpPt.matcher(block);
			...
			tmpPt = Pattern.compile("(Output queue:)(.*)(\\(size\\/max total\\/threshold\\/drops\\))", Pattern.DOTALL);
			mr = tmpPt.matcher(block);
			if (mr.find()) {
				String match = mr.group(2);
				String[] items = match.trim().split("\\/");
