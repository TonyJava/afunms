...
public class BrocadeInterfaceSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		try {
			...
			if (hash == null)
				hash = new Hashtable();
			String[] oids = new String[] { // oid描述信息 参考文件
			// brocade_mib_ref_guide_261_53_0000521_02.pdf
			...
			try {
				valueArray = SnmpUtils.getTableData(host.getIpAddress(), host.getCommunity(), oids, host.getSnmpversion(), host.getSecuritylevel(), host.getSecurityName(), host
						.getV3_ap(), host.getAuthpassphrase(), host.getV3_privacy(), host.getPrivacyPassphrase(), 3, 1000 * 30);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (valueArray != null) {
