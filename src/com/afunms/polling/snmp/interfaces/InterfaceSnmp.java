	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		try {
			...
			try {
				...
				String[] lowOids = new String[] { "1.3.6.1.2.1.2.2.1.1", // ifIndex
				...
				String[] highOids = new String[] { "1.3.6.1.2.1.2.2.1.1", // ifIndex
						"1.3.6.1.2.1.2.2.1.2", // ifDescr
						"1.3.6.1.2.1.2.2.1.2", // ifName
						"1.3.6.1.2.1.2.2.1.3", // ifType
						"1.3.6.1.2.1.2.2.1.4", // ifMtu
				...
				try {
					if (host.getSnmpversion() == 0) {
						logger.info("Interface " + host.getIpAddress() + " SnmpV1");
						...
						logger.info("Interface " + host.getIpAddress() + " SnmpV2");
						...
						logger.info("Interface " + host.getIpAddress() + " SnmpV3");
						valueArray = SnmpUtils.getTableData(host.getIpAddress(), host.getCommunity(), highOids, host.getSnmpversion(), host.getSecuritylevel(), host
							...
							if ((j == 3) && interfaceValue != null) {// 端口类型
								...
								if (scale[j] == 0) {
									...
									if (interfaceValue != null) {
										interfacedata.setThevalue(new BigInteger(interfaceValue).divide(new BigInteger(String.valueOf(scale[j]))).toString());
									} else {
				...
				for (int i = 0; i < interfaceVector.size(); i++) {
					...
					if (interfaceCollectData.getEntity().equals("ifInOctets")) {
						...
						if (currentOctetsBigIntValue.compareTo(lastOctetsBigIntValue) < 0) {
							if (host.getSnmpversion() == 0) {
								// 32位计数
								currentOctetsBigIntValue = currentOctetsBigIntValue.add(new BigInteger("4294967296"));
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifInDiscards") || interfaceCollectData.getEntity().equalsIgnoreCase("ifInErrors")) {
						...
						for (int j = 0; j < interfaceVector.size(); j++) {
							...
							if (tempInterfaceData_.getEntity().equalsIgnoreCase("ifInNucastPkts")) {
								...
								if (longInterval.compareTo(new BigInteger("0")) != 0) {
									// 本次和上次非单播的包数差值
									inNucastPktsBetween = currentInNucastPktsValue - lastInNucastPktsValue;
								}
								// 更新内存
						...
						if (interfaceCollectData.getEntity().equalsIgnoreCase("ifInDiscards")) {
							...
							if ((inUcastPktsBetween + inNucastPktsBetween) == 0 || longInterval.compareTo(new BigInteger("0")) == 0) {
								indiscardsPerc = 0;
							} else {
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifOutDiscards") || interfaceCollectData.getEntity().equalsIgnoreCase("ifOutErrors")) {
						...
						if (interfaceCollectData.getEntity().equalsIgnoreCase("ifOutDiscards")) {
							...
							if ((outUcastPktsBetween + outNucastPktsBetween) == 0 || longInterval.compareTo(new BigInteger("0")) == 0) {
								indiscardserc = 0;
							} else {
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifInMulticastPkts")) {
						...
						if (currentInMulticastOctetsValue.compareTo(lastInMulticastOctetsValue) < 0) {
							if (host.getSnmpversion() == 0) {
								currentInMulticastOctetsValue = currentInMulticastOctetsValue.add(new BigInteger("4294967296"));// 32位计数
							} else if (host.getSnmpversion() == 1 || host.getSnmpversion() == 2) {
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifInBroadcastPkts")) {
						...
						if (currentInBroadcastOctetsValue.compareTo(lastInBroadcastOctetsValue) < 0) {
							if (host.getSnmpversion() == 0) {
								currentInBroadcastOctetsValue = currentInBroadcastOctetsValue.add(new BigInteger("4294967296"));// 32位计数
							} else if (host.getSnmpversion() == 1 || host.getSnmpversion() == 2) {
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifOutMulticastPkts")) {
						...
						if (currentOutMulticastOctetsValue.compareTo(lastOutMulticastOctetsValue) < 0) {
							if (host.getSnmpversion() == 0) {
								currentOutMulticastOctetsValue = currentOutMulticastOctetsValue.add(new BigInteger("4294967296"));// 32位计数
							} else if (host.getSnmpversion() == 1 || host.getSnmpversion() == 2) {
					...
					if (interfaceCollectData.getEntity().equalsIgnoreCase("ifOutBroadcastPkts")) {
						...
						if (currentOutBroadcastOctetsValue.compareTo(lastOutBroadcastOctetsValue) < 0) {
							if (host.getSnmpversion() == 0) {
								currentOutBroadcastOctetsValue = currentOutBroadcastOctetsValue.add(new BigInteger("4294967296"));// 32位计数
							} else if (host.getSnmpversion() == 1 || host.getSnmpversion() == 2) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (alarmIndicatorsNode.getName().equalsIgnoreCase("AllInBandwidthUtilHdx") || alarmIndicatorsNode.getName().equalsIgnoreCase("AllOutBandwidthUtilHdx")
						|| alarmIndicatorsNode.getName().equalsIgnoreCase("AllBandwidthUtilHdx")) {
					// 对总出入口流量值进行告警检测
					try {
						if (returnHash != null) {
