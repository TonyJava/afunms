...
public class LoadOracleRacFile {
	...
	public Hashtable collect_Data(NodeGatherIndicators gatherIndicatorsNode) {
		...
		// Host host =
		// (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(gatherIndicatorsNode.getNodeid()+""));
		...
		if (dbmonitorlists != null && dbmonitorlists.size() > 0) {
			for (int i = 0; i < dbmonitorlists.size(); i++) {
				...
				if (vo.getId() == Integer.parseInt(gatherIndicatorsNode.getNodeid())) {
					dbmonitorlist = vo;
					break;
				}
			}
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				AlarmIndicatorsNode alarmIndicatorsNode = (AlarmIndicatorsNode) list.get(i);
				if ("racstatus".equalsIgnoreCase(alarmIndicatorsNode.getName())) {
					checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, raccrstatus, "raccrstatus");
					checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, raclistenerstatus, "raclistenerstatus");
					checkEventUtil.checkEvent(nodeDTO, alarmIndicatorsNode, racstatusC, "racstatus");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
