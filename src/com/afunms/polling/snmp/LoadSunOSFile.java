...
public class LoadSunOSFile {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		if (cpuconfigContent.trim().length() > 30) {// 判断是否有采集到的cpu配置信息没有信息不做处理
			...
			// String[] processorsresult =
			// cpuconfigLineArr[1].trim().split("\\s++");
			//
			// //处理器个数
		...
		try {
			...
			// AlarmIndicatorsUtil();
			// List list =
			// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(host.getId()),
			// AlarmConstant.TYPE_HOST, "solaris");
		...
		for (int i = 0; i < cpuperfLineArr.length; i++) {
			...
			if (diskperf_tmpData != null && diskperf_tmpData.length == 5) {
				if (diskperf_tmpData[0].trim().equalsIgnoreCase("Average")) {
					...
					// List list =
					// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(host.getId()),
					// AlarmConstant.TYPE_HOST, "solaris","cpu");
		...
		for (int i = 0; i < memperfLineArr.length; i++) {
			...
			if (diskperf_tmpData != null && diskperf_tmpData.length >= 22) {
				...
				//					
				// if(memory_tempData[0].trim().equalsIgnoreCase("Memory
				// size")){
				...
				if (diskperf_tmpData[0] != null && !diskperf_tmpData[0].equalsIgnoreCase("r")) {
					// freeMemory
					// freePhysicalMemory =
					// Integer.parseInt(diskperf_tmpData[4])*4/1024/1024;
		...
		MemoryCollectEntity mem_data = null;
		...
		if (PhysicalMemCap > 0) {
			...
			// List list =
			// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(host.getId()),
			// AlarmConstant.TYPE_HOST, "solaris","physicalmemory");
			// for(int i = 0 ; i < list.size() ; i ++){
			// AlarmIndicatorsNode alarmIndicatorsnode =
			// (AlarmIndicatorsNode)list.get(i);
			// //对物理内存值进行告警检测
			...
			// //物理内存总大小变化告警检测
			// CheckEventUtil checkutil = new CheckEventUtil();
			// checkutil.hardwareInfo(host, "PhysicalMemory",
			// Float.toString(PhysicalMemCap)+"M");
			// }catch(Exception e){
			// e.printStackTrace();
			// }
		}
		...
		// ----------------解析swap内容--创建监控项---------------------
		String swap_Content = "";
		tmpPt = Pattern.compile("(cmdbegin:swapinfo)(.*)(cmdbegin:process)", Pattern.DOTALL);
		mr = tmpPt.matcher(fileContent.toString());
		if (mr.find()) {
			try {
		...
		try {
			...
			if (SwapMemCap > 0) {
				...
				// List list =
				// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(host.getId()),
				// AlarmConstant.TYPE_HOST, "solaris","swapmemory");
				// for(int i = 0 ; i < list.size() ; i ++){
				// AlarmIndicatorsNode alarmIndicatorsnode =
				// (AlarmIndicatorsNode)list.get(i);
				// //对交换内存值进行告警检测
		...
		for (int i = 1; i < process_LineArr.length; i++) {
			...
			if ((processtmpData != null) && (processtmpData.length == 7)) {
				...
				if (vbstring5.contains("-")) {
					...
					int hour = Integer.parseInt(vbstrings[0]) * 24 + Integer.parseInt(timestrs[0]);// 3*24
																									// + 5
					timestrs[0] = hour + "";
		...
		if (procsV != null && procsV.size() > 0) {
			...
			// try{
			// Hashtable iplostprocdata =
			// (Hashtable)ShareData.getLostprocdata(ipaddress);
		...
		for (int i = 0; i < netmacLineArr.length; i++) {
			...
			if (infflg = true && netmacLineArr[i].indexOf("ether") > 0) {// 解析出mac地址
		...
		for (int i = 0; i < netconfigLineArr.length; i++) {
			...
			if (netconfig_tmpData != null && (netconfig_tmpData.length == 8)) {
				...
				if (!"lo0".equals(netconfig_tmpData[0].trim())) {// 过滤网卡 lo0
		...
		for (int i = 1; i < netflowLineArr.length; i++) {
			...
			if (netconfig_tmpData != null && (netflow_tmpData.length == 10)) {
				...
				if (!"lo0".equals(netflow_tmpData[1].trim())) {// 过滤网卡 lo0
		...
		// if (iflist != null &&
		// iflist.size()>0)returnHash.put("iflist",iflist);
