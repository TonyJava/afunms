...
public class Host extends Node {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				for (int j = 0; j < hostServiceList.size(); j++) {
					...
					if (hostServiceVector != null) {
						for (int k = 0; k < hostServiceVector.size(); k++) {
							ServiceCollectEntity servicedata = (ServiceCollectEntity) hostServiceVector.get(k);
				...
				if (wrongList.size() > 0) {
					...
					// hostNode =
					// PollingEngine.getInstance().getNodeByID(hostNode.getId());
			...
			for (int i = 0; i < list.size(); i++) {
				try {
					...
					if (wrongList.size() > 0) {
						...
						// eventList.getSubentity(),ip , hostNode.getId() + "",
						// message , eventList.getLevel1() , 1 ,
						// jobForAS400Group.getName() ,
						// eventList.getBusinessid(),hostNode.getAlias() + "(" +
						// ip + ")" + " 作业组为：" + jobForAS400Group.getName());
						// }catch(Exception e){
						//							
						// }
					}
			...
			for (int i = 0; i < list.size(); i++) {
				...
				for (int j = 0; j < processGroupConfigurationList.size(); j++) {
					...
					for (int k = 0; k < proVector.size(); k++) {
						ProcessCollectEntity processdata = (ProcessCollectEntity) proVector.elementAt(k);
	...
	public String doPoll() {
		...
		// List monitorItemList = new
		// ArrayList();//alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(id+"",
		// "net", "");
		// try{
		// //获取被启用的所有被监视指标
		// if(category==4){
		// monitorItemList =
		// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(id+"", "host",
		// "");
		// }else{
		// monitorItemList =
		// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(id+"", "net",
		// "");
		// }
		// }catch(Exception e){
		// e.printStackTrace();
					...
					if (alarmIndicatorsNode != null) {
						...
						try {
							if (checkEventHash != null && checkEventHash.size() > 0) {
								// List<CheckEvent> elist =
								// checkeventdao.findCheckEvent(id+"",alarmIndicatorsNode.getType(),alarmIndicatorsNode.getSubtype(),alarmIndicatorsNode.getName());
								if (checkEventHash.get(id + ":" + alarmIndicatorsNode.getType() + ":" + alarmIndicatorsNode.getSubtype() + ":" + alarmIndicatorsNode.getName()) != null) {
									flag = (Integer) checkEventHash.get(id + ":" + alarmIndicatorsNode.getType() + ":" + alarmIndicatorsNode.getSubtype() + ":"
											+ alarmIndicatorsNode.getName());// elist.size();
								}
							}
							if (alarmIndicatorsNode.getType().equalsIgnoreCase("net")) {
								...
								if (alarmIndicatorsNode.getName().equals("cpu")) {
									...
									if (pingvalue == null || pingvalue.trim().length() == 0) {
										...
										if ("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())) {
											if (flag > 0) {
												...
												// Hashtable createeventdata =
												// ShareData.getCreateEventdata();
												// if
												// (createeventdata.containsKey("network:ping:"+ipAddress)){
												// //发送告警恢复信息,并写告警恢复提示
												// createeventdata.remove("network:ping:"+ipAddress);
												// try{
												// createSMS("network","ping",ipAddress,getId()+"","故障恢复:"+ipAddress+"
												// "+alarmIndicatorsNode.getAlarm_info()+"的故障已恢复",0,0,0);
												// }catch(Exception e){
												// e.printStackTrace();
												// }
												// }
									...
									if ("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())) {
										...
										// int flag = 0;
										// flag =
										// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"AllOutBandwidthUtilHdx");
										if (flag > 0) {
											// 有告警产生
											alarmMsg.append(alarmIndicatorsNode.getAlarm_info() + "当前值:" + inhdx + " 阀值:" + alarmIndicatorsNode.getLimenvalue0() + "<br>");
											// alarm = true;
									...
									if (interfaceVector != null && interfaceVector.size() > 0) {
											...
											if (tempv != null && tempv.size() > 0) {
												for (int k = 0; k < tempv.size(); k++) {
													...
													if (inpacks.getEntity().equalsIgnoreCase("ifInMulticastPkts")) {
														// int flag = 0;
														// flag =
														// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"ifInMulticastPkts");
														if (flag > 0) {
															// 有告警产生
											...
											if (tempv != null && tempv.size() > 0) {
												for (int k = 0; k < tempv.size(); k++) {
													...
													if (inpacks.getEntity().equalsIgnoreCase("ifInBroadcastPkts")) {
														// int flag = 0;
														// flag =
														// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"ifInBroadcastPkts");
														if (flag > 0) {
															// 有告警产生
															alarmMsg.append("第" + inpacks.getSubentity() + "端口" + alarmIndicatorsNode.getAlarm_info()
																	+ alarmIndicatorsNode.getLimenvalue0() + "<br>");
											...
											if (tempv != null && tempv.size() > 0) {
												for (int k = 0; k < tempv.size(); k++) {
													...
													if (outpacks.getEntity().equalsIgnoreCase("ifOutMulticastPkts")) {
														// int flag = 0;
														// flag =
														// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"ifOutMulticastPkts");
														if (flag > 0) {
															// 有告警产生
											...
											if (tempv != null && tempv.size() > 0) {
												for (int k = 0; k < tempv.size(); k++) {
													...
													if (outpacks.getEntity().equalsIgnoreCase("ifOutBroadcastPkts")) {
														// int flag = 0;
														// flag =
														// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"ifOutBroadcastPkts");
														if (flag > 0) {
															// 有告警产生
															alarmMsg.append("第" + outpacks.getSubentity() + "端口" + alarmIndicatorsNode.getAlarm_info()
																	+ alarmIndicatorsNode.getLimenvalue0() + "<br>");
										...
										if (tempv != null && tempv.size() > 0) {
											for (int k = 0; k < tempv.size(); k++) {
												...
												// int flag = 0;
												// flag =
												// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"discardsperc");
												if (flag > 0) {
													// 有告警产生
										...
										if (tempv != null && tempv.size() > 0) {
											for (int k = 0; k < tempv.size(); k++) {
												...
												// int flag = 0;
												// flag =
												// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"errorsperc");
												if (flag > 0) {
													// 有告警产生
								...
								if (alarmIndicatorsNode.getName().equals("cpu")) {
									try {
										...
										if ("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())) {
											// int flag = 0;
											// flag =
											// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"cpu");
											if (flag > 0) {
												// 有告警产生
												alarmMsg.append(alarmIndicatorsNode.getAlarm_info() + "<br>");
												// alarm = true;
									...
									try {
										...
										if ("1".equalsIgnoreCase(alarmIndicatorsNode.getEnabled())) {
											if (pingvalue == null || pingvalue.trim().length() == 0) {
												...
												// int flag = 0;
												// flag =
												// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+"ping");
												...
												for (int si = 0; si < diskVector.size(); si++) {
													...
													if (diskdata.getEntity().equalsIgnoreCase("Utilization")) {
														...
														if (getOstype() == 5 || getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")) {
															// System.out.println(ipAddress+"===flag==="+id+":"+alarmIndicatorsNode.getType()+":"+"diskperc:diskperc:"+diskdata.getSubentity().substring(0,
															// 3));
															// elist =
															// checkeventdao.findCheckEvent(id+"",alarmIndicatorsNode.getType(),alarmIndicatorsNode.getSubtype(),alarmIndicatorsNode.getName(),diskdata.getSubentity().substring(0,
															// 3));
															if (checkEventHash != null && checkEventHash.size() > 0) {
																// List<CheckEvent>
																// elist =
																// checkeventdao.findCheckEvent(id+"",alarmIndicatorsNode.getType(),alarmIndicatorsNode.getSubtype(),alarmIndicatorsNode.getName());
																if (checkEventHash.get(id + ":" + alarmIndicatorsNode.getType() + ":" + alarmIndicatorsNode.getSubtype() + ":"
																		+ alarmIndicatorsNode.getName() + ":" + diskdata.getSubentity().substring(0, 3)) != null) {
																	flag = (Integer) checkEventHash.get(id + ":" + alarmIndicatorsNode.getType() + ":"
															...
															// elist =
															// checkeventdao.findCheckEvent(id+"",alarmIndicatorsNode.getType(),alarmIndicatorsNode.getSubtype(),alarmIndicatorsNode.getName(),diskdata.getSubentity());
															if (checkEventHash != null && checkEventHash.size() > 0) {
																// List<CheckEvent>
																// elist =
																// checkeventdao.findCheckEvent(id+"",alarmIndicatorsNode.getType(),alarmIndicatorsNode.getSubtype(),alarmIndicatorsNode.getName());
														...
														// DiskconfigDao();
														// Diskconfig vo =
														// diskconfigDao.loadByIpNameBak(ipAddress,
														// diskdata.getSubentity(),
														// "利用率阈值");
														// if(getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")){
														// if(vo!=null&&vo.getIs_show()==1)msg.append(alarmIndicatorsNode.getDescr()
														// + ":"
														// +diskdata.getSubentity().substring(0,
														// 3)+" "+
														// CEIString.round(new
														// Double(diskdata.getThevalue()).doubleValue(),2)
														// + "
													...
													if (diskdata.getEntity().equalsIgnoreCase("UtilizationInc")) {
														// //磁盘增长率 yangjun
														// int flag =
														// checkeventdao.findByName(id+":"+nm.getType()+":"+"diskinc:"+diskdata.getSubentity());
														// //int flag =
														// checkeventutil.checkEvent("disk",new
														// Double(diskdata.getThevalue()),nm,diskdata.getSubentity(),"增长率阈值");
														// //
														...
														// = null;
														// if(getOstype() ==4 ||
														// getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")){
														// diskconfig =
														// (Diskconfig)alldiskalarmdata.get(ipAddress+":"+diskdata.getSubentity().substring(0,
														// 3)+":"+"增长率阈值");
														// }else{
														// diskconfig =
														// (Diskconfig)alldiskalarmdata.get(ipAddress+":"+diskdata.getSubentity()+":"+"增长率阈值");
														// }
														//				            	        				
														// int limevalue = 0;
														// if(flag == 1){
														// limevalue =
														// diskconfig.getLimenvalue();
														// }else if(flag == 2){
														// limevalue =
														// diskconfig.getLimenvalue1();
														// }else
														// limevalue =
														// diskconfig.getLimenvalue2();
														//				            	        				
														// if(getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")){
														// //有告警产生
														// alarmMsg.append("硬盘增长率超过阈值
														// "+diskdata.getSubentity().substring(0,
														// 3)+"
														// 阀值:"+limevalue+"<br>");
														// alarm = true;
											...
											if (memoryVector != null && memoryVector.size() > 0) {
												for (int si = 0; si < memoryVector.size(); si++) {
													...
													if (memorydata.getEntity().equalsIgnoreCase("Utilization") && memorydata.getSubentity().equalsIgnoreCase("SwapMemory")) {
														...
														// int flag = 0;
														// flag =
														// checkeventdao.findByName(id+":"+alarmIndicatorsNode.getType()+":"+alarmIndicatorsNode.getName());
														if (flag > 0) {
															// 有告警产生
															alarmMsg.append(alarmIndicatorsNode.getAlarm_info() + "<br>");
															// alarm = true;
		...
		// hostServiceEventList = createHostServiceGroupEventList(ipAddress ,
		// winserviceVector);
		// }catch(Exception e){
		// e.printStackTrace();
		// }
		...
		// try{
		// if(jobForAS400List != null && jobForAS400List.size()>0){
		// jobForAS400EventList = createJobForAS400GroupEventList(ipAddress ,
		// jobForAS400List);
		// }
		// }catch(Exception e){
		// e.printStackTrace();
	...
	public synchronized void refreshNodeState() {
		AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
		List monitorItemList = new ArrayList();// alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(id+"",
		// "net", "");
		try {
			// 获取被启用的所有被监视指标
			if (category == 4) {
		...
		Hashtable checkEventHash = ShareData.getCheckEventHash();
		if (checkEventHash == null) {
			checkEventHash = new Hashtable();
		}
		try {
					...
					if (nm.getName().equals("cpu")) {
						...
						if ("1".equalsIgnoreCase(nm.getEnabled())) {
							...
							if (flag > 0) {
								...
								if (this.alarmlevel < flag) {
									this.alarmlevel = flag;
								}
							}
						}
								...
								if (tempv != null && tempv.size() > 0) {
									for (int k = 0; k < tempv.size(); k++) {
										...
										if (inpacks.getEntity().equalsIgnoreCase("ifInMulticastPkts")) {
											...
											// flag =
											// checkeventdao.findByName(id+":"+nm.getType()+":"+"ifInMulticastPkts");
											if (flag > 0) {
												// 有告警产生
												alarm = true;
												if (status < flag) {
								...
								if (tempv != null && tempv.size() > 0) {
									for (int k = 0; k < tempv.size(); k++) {
										...
										if (inpacks.getEntity().equalsIgnoreCase("ifInBroadcastPkts")) {
											...
											// flag =
											// checkeventdao.findByName(id+":"+nm.getType()+":"+"ifInBroadcastPkts");
											if (flag > 0) {
												// 有告警产生
												alarm = true;
												if (status < flag) {
							...
							if ("1".equalsIgnoreCase(nm.getEnabled())) {
								...
								if (tempv != null && tempv.size() > 0) {
									for (int k = 0; k < tempv.size(); k++) {
										...
										if (outpacks.getEntity().equalsIgnoreCase("ifOutMulticastPkts")) {
											// int flag =
											// checkeventutil.checkEvent("ifOutMulticastPkts",Double.parseDouble(outpacks.getThevalue()),nm,"","");
											int flag = 0;
											...
											// flag =
											// checkeventdao.findByName(id+":"+nm.getType()+":"+"ifOutMulticastPkts");
											if (flag > 0) {
												// 有告警产生
												alarm = true;
												if (status < flag) {
							...
							if ("1".equalsIgnoreCase(nm.getEnabled())) {
								...
								if (tempv != null && tempv.size() > 0) {
									for (int k = 0; k < tempv.size(); k++) {
										...
										if (outpacks.getEntity().equalsIgnoreCase("ifOutBroadcastPkts")) {
											// int flag =
											// checkeventutil.checkEvent("ifOutBroadcastPkts",Double.parseDouble(outpacks.getThevalue()),nm,"","");
											int flag = 0;
					...
					if (nm.getName().equals("cpu")) {
						...
						if ("1".equalsIgnoreCase(nm.getEnabled())) {
							if (pingvalue == null || pingvalue.trim().length() == 0) {
								...
								// int flag =
								// checkeventdao.findByName(id+":"+nm.getType()+":"+"ping");
								if (flag > 0) {
									// 有告警产生
									alarm = true;
									if (status < flag) {
								...
								for (int si = 0; si < diskVector.size(); si++) {
									...
									if (diskdata.getEntity().equalsIgnoreCase("Utilization")) {
										...
										if (getOstype() == 4 || getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")) {
											...
											// flag =
											// checkeventdao.findByName(id+":"+nm.getType()+":"+"diskperc:diskperc:"+diskdata.getSubentity().substring(0,
											// 3));
										} else {
											if (checkEventHash.containsKey(id + ":" + nm.getType() + ":" + "ping")) {
												if (checkEventHash.containsKey(id + ":" + nm.getType() + ":" + "ping")) {
												...
												// flag =
												// (Integer)checkEventHash.get(id+":"+nm.getType()+":"+"ping");
											}
											// flag =
											// checkeventdao.findByName(id+":"+nm.getType()+":"+"diskperc:diskperc:"+diskdata.getSubentity());
										}
										if (flag > 0) {
											if (getSysOid().startsWith("1.3.6.1.4.1.311.1.1.3")) {
												// 有告警产生
							...
							if (memoryVector != null && memoryVector.size() > 0) {
								for (int si = 0; si < memoryVector.size(); si++) {
									...
									if (memorydata.getEntity().equalsIgnoreCase("Utilization") && memorydata.getSubentity().equalsIgnoreCase("PhysicalMemory")) {
										...
										// flag =
										// checkeventdao.findByName(id+":"+nm.getType()+":"+nm.getName());
										if (flag > 0) {
											// 有告警产生
											alarm = true;
											if (status < flag) {
	...
	// ipaddress,String objid,String content,int flag,int checkday,int recover){
	// //建立短信
	// //从内存里获得当前这个IP的PING的值
	...
	// Calendar formerdate
	// =(Calendar)sendeddata.get(subtype+":"+subentity+":"+ipaddress);
	...
	// createeventdata.put(subtype+":"+subentity+":"+ipaddress, date);
	// }
	// //createEvent("poll",sysLocation,getBid(),nm.getAlarmInfo()+"当前值:"+CEIString.round(new
	// Double(memorydata.getThevalue()).doubleValue(),2)+"
	// 阀值:"+nm.getLimenvalue0(),flag,"host","memory");
	// }
	// } else {
	// //若不在，则建立短信，并且添加到发送列表里
	// Smscontent smscontent = new Smscontent();
