					...
					if (null != resultTemp && !resultTemp.equals("user or password error")) {
						...
						for (int j = 0; j < st.length; j++) {
							...
							if (!st[j].startsWith("  ---- More ----")) {
								if (st[j].indexOf("% Unrecognized command found at '^' position.") > -1 || st[j].indexOf("% Ambiguous command found at '^' position.") > -1
										|| st[j].indexOf("% Incomplete command found at '^' position.") > -1 || st[j].indexOf("Error:") > -1) {
									cmdResult.setIp(ip);
	...
	public boolean login() {
		...
		if (null != this.password && this.password.trim().length() > 0) {
			...
			if (connetflg) {// 成功
				// /////////////////设置不同的账户类型，会有不同的字符提示，这个地方会影响到代码执行速度/////////////
				String[] patterns = { USER_PROMPT, SYS_PROMPT, this.Loginuser, Loginpassword, "Password:" };
				String[] pass_patterns = { "in unit1 login", USER_PROMPT, SYS_PROMPT, SU_PROMPT, Loginpassword, "% Login failed!" };
				...
				if (aa.endsWith("% Login failed!") || aa.endsWith("user or password error")) {
					loginflg = false;
				} else if (aa.endsWith(USER_PROMPT) || aa.endsWith(SU_PROMPT) || aa.endsWith(SU_PROMPT)) {
					loginflg = true;
				} else if (aa.endsWith("Password:")) {
					loginflg = false;
				} else if (aa.toString().endsWith("in unit1 login")) {
					write("\r\n");
		...
		try {
			...
			while (n >= 0) {
				...
				if (n > 0) {
					...
					if (ret.toString().toLowerCase().indexOf("---- more ----") > 0) {
						out.write(32);
						out.flush();
					}
			...
			while (n >= 0) {
				...
				if (n > 0) {
					...
					if (ret.toString().endsWith(pattern1)) {
						...
						break;
					} else if (ret.toString().endsWith(pattern2)) {
