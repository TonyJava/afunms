...
public class AIXInfoParser {
	...
	public static Hashtable getTelnetMonitorDetail(Wrapper telnet) {
		...
		Host host = (Host) PollingEngine.getInstance().getNodeByIP(telnet.getHost());
		if (host == null) {
			return null;
		}
		...
		if (gatherHash.containsKey("swapmemory")) {
			...
			try {
				...
				if (SwapMemCap > 0) {
					...
					// System.out.println("交换分区使用率 "+ Math.round(usedSwapMemory
					// * 100 / SwapMemCap) + "");
					memoryVector.addElement(memorydata);
		...
		if (gatherHash.containsKey("netperf")) {
			...
			try {
				...
				// netallperfContent = netallperfContent +telnet.send("entstat
				// -d en0 |egrep 'Bytes'");
			...
			if (netperf != null && netperf.size() > 0) {
				...
				for (int k = 0; k < netperf.size(); k++) {
					...
					/**
					 * 由于aix 同一个命令采集的数据出现不同的数据格式需要在做下面的判断 第一行数据有下面的模式 模式一
					 * ETHERNET STATISTICS (en1 netflg="neten" 模式二： Hardware
					 * Address: netflg="netmac" 使用一个 netflg 判断是什么模式
					 * 
					 */
					int beginindex = 0;
					...
					try {
						...
						for (int i = 0; i < netLineArr.length; i++) {
							...
							// if(tempStr.indexOf("Hardware Address:")>=0){
							// MAC =
							// tempStr.trim().substring(tempStr.trim().indexOf("Hardware
							// Address:")+17);
							// }else if(tempStr.indexOf("Media Speed
							// Running:")>=0){
							// mideaspeed =
							// tempStr.trim().substring(tempStr.trim().indexOf("Media
							// Speed Running:")+20);
							// }else if(tempStr.indexOf("Link Status :")>=0){
							// status =
							// tempStr.trim().substring(tempStr.trim().indexOf("Link
							// Status :"));
							// LinkStatus=status;
						...
						// }
						// mideaspeed =
						// netLineArr[45].trim().substring(netLineArr[45].trim().indexOf("Media
						// Speed Running:"));
						// status =
						// netLineArr[43].trim().substring(netLineArr[43].trim().indexOf("Link
						// Status :"));
		...
		if (gatherHash.containsKey("process")) {
			...
			try {
				...
				// .send("ps gv | head -n 1; ps gv | egrep -v \"RSS\" | sort +6b
				// -7 -n -r");
			} catch (IOException e1) {
				e1.printStackTrace();
			}
	...
	public void createSMS(Procs procs) {
		...
		// lastprocs =
		// (Procs)sendeddata.get(procs.getIpaddress()+":"+procs.getProcname());
		/*
