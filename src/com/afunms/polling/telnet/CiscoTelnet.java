				...
				if (null != result && !result.equals("user or password error")) {
					...
					for (int j = 0; j < st.length; j++) {
						if (!st[j].toLowerCase().contains("--more--")) {
							if (st[j].indexOf("% Ambiguous command:") > -1 || st[j].indexOf("% Unknown command or computer name, or unable to find computer address") > -1
									|| st[j].indexOf("% Invalid input detected at '^' marker.") > -1 || st[j].indexOf("% Unrecognized host or address") > -1) {
								cmdResult.setIp(ip);
		...
		try {
			...
			try {
				...
				if ("tcpconnect-noresponder".equalsIgnoreCase(slaconfig.getSlatype())) {
					...
					command[2] = "type tcpConnect dest-ipaddr " + (String) slaParamHash.get("tcpconnectnoresponder_destip") + " dest-port "
							+ (String) slaParamHash.get("tcpconnectnoresponder_destport") + " control disable";
					...
					command[0] = "config";
					command[1] = "rtr " + nextentry;
					command[2] = "type udpEcho dest-ipaddr " + (String) slaParamHash.get("udp_destip") + " dest-port " + (String) slaParamHash.get("udp_destport")
							+ " control disable";
					command[3] = "rtr sch " + nextentry + " life forever start-time now";
					command[4] = "exit";
					command[5] = "write";
					...
					command[0] = "config";
					command[1] = "rtr " + nextentry;
					command[2] = "type tcpConnect dest-ipaddr " + (String) slaParamHash.get("tcpconnectwithresponder_destip") + " dest-port "
							+ (String) slaParamHash.get("tcpconnectwithresponder_destport") + " control disable";
		...
		try {
			...
			if (null != this.password && this.password.trim().length() > 0) {
				...
				if (connetflg) {// 成功
					...
					String[] patterns = { USER_PROMPT, SU_PROMPT, this.Loginuser, Loginpassword, "Password:" };
		...
		try {
			...
			if (!isContainInvalidateWords(temp)) {
				...
				if (!isContainInvalidateWords(temp)) {
					...
					cmd = "line vty 0 5";
					// temp = sendCommand("username " + newUser + " password 0 "
					// + newPasswd);
