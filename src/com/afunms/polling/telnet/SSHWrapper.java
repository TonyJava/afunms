				...
				for (int i = 0; i < results.length - 1; i++) {
					...
					if (needAppend) {
						...
						if (line.equals(identity) || line.equals("$" + identity) || line.equals("#" + identity)) {
							...
							if (results[i + 1].indexOf("Hardware Address:") >= 0 || results[i + 1].indexOf("load average:") >= 0 || results[i + 1].indexOf("$hdisk") >= 0
									|| results[i + 1].indexOf("BEIST") >= 0 || command.equalsIgnoreCase("lsuser ALL") || results[i + 1].indexOf("$AIX") >= 0
