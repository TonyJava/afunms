...
public class SolarisInfoParser {
	...
	public static Hashtable getTelnetMonitorDetail(Wrapper telnet) {
		...
		// if (diskVector != null &&
		// diskVector.size()>0)returnHash.put("disk",diskVector);
		// if (cpuVector != null &&
		// cpuVector.size()>0)returnHash.put("cpu",cpuVector);
		// if (memoryVector != null &&
		// memoryVector.size()>0)returnHash.put("memory",memoryVector);
		// if (userVector != null &&
		// userVector.size()>0)returnHash.put("user",userVector);
		// if (processVector != null &&
		// processVector.size()>0)returnHash.put("process",processVector);
		// if (systemVector != null &&
		// systemVector.size()>0)returnHash.put("system",systemVector);
		// if (nodeconfig != null)returnHash.put("nodeconfig",nodeconfig);
		// //if (iflist != null &&
		// iflist.size()>0)returnHash.put("iflist",iflist);
		// if (utilhdxVector != null &&
		// utilhdxVector.size()>0)returnHash.put("utilhdx",utilhdxVector);
		// if (interfaceVector != null &&
		// interfaceVector.size()>0)returnHash.put("interface",interfaceVector);
		// if (alldiskperf != null &&
		// alldiskperf.size()>0)returnHash.put("alldiskperf",alldiskperf);
		// if (cpuconfiglist != null &&
		// cpuconfiglist.size()>0)returnHash.put("cpuconfiglist",cpuconfiglist);
		// if (cpuperflist != null &&
		// cpuperflist.size()>0)returnHash.put("cpuperflist",cpuperflist);
		return returnHash;
