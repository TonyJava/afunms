...
public class M30PasswdBackupTelnetConfigTask extends MonitorTask {
	...
	private void subscribe() {
		...
		String sql = "SELECT * FROM sys_pwdbackup_telnetconfig s WHERE status = '是' and s.BACKUP_DATE > 10000 AND s.BACKUP_DATE <= " + time;
		...
		if (ssconfAL != null) {
			...
			try {
				for (int i = 0; i < ssconfAL.size(); i++) {
					...
					/**
					 * 判断时间间隔 获取小时，范围0-23。 获取一月中的第几天取值范围1-31。 取一个星期的第几天。
					 * 周末为一个星期的第一天； 获取月份，范围1-12。
					 */
