 ...
 *
 */
public class D1Task extends MonitorTask {
...
//        		try{
//            		allsystemdataHash = xmlutil.parserXml(ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_systemgroup.xml");
        ...
        public void run() {
        	try {                	
            	...
            	if(node.getCollecttype() == SystemConstant.COLLECTTYPE_DATAINTERFACE){
            		//接口数据方式进行采集,当前主要采集路由信息
            		//进行文件解析
            		Vector routerVec=null;
            		ParserXmlUtil xmlutil = new ParserXmlUtil();
            		try{
            			SysLogger.info("开始解析"+ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_router"+node.getIpAddress()+".xml");
            			returnHash = xmlutil.parserIpRouterXml(ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_router"+node.getIpAddress()+".xml");
            			try
            			{
            				File delFile = new File(ResourceCenter.getInstance().getSysPath() + "linuxserver/network_router"+node.getIpAddress()+".xml");
