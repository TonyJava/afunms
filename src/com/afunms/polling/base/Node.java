...
public class Node {
	...
	public String getShowMessage() {
		// Hashtable ipAllData =
		// (Hashtable)ShareData.getSharedata().get(ipAddress);
		...
		/*
		 * if(status==0||status>1) { msg.append("<font color='red'>");
		 * if(status==0) msg.append("不被管理</font>"); else if(status==2)
		 * msg.append("设备忙</font>"); else msg.append("Ping不通</font>"); return
		 * msg.toString(); }
		 * 
		 * alarm = false; StringBuffer alarmMsg = new StringBuffer(100);
		 * //SysLogger.info("############################################");
		 * for(int i=0;i<moidList.size();i++) {
		 * 
		 * NodeMonitor nm = (NodeMonitor)moidList.get(i);
		 * if(nm.getNodetype().equalsIgnoreCase("net")){ //网络设备
		 * if(nm.getCategory().equals("cpu")){ //CPU利用率
		 * SysLogger.info("limenvalue0:"+nm.getLimenvalue0()+"
		 * cpuvalue:"+cpuvalue); if(nm.getLimenvalue0()<cpuvalue){ //产生告警
		 * alarmMsg.append(nm.getAlarmInfo()); alarm = true; }
		 * msg.append(nm.getDescr() + ":" + cpuvalue + nm.getUnit() + "<br>");
		 * }else if(nm.getCategory().equals("ping")){ //网络连通性
		 * msg.append(nm.getDescr() + ":" + pingvalue + nm.getUnit() + "<br>");
		 * }else if(nm.getCategory().equals("interface")){ //接口信息
		 * if(nm.getSubentity().equals("AllInBandwidthUtilHdx")){ //入口流速
		 * if(inhdx != null )msg.append(nm.getDescr() + ":" + inhdx +
		 * nm.getUnit() + "<br>"); }else
		 * if(nm.getSubentity().equals("AllOutBandwidthUtilHdx")){ //出口流速
		 * if(outhdx != null )msg.append(nm.getDescr() + ":" + outhdx +
		 * nm.getUnit() + "<br>"); }
		 * 
		 * //msg.append(nm.getDescr() + ":" + pingvalue + nm.getUnit() + "<br>"); } }
		 *  }
