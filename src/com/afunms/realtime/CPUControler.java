				...
				for (int i = 0; i < gatherlist.size(); i++) {
					...
					if ("cpu".equalsIgnoreCase(nodeGatherIndicators.getName())) {
						...
						try {
							...
							hostdataManager.createHostItemData(hostNode.getIpAddress(), returnHash, "host", "windows", "cpu");
							Vector cpuUseRatio = (Vector) returnHash.get("cpu");
							if (cpuUseRatio != null && cpuUseRatio.size() > 0) {
								CpuCollectEntity cpudata = (CpuCollectEntity) cpuUseRatio.get(0);
				...
				for (int i = 0; i < gatherlist.size(); i++) {
					...
					if ("cpu".equalsIgnoreCase(nodeGatherIndicators.getName())) {
						...
						try {
							...
							HostCollectDataManager hostdataManager = new HostCollectDataManager();
							hostdataManager.createHostItemData(hostNode.getIpAddress(), returnHash, "host", "h3c", "cpu");
							Vector cpuUseRatio = (Vector) returnHash.get("cpu");
							if (cpuUseRatio != null && cpuUseRatio.size() > 0) {
								CpuCollectEntity cpudata = (CpuCollectEntity) cpuUseRatio.get(0);
								dm.setData(Double.valueOf(cpudata.getThevalue()));
