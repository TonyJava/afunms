...
public class SubscribeTask extends TimerTask {
	...
	private void subscribe() {
		...
		if (ssconfAL != null) {
			log.info("订阅定时器执行时间：" + dt.getMyDateTime(DateTime.Datetime_Format_2) + "---" + flag);
			try {
				for (int i = 0; i < ssconfAL.size(); i++) {
					...
					if ("0".equals(report_sendfrequency)) {
						...
						if (report_time_week.contains("/" + (dt.getDay() - 1) + "/")
								&& report_time_hou.contains("/" + (dt.getHours() < 10 ? "0" + dt.getHours() : dt.getHours()) + "/")) {
							istrue = true;
						}
						...
						if (report_time_month.contains("/" + (dt.getMonth() < 10 ? "0" + dt.getMonth() : dt.getMonth()) + "/")
								&& report_time_day.contains("/" + (dt.getDate() < 10 ? "0" + dt.getDate() : dt.getDate()) + "/")
								&& report_time_hou.contains("/" + (dt.getHours() < 10 ? "0" + dt.getHours() : dt.getHours()) + "/")) {
							istrue = true;
						}
					...
					if (istrue) {
						...
						String filePath = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + "report" + "_" + System.currentTimeMillis() + "." + exportType;
	...
	private void updateSubscribeResources(String subscribe_id) {
		...
		String sql = null;
		try {
			sql = "update sys_subscribe_resources set report_senddate = '" + dt.getMyDateTime(DateTime.Datetime_Format_14) + "' where	subscribe_id = '" + subscribe_id + "' ";
			cd.saveOrUpdate(sql);
		} catch (Exception e) {
	...
	private void updateDkdbSubscribeResources(String subscribe_id) {
		...
		String sql = null;
		try {
			sql = "update sys_dkdb_ssresources set report_senddate = '" + dt.getMyDateTime(DateTime.Datetime_Format_14) + "' where	subscribe_id = '" + subscribe_id + "' ";
			cd.saveOrUpdate(sql);
		} catch (Exception e) {
			log.error("更新订阅发送时间失败  sql: " + sql, e);
		}
	...
	private void insertSubscribeHistory(Map<String, String> ssidAL, String filepath) {
		...
		try {
			...
			sql = "insert into sys_subscribe_history (subscribe_id,file_name,report_type,create_date,success_post,repeat_post) values (" + subscribe_id + ",'" + filepath + "','"
					+ report_type + "','" + dt.getMyDateTime(DateTime.Datetime_Format_1) + "','0','0'	)";
			cd.saveOrUpdate(sql);
		} catch (Exception e) {
	...
	private void updateSubscribeHistory(Map<String, String> ssidAL, String filepath, boolean flag) {
		...
		try {
			...
			filepath = filepath.replace("\\", "\\\\");
			if (flag) {
				sql = "update sys_subscribe_history set success_post = success_post+1 where subscribe_id = '" + subscribe_id + "' and file_name = '" + filepath + "'";
			} else {
				sql = "update sys_subscribe_history set repeat_post = repeat_post+1 where subscribe_id = '" + subscribe_id + "' and file_name = '" + filepath + "'";
			}
			cd.saveOrUpdate(sql);
		} catch (Exception e) {
