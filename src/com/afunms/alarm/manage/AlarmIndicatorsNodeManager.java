							...
							try {
								for (int k = 0; k < addindilist.size(); k++) {
									...
									if (nodeindihash.containsKey(alarmIndicators.getName() + ":" + alarmIndicators.getType() + ":" + alarmIndicators.getSubtype())) {
										AlarmIndicatorsNode alarmIndicatorsNode_update = (AlarmIndicatorsNode) nodeindihash.get(alarmIndicators.getName() + ":"
												+ alarmIndicators.getType() + ":" + alarmIndicators.getSubtype());
		...
		if (request.getParameter("type").equals("storage") && request.getParameter("subtype").equals("hds")) {
			if (request.getParameter("subtype").equals("emc")) {
				if (category != null) {
					...
					} else if (category.equalsIgnoreCase("environment") || category.equalsIgnoreCase("envstore") || category.equalsIgnoreCase("envpower")
							|| category.equalsIgnoreCase("bakpower")) {
						...
						if (alarmIndicators.getName().equalsIgnoreCase("ping")) {
							...
							if (l != null && l.size() > 0) {
								for (int j = 0; j < l.size(); j++) {
									AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
									AlarmIndicatorsNode alarmIndicatorsNode = alarmIndicatorsUtil.VMcreateAlarmIndicatorsNodeByAlarmIndicators(nodeid, alarmIndicators, l.get(j)
											.toString());
									list2.add(alarmIndicatorsNode);
								}
							}
						...
						if (alarmIndicators.getName().equalsIgnoreCase("ping")) {
							...
							if (name != null && name.size() > 0) {
								for (int j = 0; j < name.size(); j++) {
									AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
									AlarmIndicatorsNode alarmIndicatorsNode = alarmIndicatorsUtil.VMcreateAlarmIndicatorsNodeByAlarmIndicators(nodeid, alarmIndicators, name.get(j)
											.toString());
									list2.add(alarmIndicatorsNode);
								}
							} else {
