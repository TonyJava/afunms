...
public class IndicatorsTopoRelationDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sql.append("insert into nms_indicators_topo_relation(indicators_id,sindex,topo_id" + ") values('");
	...
	public boolean save(List<IndicatorsTopoRelation> list) {
		...
		try {
			for (int i = 0; i < list.size(); i++) {
				...
				StringBuffer sql = new StringBuffer();
				sql.append("insert into nms_indicators_topo_relation(indicators_id,sindex,topo_id,nodeid" + ") values('");
				sql.append(relation.getIndicatorsId());
				sql.append("','");
				sql.append(relation.getSIndex());
				sql.append("','");
