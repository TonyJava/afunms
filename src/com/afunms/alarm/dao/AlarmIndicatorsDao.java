...
public class AlarmIndicatorsDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sql.append("insert into nms_alarm_indicators(name, type, subtype, datatype, moid, threshold, " + "threshold_unit, compare, compare_type, alarm_times, alarm_info, alarm_level," + " enabled, poll_interval, interval_unit, subentity, limenvalue0, limenvalue1, " + "limenvalue2, time0, time1, time2, sms0, sms1, sms2, category, descr, unit) values('");
	...
	public List getByType(String type) {
		...
		try {
			rs = conn.executeQuery("select distinct name,descr from nms_alarm_indicators where type='" + type + "' group by name");
			if (rs == null) {
				return null;
			}
			while (rs.next()) {
	...
	public List updatelist(int id) {
		...
		String subtype = "";
		try {
			rs = conn.executeQuery("select * from nms_alarm_indicators where id='" + id + "';");
			while (rs.next()) {
