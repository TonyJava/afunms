		...
		if ("0".equals(runmodel)) {
			try {
				if (type.equals("Memory")) {
					memhash = hostlastmanager.getMemory_share(ip, type, null, null);
				}
				if (type.equals("Disk")) {
					diskhash = hostlastmanager.getDisk_share(ip, type, null, null);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if (type.equals("Disk")) {
			if (diskhash != null && diskhash.size() > 0) {
				...
				for (int j = 0; j < diskhash.size(); j++) {
					Hashtable diskhash1 = (Hashtable) (diskhash.get(new Integer(j)));
