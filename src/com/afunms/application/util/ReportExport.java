...
public class ReportExport {
	 ...
	 *            导出文件类型
	 */
	public void exportReport(String ids, String type, String filePath, String startTime, String toTime, String exportType) {
	 ...
	 * 机房报表
	 */
	public void exportRoomReport(String type, String filePath, int cabinetid, String startTime, String toTime, String exportType) {
		// 存放表格信息
	 ...
	 *            导出文件类型
	 */
	public void portcompareExport(String ids, String type, String filePath, String startTime, String toTime, String exportType) {
		HashMap<?, ArrayList<?>> hm = null;
		// 存放表格信息
		ArrayList<ArrayList<String[]>> tableList = null;
		// 存放图片路径
	...
	public void exportCfgReport(String allipStr, String filePath, String startTime, String toTime, String exportType) {
		// 存放表格信息
		ArrayList<ArrayList<String[]>> tableList = null;
		// 存放图片路径
		ArrayList<String> chartList = new ArrayList<String>();
	 ...
	 * Acl匹配信息
	 */
	public void exportAclReport(String allipStr, String filePath, String startTime, String toTime, String exportType) {
		// 存放表格信息
		ArrayList<ArrayList<String[]>> tableList = null;
		// 存放图片路径
		ArrayList<String> chartList = new ArrayList<String>();
	 ...
	 * @param time
	 */
	private void export(ExportInterface export, ArrayList<ArrayList<String[]>> tableList, ArrayList<String> chartList, String title, String time) {
		if (tableList != null) {
	...
	private void export(ExportInterface export, ArrayList<String[]> tableList, String title, String time) {
		if (tableList != null) {
			if (tableList.get(0) != null) {
				try {
					export.insertTitle(title, tableList.get(0).length, time);
				} catch (Exception e) {
					e.printStackTrace();
				}
			} else {
	...
	private void exportCfg(ExportInterface export, ArrayList<ArrayList<String[]>> tableList, ArrayList<String> chartList, String title, String time) {
		if (tableList != null) {
			if (tableList.get(0) != null) {
				try {
					export.insertTitle(title, tableList.get(0).get(0).length, time);
	 ...
	 * @param time
	 */
	private void exportAcl(ExportInterface export, ArrayList<ArrayList<String[]>> tableList, ArrayList<String> chartList, String title, String time) {
		if (tableList != null) {
			if (tableList.get(0) != null) {
				try {
					export.insertTitle(title, tableList.get(0).get(0).length, time);
	...
	public ArrayList<String> getChartList(String allipStr, String starttime, String totime) {
		ArrayList<String> charList = new ArrayList<String>();
		CreateAmLinePic linePic = new CreateAmLinePic();
		List policyInterfaceList = linePic.getInterfaceList(allipStr, starttime, totime);
		...
		String[] imgpaths = makeJfreeChartInterface(sortData, "队列匹配及丢包曲线图(端口名:类名:指标)", "", "");
	 ...
	 * Acl信息报表
	 */
	public ArrayList<String> getAclChartList(String ip, String starttime, String totime) {
		ArrayList<String> charList = new ArrayList<String>();
		...
		String[] imgpaths = makeAclJfreeChart(sortData, ip, "Acl匹配曲线图", "", "");
		if (imgpaths != null) {
			for (int i = 0; i < imgpaths.length; i++) {
				charList.add(imgpaths[i]);
			}
	...
	public ArrayList<?> getTableList(String allipStr) {
		...
		if (classList != null && classList.size() > 0) {
			for (int i = 0; i < classList.size(); i++) {
				...
				String[] arr1 = { info.getName(), info.getValue(), info.getCollecttime() };
		...
		if (policyList != null && policyList.size() > 0) {
			for (int i = 0; i < policyList.size(); i++) {
				...
				String[] arr1 = { info.getPolicyName(), info.getName(), info.getValue(), info.getCollecttime() };
				policyal.add(arr1);
			}
		}
	 ...
	 * @return
	 */
	private HashMap<?, ArrayList<?>> exportNetHost(String ids, String type, String filePath, String startTime, String toTime) {
		...
		for (int i = 0; i < portin.size(); i++) {
			...
			String[] arry1 = { snin.getIp() + "(" + snin.getName() + "入口)", snin.getCurrent(), snin.getMaximum(), snin.getAverage() };
			String[] arry2 = { snout.getIp() + "(" + snout.getName() + "出口)", snout.getCurrent(), snout.getMaximum(), snout.getAverage() };
			portal.add(arry1);
			portal.add(arry2);
		}
	 ...
	 * @return
	 */
	private HashMap<?, ArrayList<?>> exportDkdb(String ids, String type, String filePath, String startTime, String toTime) {
		HashMap hm = new HashMap();
		// 存放表格信息
		ArrayList<ArrayList<String[]>> tableList = new ArrayList<ArrayList<String[]>>();
		// 存放图片路径
		...
		for (int i = 0; i < portin.size(); i++) {
			...
			portInName.add("(" + snin.getName() + "入口)");
			portOutName.add("(" + snout.getName() + "出口)");
			String[] arry1 = { snin.getIp() + "(" + snin.getName() + "入口)", snin.getCurrent(), snin.getMaximum(), snin.getMininum(), snin.getAverage() };
			String[] arry2 = { snout.getIp() + "(" + snout.getName() + "出口)", snout.getCurrent(), snout.getMaximum(), snout.getMininum(), snout.getAverage() };
			portal.add(arry1);
			portal.add(arry2);
		}
		tableList.add(portal);
	 ...
	 * @return
	 */
	private HashMap<?, ArrayList<?>> exportMidware(String ids, String type, String filePath, String startTime, String toTime) {
		HashMap hm = new HashMap();
		// 存放表格信息
		ArrayList<ArrayList<String[]>> tableList = new ArrayList<ArrayList<String[]>>();
		// 存放图片路径
	...
	private HashMap<?, ArrayList<?>> exportDb(String ids, String type, String filePath, String startTime, String toTime) {
		...
		if (tablespace.getListValue() != null && tablespace.getListValue().size() > 0) {
			tablespacealspath = makeJfreeChartData((List) tablespace.getListValue().get(0), tablespace.getIpList(), "oracle表空间", "时间", "");
		}
	 ...
	 * @return
	 */
	private String makePortJfreeChartData(ArrayList<List<?>> alport, ArrayList<ArrayList<String>> portName, List<String> ipList, String title, String xdesc, String ydesc) {
				...
				for (int j = 0; j < dataList.size(); j++) {
					...
					for (int m = 0; m < dataList1.size(); m++) {
						...
						if (xcount > xlabel) {
							...
							int mod = Integer.parseInt(bd.divide(new BigDecimal(xlabel), BigDecimal.ROUND_CEILING).toString());
							if (m % mod == 0) {
								dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j) + pn), String.valueOf(v0.get(1)));
							} else {
								// 解决x坐标太多挤在一起看不清
								dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j) + pn), x);
							}
						} else {
							dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j) + pn), String.valueOf(v0.get(1)));
						}
	 ...
	 * @return 图片路径
	 */
	public String makeJfreeChartData(List dataList, List ipList, String title, String xdesc, String ydesc) {
		String chartPath = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + System.currentTimeMillis() + ".png";
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		if (dataList != null && dataList.size() > 0) {
			int xcount = 0;
			...
			for (int j = 0; j < dataList.size(); j++) {
				...
				for (int m = 0; m < dataList1.size(); m++) {
					...
					if (xcount > xlabel) {
						...
						int mod = Integer.parseInt(bd.divide(new BigDecimal(xlabel), BigDecimal.ROUND_CEILING).toString());
						if (m % mod == 0) {
							dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j)), String.valueOf(v0.get(1)));
						} else {
							// 解决x坐标太多挤在一起看不清
							dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j)), x);
						}
					} else {
						dataset.addValue(Double.parseDouble(String.valueOf(v.get(0))), String.valueOf(ipList.get(j)), String.valueOf(v0.get(1)));
					}
	...
	public String makeJfreeChartData(List dataList, String title, String xdesc, String ydesc) {
		String chartPath = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + System.currentTimeMillis() + ".png";
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		;
		if (dataList != null && dataList.size() > 0) {
				...
				if (interfaceList != null && interfaceList.size() > 0) {
					for (int i = 0; i < interfaceList.size(); i++) {
						if (i > timeVector.size())
							continue;
						PolicyInterface vo = (PolicyInterface) interfaceList.get(i);
						if (xcount > xlabel) {
							x = x + "\r";
							BigDecimal bd = new BigDecimal(xcount);
							int mod = Integer.parseInt(bd.divide(new BigDecimal(xlabel), BigDecimal.ROUND_CEILING).toString());
							if (i % mod == 0) {
								dataset.addValue(Double.parseDouble(String.valueOf(vo.getOfferedRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":offered rate",
										timeVector.get(i));
								dataset.addValue(Double.parseDouble(String.valueOf(vo.getDropRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":drop rate", timeVector
										.get(i));
							} else {
								dataset.addValue(Double.parseDouble(String.valueOf(vo.getOfferedRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":offered rate", x);
								dataset.addValue(Double.parseDouble(String.valueOf(vo.getDropRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":drop rate", x);
							}
						} else {
							dataset.addValue(Double.parseDouble(String.valueOf(vo.getOfferedRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":offered rate", timeVector
									.get(i));
							dataset.addValue(Double.parseDouble(String.valueOf(vo.getDropRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":drop rate", timeVector
									.get(i));
						}
	...
	public String makeJfreeChartQueue(List dataList, String title, String xdesc, String ydesc) {
		String chartPath = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + System.currentTimeMillis() + ".png";
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		String[] items = { "input size", "input max", "input drops", "input flushes", "output size", "output max total", "output threshold", "output drops",
				...
				if (xcount > xlabel) {
					...
					if (j % mod == 0) {
						for (int k = 0; k < items.length; k++) {
							dataset.addValue(Double.parseDouble(String.valueOf(values[k])), items[k], time);
						}

					} else {
						// 解决x坐标太多挤在一起看不清
						for (int k = 0; k < items.length; k++) {
							dataset.addValue(Double.parseDouble(String.valueOf(values[k])), items[k], x);
						}
	...
	// 接口丢包情况
	public String[] makeJfreeChartInterface(List dataList, String title, String xdesc, String ydesc) {
		...
		if (dataList != null && dataList.size() > 0) {
			...
			for (int j = 0; j < dataList.size(); j++) {
				chartPaths[j] = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + System.currentTimeMillis() + j + ".png";
	...
	// Acl曲线图情况
	public String[] makeAclJfreeChart(HashMap<Integer, List<?>> alldata, String ip, String title, String xdesc, String ydesc) {
		String[] chartPaths = null;
		if (alldata != null) {
			AclBaseDao baseDao = null;
			List<AclBase> baseList = null;
			...
			for (int j = 0; j < dataList.size(); j++) {
				...
				if (interfaceList != null && interfaceList.size() > 0) {
					for (int i = 0; i < interfaceList.size(); i++) {
						...
						AclDetail vo = (AclDetail) interfaceList.get(i);
						if (xcount > xlabel) {
							x = x + "\r";
							BigDecimal bd = new BigDecimal(xcount);
							int mod = Integer.parseInt(bd.divide(new BigDecimal(xlabel), BigDecimal.ROUND_CEILING).toString());
							if (i % mod == 0) {
								...
								dataset.addValue(Double.parseDouble(String.valueOf(vo.getMatches())), String.valueOf(vo.getName()), x);
							}
						} else {
							dataset.addValue(Double.parseDouble(String.valueOf(vo.getMatches())), String.valueOf(vo.getName()), vo.getCollecttime());
						}
	...
	public void addDataSet(DefaultCategoryDataset dataset, PolicyInterface vo, String time) {
		if (vo.getMatchedBytes() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getOfferedRate())), vo.getInterfaceName() + ":" + vo.getClassName() + ":bytes matched", time);
		}
		if (vo.getMatchedPkts() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getMatchedPkts())), vo.getInterfaceName() + ":" + vo.getClassName() + ":pkts matched", time);

		}
		if (vo.getDropsTotal() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getDropsTotal())), vo.getInterfaceName() + ":" + vo.getClassName() + ":total drops", time);

		}
		if (vo.getDropsBytes() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getDropsBytes())), vo.getInterfaceName() + ":" + vo.getClassName() + ":bytes drops", time);
		}
		if (vo.getDepth() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getDepth())), vo.getInterfaceName() + ":" + vo.getClassName() + ":depth", time);
		}
		if (vo.getTotalQueued() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getTotalQueued())), vo.getInterfaceName() + ":" + vo.getClassName() + ":total queued", time);
		}
		if (vo.getNoBufferDrop() != -1) {
			dataset.addValue(Double.parseDouble(String.valueOf(vo.getNoBufferDrop())), vo.getInterfaceName() + ":" + vo.getClassName() + ":no-buffer drops", time);
		}
	}
	 ...
	 * @return
	 */
	private String makeDiskJfreeChartData(ArrayList<String[]> dataList, List ipList, String title, String xdesc, String ydesc) {
		String chartPath = ResourceCenter.getInstance().getSysPath() + "temp" + File.separator + System.currentTimeMillis() + ".png";
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		if (dataList != null) {
			for (int j = 0; j < dataList.size(); j++) {
