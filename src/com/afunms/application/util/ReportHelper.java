			...
			for (int i = 0; i < idValue.length; i++) {
				if (idValue[i].indexOf("ping") >= 0) {
					...
					try {
						...
						if ("0".equals(runmodel)) {
							...
							if (pingData != null && pingData.size() > 0) {
								PingCollectEntity ping = (PingCollectEntity) pingData.get(0);
					...
					try {
						...
						if (utilOutDataList != null && utilOutDataList.size() > 0)
				...
				if (itemId.equals("ping")) {
					...
					try {
						if (typevo.getDbtype().equalsIgnoreCase("oracle")) {
							...
							curhash = hostmanager.getCurByCategory(id, "ORAPing", "ConnectUtilization");
							...
							curhash = hostmanager.getCurByCategory(id, "SQLPing", "ConnectUtilization");
							...
							curhash = hostmanager.getCurByCategory(ip, "DB2Ping", "ConnectUtilization");
							...
							curhash = hostmanager.getCurByCategory(id, "SYSPing", "ConnectUtilization");
							...
							curhash = hostmanager.getCurByCategory(id, "INFORMIXPing", "ConnectUtilization");
							...
							curhash = hostmanager.getCurByCategory(id, "MYPing", "ConnectUtilization");
						}
						if (pinghash != null && pinghash.size() > 0) {
							List pingDataList = (List) pinghash.get("list");
