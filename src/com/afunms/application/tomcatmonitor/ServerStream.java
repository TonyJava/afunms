...
public class ServerStream {
	...
	public static Logger logger = Logger.getLogger(ServerStream.class);
			...
			while ((inputLine = in.readLine()) != null) {
				...
				if (flag == 0) {
					if (inputLine.contains("<tr>")) {
						inputLine = inputLine.substring(inputLine.indexOf("<tr"), inputLine.indexOf("<tr") + 3) + " bgcolor=\"#ECECEC\" height=\"28\">";
						i = 1;
						resultBuffer.append(inputLine);
						resultBuffer.append("\n");
					}
					if (i < 7 && i > 0 && inputLine.contains("<td")) {
						inputLine = inputLine.substring(inputLine.indexOf("<td"), inputLine.indexOf("<td") + 3) + " align=center>"
					...
					if (inputLine.contains("<tr>")) {
						tdCountFlag = 0;
						inputLine = inputLine.substring(inputLine.indexOf("<tr"), inputLine.indexOf("<tr") + 3)
								+ " bgcolor=\"#FFFFFF\"   onmouseout=\"this.style.background='#FFFFFF'\"  onmouseover=\"this.style.background='#AACCFF'\" height=\"28\">";
						j = 1;
						resultBuffer.append(inputLine);
						resultBuffer.append("\n");
					}
					if (j <= 4 && j > 0 && inputLine.contains("<td")) {
						inputLine = inputLine.substring(inputLine.indexOf("<td"), inputLine.indexOf("<td") + 3)
								+ " align=center class=\"application-detail-data-body-list\">&nbsp;" + inputLine.substring(inputLine.indexOf(">") + 1);
	...
	public Hashtable sendParamToServer(Hashtable h_server, Hashtable h_jvm, Hashtable h_portsum1, Hashtable h_portdetail1, Hashtable h_portsum2, Hashtable h_portdetail2,
	...
	public boolean isValid(String ip, String port, String target, String realm, String user, String password) {
		...
		try {
			...
			httpclient.getCredentialsProvider().setCredentials(new AuthScope(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT)),
