...
public abstract class AbstractSnmp implements PDUFactory {
	...
	public String[] getResponse(String[] oid, String host, String community, int length) {
		...
		try {
			...
			for (int i = 0; i < listSize; i++) {
				...
				String operat = response.getVariableBindings().get(i).toString();
				sResponse[i] = operat.substring(operat.lastIndexOf("=") + 1, operat.length()).trim();
			}
	...
	public boolean testSnmp(String host) throws Exception {
		boolean b = false;
		try {
			String s_testSnmp = getResponse(new String[] { "1.3.6.1.2.1.1.1.0" }, host);
