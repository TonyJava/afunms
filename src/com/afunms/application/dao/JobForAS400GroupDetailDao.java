...
public class JobForAS400GroupDetailDao extends BaseDao implements DaoInterface {
	...
	public List getJobForAS400GroupDetailByGroupId(String groupId) {
		...
		try {
			String sql = "select * from nms_as400_job_group_detail where group_id='" + groupId + "'";
	...
	public boolean saveJobForAS400GroupDetailList(String gourpId, List list) {
		try {
			...
			while (iterator.hasNext()) {
				JobForAS400GroupDetail jobForAS400GroupDetail = (JobForAS400GroupDetail) iterator.next();
				sql = "insert into nms_as400_job_group_detail(group_id,name,num,status,active_status,active_status_type) values ('" + gourpId + "','"
						+ jobForAS400GroupDetail.getName() + "','" + jobForAS400GroupDetail.getNum() + "','" + jobForAS400GroupDetail.getStatus() + "','"
						+ jobForAS400GroupDetail.getActiveStatus() + "','" + jobForAS400GroupDetail.getActiveStatusType() + "')";
				try {
					conn.executeUpdate(sql);
				} catch (Exception e) {
					e.printStackTrace();
