...
public class TemperatureHumidityThresholdDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sql.append("insert into system_temper_humi_thresh(id,node_id,min_temperature,max_temperature," + "min_humidity,max_humidity)values(");
	...
	public TemperatureHumidityThresholdConfig findByNodeId(String nodeId) {
		...
		String sql = "select * from system_temper_humi_thresh where node_id='" + nodeId + "'";
		List list = findByCriteria(sql);
		if (list != null && list.size() > 0) {
			temperatureHumidityThresholdConfig = (TemperatureHumidityThresholdConfig) list.get(0);
		}
		return temperatureHumidityThresholdConfig;
	}
