...
public class ProcessGroupConfigurationDao extends BaseDao implements DaoInterface {
	...
	public List getProcessGroupConfigurationByGroupId(String groupId) {
		...
		try {
			String sql = "select * from nms_process_group_config where group_id='" + groupId + "'";
	...
	public boolean saveProcessGroupConfigurationList(String gourpId, List processGroupConfigurationList) {
		try {
			...
			while (iterator.hasNext()) {
				ProcessGroupConfiguration processGroupConfiguration = (ProcessGroupConfiguration) iterator.next();
				if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
					sql = "insert into nms_process_group_config(group_id,name,times,status) values ('" + gourpId + "','" + processGroupConfiguration.getName() + "','"
							+ processGroupConfiguration.getTimes() + "','" + processGroupConfiguration.getStatus() + "')";
				} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
					sql = "insert into nms_process_group_config(group_id,name,times,status) values ('" + gourpId + "','" + processGroupConfiguration.getName() + "'," + "to_date('"
							+ processGroupConfiguration.getTimes() + "','YYYY-MM-DD HH24:MI:SS')" + ",'" + processGroupConfiguration.getStatus() + "')";
				}
				try {
					conn.executeUpdate(sql);
				} catch (Exception e) {
