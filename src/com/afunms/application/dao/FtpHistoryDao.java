...
public class FtpHistoryDao extends BaseDao implements DaoInterface {
	...
	public Vector getByFTPid(Integer ftpid, String starttime, String totime, Integer isconnected) throws Exception {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				...
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " + "a.ftp_id=" + ftpid + " and (a.mon_time >= '" + starttime
						+ "' and  a.mon_time <= '" + totime + "')";
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " + "a.ftp_id=" + ftpid + " and (a.mon_time >= " + "to_date('" + starttime
	...
	public Hashtable getPingData(Integer ftp_id, String starttime, String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			String sql = "";
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " + "a.ftp_id=" + ftp_id + " and (a.mon_time >= '" + starttime
						+ "' and  a.mon_time <= '" + endtime + "')";
				;
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " + "a.ftp_id=" + ftp_id + " and (a.mon_time >= " + "to_date('" + starttime
