...
public class UpAndDownMachineDao extends BaseDao implements DaoInterface {
	...
	public Cluster findDataById(int id) {
		String sql = "select c.id as id,c.name as name,c.serverType as serverType,c.createtime as createtime from nms_remote_up_down_cluster c,nms_remote_up_down_machine m where m.clusterId=c.id and m.id="
				+ id + " order by m.clusterId,m.id";
		rs = conn.executeQuery(sql);
