	...
	public Hashtable getPingDataById(String ip, Integer id, String starttime, String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue,a.collecttime from jbossping" + id + " a where " + "(a.collecttime >= '" + starttime + "' and  a.collecttime <= '" + endtime
						+ "') order by id";
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue from jbossping" + id + " a where " + " (a.collecttime >= " + "to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS')"
