...
public class DHCPConfigDao extends BaseDao implements DaoInterface {
	...
	public boolean delete(String id) {
		...
		try {
			...
			ctable.deleteTable("dhcpping", allipstr, "dhcpping");// Ping
			ctable.deleteTable("dhcpphour", allipstr, "dhcpphour");// Ping
			ctable.deleteTable("dhcppday", allipstr, "dhcppday");// Ping
	...
	public boolean save(BaseVo vo) {
		...
		try {
			...
			ctable.createTable("dhcpping", allipstr, "dhcpping");// Ping
			ctable.createTable("dhcpphour", allipstr, "dhcpphour");// Ping
			ctable.createTable("dhcppday", allipstr, "dhcppday");// Ping
	...
	public synchronized boolean createHostData(Vector pingdataV, DHCPConfig dhcpconf) {
		...
		try {
			...
			for (int i = 0; i < pingdataV.size(); i++) {
				PingCollectEntity pingdata = (PingCollectEntity) pingdataV.get(i);
	...
	public Hashtable getPingDataById(String ip, Integer id, String starttime, String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			String sql = "";
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue,a.collecttime from dhcpping" + ip + " a where " + "(subentity = 'ConnectUtilization' and a.collecttime >= '" + starttime
						+ "' and  a.collecttime <= '" + endtime + "') order by id";
				;
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue from dhcpping" + ip + " a where " + " ( subentity = 'ConnectUtilization' and a.collecttime >= " + "to_date('" + starttime
