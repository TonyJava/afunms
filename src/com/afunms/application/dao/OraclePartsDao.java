...
public class OraclePartsDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sql = sql.append("insert into nms_oracle_sids (id,dbid,sid,users,password,gzerid,collecttype,alias,managed,bid) values(");
	...
	public List findOracleParts(int dbid, int managed) {
		String sql = "select id,sid,users,password,gzerid,dbid,collecttype,alias,managed,bid from nms_oracle_sids where dbid=" + dbid + " and managed=" + managed;
	...
	public List findOracleParts(int dbid) {
		String sql = "select id,sid,users,password,gzerid,dbid,collecttype,alias,managed,bid from nms_oracle_sids where dbid=" + dbid;
		return findByCriteria(sql);
	}

	public BaseVo getOracleById(int id) {
		StringBuilder sql = new StringBuilder();
		sql.append("select id,dbid,sid,users,password,gzerid,collecttype,alias,managed,bid from nms_oracle_sids where id=");
		sql.append(id);
	...
	public BaseVo getOracleBySId(int dbid, String sid) {
		StringBuilder sql = new StringBuilder();
		sql.append("select id,users,password,gzerid,dbid,sid,collecttype,alias,managed,bid from nms_oracle_sids where dbid=");
		sql.append(dbid);
		sql.append(" and sid='");
		sql.append(sid + "'");
		ResultSet rs = conn.executeQuery(sql.toString());
	...
	public boolean saveOracles(List<OracleEntity> list, int dbid) {
		if (list != null && list.size() > 0) {
			...
			builder.append("insert into nms_oracle_sids (id,dbid,sid,users,password,gzerid,collecttype,alias,managed,bid) values");
			int i = 0;
	...
	public List getOraclesByDbAndBid(int dbid, Vector bids) {
		StringBuilder sql = new StringBuilder();
		sql.append("select id,sid,users,password,gzerid,dbid,collecttype,alias,managed,bid from nms_oracle_sids where dbid=" + dbid);
		boolean flag = true;
		if (bids == null)
			flag = false;
		if (flag & bids.size() == 1) {
