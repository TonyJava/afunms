...
public class PSTypeDao extends BaseDao implements DaoInterface {
	...
	public boolean update(BaseVo baseVo) {
		...
		if (!vo.getIpaddress().equals(pvo.getIpaddress()) || !vo.getPort().equalsIgnoreCase(pvo.getPort())) {
			try {
				...
				ctable.deleteTable("spping" + pvo.getPort(), allipstr, "spping");// Ping
				ctable.deleteTable("sppinghour" + pvo.getPort(), allipstr, "sppinghour");// PingHour
				ctable.deleteTable("sppingday" + pvo.getPort(), allipstr, "sppingday");// PingDay
				String ip = vo.getIpaddress();
				allipstr = SysUtil.doip(ip);
				ctable = new CreateTableManager();
				ctable.createTable("spping" + vo.getPort(), allipstr, "spping");// Ping
				ctable.createTable("sppinghour" + vo.getPort(), allipstr, "sppinghour");// PingHour
				ctable.createTable("sppingday" + vo.getPort(), allipstr, "sppingday");// PingDay
			} catch (Exception e) {
				flag = false;
				e.printStackTrace();
			}  
	...
	public boolean delete(String id) {
		...
		try {
			...
			CreateTableManager ctable = new CreateTableManager();
			conn = new DBManager();
			ctable.deleteTable("spping" + vo.getPort(), allipstr, "spping");// Ping
			ctable.deleteTable("sppinghour" + vo.getPort(), allipstr, "sppinghour");// PingHour
			ctable.deleteTable("sppingday" + vo.getPort(), allipstr, "sppingday");// PingDay
	...
	// 处理Ping得到的数据，放到历史表里
	public synchronized boolean createHostData(PingCollectEntity pingdata, String port) {
		if (pingdata == null)
			return false;
		try {
	...
	public Hashtable getCategory(String ip, String category, String subentity, String starttime, String endtime, String port) throws Exception {
		...
		try {
			if (!starttime.equals("") && !endtime.equals("")) {
				...
				if ((category.equals("Ping") || category.equals("ORAPing") || category.equals("DB2Ping") || category.equals("SYSPing") || category.equals("SQLPing") || category
						.equals("SOCKETPing"))
