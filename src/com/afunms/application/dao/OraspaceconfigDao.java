...
public class OraspaceconfigDao extends BaseDao implements DaoInterface {
		...
		try {
			...
			if (list != null && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					...
					String[] tip = IpTranslation.getIpFromHex(iparry[0]);
	...
	/*
	 * 查询唯一IP列表 auth@hukelei date 2010-01-21
	 * 
	 */
					...
					if (spaces != null && spaces.size() > 0) {
						...
						if (tableinfo_v != null && tableinfo_v.size() > 0) {
							...
							for (int k = 0; k < tableinfo_v.size(); k++) {
								...
								String tip = IpTranslation.formIpToHex(iparr[0]);
								if (!oraspacehash.containsKey(tip + ":" + iparr[1] + ":" + spacename)) {
									...
									oraspaceconfig.setSms(new Integer(0));// 0：不告警
									oraspaceconfig.setReportflag(new Integer(0));// 0：不存在于报表
									conn = new DBManager();
									save(oraspaceconfig);
