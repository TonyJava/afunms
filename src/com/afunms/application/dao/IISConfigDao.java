...
public class IISConfigDao extends BaseDao implements DaoInterface {
			...
			if (ids != null && ids.length > 0) {
				for (int i = 0; i < ids.length; i++) {
					try {
						...
						ctable.deleteTable("iisping", allipstr, "iisping");// Ping
						ctable.deleteTable("iispinghour", allipstr, "iispinghour");// Ping
						ctable.deleteTable("iispingday", allipstr, "iispingday");// Ping
						ctable.deleteTable("iisconn", allipstr, "iisconn");// Ping
						ctable.deleteTable("iiserr", allipstr, "iiserr");// Ping
	...
	public boolean save(BaseVo vo) {
		...
		try {
			...
			ctable.createTable("iisping", allipstr, "iisping");// Ping
			ctable.createTable("iispinghour", allipstr, "iispinghour");// Ping
			ctable.createTable("iispingday", allipstr, "iispingday");// Ping
			ctable.createTable("iisconn", allipstr, "iisconn");// Connection
			ctable.createTable("iiserr", allipstr, "iiserr");// totalNotFoundErrors
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}  
	...
	public Hashtable getPingDataById(String ip, Integer id, String starttime, String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			String sql = "";
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue,a.collecttime from iisping" + ip.replace(".", "_") + " a where " + "(a.collecttime >= '" + starttime + "' and  a.collecttime <= '"
						+ endtime + "') order by id";
				;
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue from iisping" + ip.replace(".", "_") + " a where " + " (a.collecttime >= " + "to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS')"
