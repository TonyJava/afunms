...
public class ApacheManager extends BaseManager implements ManagerInterface {
	...
	private String add() {
		...
		try {
			...
			ctable.createTable("apaping", allipstr, "apaping");// Ping
			ctable.createTable("apapingh", allipstr, "apapingh");// Ping
			ctable.createTable("apapingd", allipstr, "apapingd");// Ping
	...
	public String delete() {
		...
		if (ids != null && ids.length > 0) {
			...
			for (int i = 0; i < ids.length; i++) {
				...
				CreateTableManager ctable = new CreateTableManager();
				try {
					ctable.deleteTable("apaping", allipstr, "apaping");// Ping
					ctable.deleteTable("apapingh", allipstr, "apapingh");// Ping
					ctable.deleteTable("apapingd", allipstr, "apapingd");// Ping
				} catch (Exception e) {
	...
	public Hashtable collectapachedata(ApacheConfig apacheConfig) {
		...
		String src = HttpClientJBoss.getGetResponseWithHttpClient("http://" + apacheConfig.getIpaddress() + ":" + apacheConfig.getPort() + "/server-status", "GBK");
		int status = 0;
	...
	private String alarm() {
		...
		try {
			...
			// 是否连通
			// String conn[] = new String[2];
			// conn = configdao.getAvailability(vo.getIpaddress(), starttime,
			// totime, "thevalue");
