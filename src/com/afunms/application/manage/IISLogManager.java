...
public class IISLogManager extends BaseManager implements ManagerInterface {
	...
	public String delete() {
		...
		if (ids != null && ids.length > 0) {
			...
			TimeGratherConfigUtil timeGratherConfigUtil = new TimeGratherConfigUtil(); // snow
																						...
																						// 删除数据库采集时间
			for (String id : ids) {
				timeGratherConfigUtil.deleteTimeGratherConfig(id, timeGratherConfigUtil.getObjectType("18")); // snow
																												// add
																												// at
																												// 2010-5-20
			}
	...
	private String getSQL(String ip) {
		...
		if (sip != null && !" ".equals(sip)) {
			...
			// tempStr1 =
			// sip.substring(sip.indexOf(".")+1,sip.lastIndexOf("."));
			// }
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				if (cip != null && !" ".equals(cip) && " ".equals(sip)) {
					s.append("select * from iislog" + allipstr + " where recordtime>= '" + starttime1 + "' " + "and recordtime<='" + totime1 + "' and cip like '%" + cip + "%'");
				} else if (sip != null && "".equals(cip)) {
					s.append("select * from iislog" + allipstr1 + " where recordtime>= '" + starttime1 + "' " + "and recordtime<='" + totime1 + "'");

				} else if (sip != null && cip != null) {
					s.append("select * from iislog" + allipstr1 + " where recordtime>= '" + starttime1 + "' " + "and recordtime<='" + totime1 + "' and cip like '%" + cip + "%'");

				} else {
					s.append("select * from iislog" + allipstr + " where recordtime>= '" + starttime1 + "' " + "and recordtime<='" + totime1 + "'");
				}
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				if (cip != null && !" ".equals(cip) && " ".equals(sip)) {
					s.append("select * from iislog" + allipstr + " where recordtime>= to_date('" + starttime1 + "','YYYY-MM-DD HH24:MI:SS') " + "and recordtime<=to_date('"
							+ totime1 + "','YYYY-MM-DD HH24:MI:SS') and cip like '%" + cip + "%'");
				} else if (sip != null && "".equals(cip)) {
					s.append("select * from iislog" + allipstr1 + " where recordtime>=to_date('" + starttime1 + "','YYYY-MM-DD HH24:MI:SS') " + "and recordtime<=to_date('"
