...
public class InformixSpaceConfigManager extends BaseManager implements ManagerInterface {
	...
	/*
	 * 修改INFORMIX的表空间告警配置 auth@hukelei date@2010-01-20
	 */
		...
		try {
			...
			if (list != null && list.size() > 0) {
				for (int k = 0; k < list.size(); k++) {
					...
					if (ips.contains(infromixspaceconfig.getIpaddress()))
						conflist.add(infromixspaceconfig);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (list != null && list.size() > 0) {
				for (int k = 0; k < list.size(); k++) {
					...
					if (ips.contains(informixspaceconfig.getIpaddress()))
						conflist.add(informixspaceconfig);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	/*
	 * 依据IP进行告警查询 auth@hukelei date 2010-01-21
	 */
	 ...
	 * 获取单个库的表空间阀值设置
	 * 
	 * @return {@link String} - 返回单个库的表空间阀值设备
	 * 
	 * @since v1.01
	 */
