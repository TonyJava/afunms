...
public class IISManager extends BaseManager implements ManagerInterface {
	...
	public String delete() {
		...
		if (ids != null && ids.length > 0) {
			...
			for (int i = 0; i < ids.length; i++) {
				...
				timeGratherConfigUtil.deleteTimeGratherConfig(ids[i], timeGratherConfigUtil.getObjectType("14")); // snow
																													// add
																													// at
																													// 2010-5-20
	...
	public String eventdetail() {
		...
		try {
			...
			try {
				...
				list = eventdao.getQuery(starttime1, totime1, "iis", status + "", level1 + "", user.getBusinessids(), iisconf.getId());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	private String showPingReport() {
		...
		try {
			...
			Hashtable ConnectUtilizationhash = iisdao.getPingDataById(ip, queryid, starttime, totime);
			String curPing = "";
			String pingconavg = "";
			...
			pollMonitorManager.chooseDrawLineType(timeType, ConnectUtilizationhash, "连通率", newip + "iispingConnect", 740, 150);
	...
	public String event() {
		...
		try {
			...
			try {
				...
				list = dao.getQuery(starttime1, totime1, status + "", level1 + "", vo.getBusinessids(), Integer.parseInt(tmp), "iis");
