...
public class MQchannelManager extends BaseManager implements ManagerInterface {
	private String list() {
		...
		String sqlWhere = "";
		if (ipaddress != null)
			sqlWhere = " where ipaddress='" + ipaddress + "'";
		try {
	...
	private String detail() {
		 ...
		 * java.text.SimpleDateFormat( "yyyy-MM-dd"); SimpleDateFormat sdf1 =
		 * new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); MQchannelConfigDao
		 * configdao = new MQchannelConfigDao(); Urlmonitor_realtimeDao
		 * realtimedao = new Urlmonitor_realtimeDao(); Urlmonitor_historyDao
		 * historydao = new Urlmonitor_historyDao(); List urllist = new
		 * ArrayList(); // 用做条件选择列表 MQchannelConfig initconf = new
		 * MQchannelConfig(); // 当前的对象 String lasttime = ""; String nexttime =
		 * ""; String conn_name = ""; String valid_name = ""; String fresh_name =
		 * ""; String wave_name = ""; String delay_name = ""; String connrate =
		 * "0"; String validrate = "0"; String freshrate = "0"; Calendar now =
		 * Calendar.getInstance(); now.setTime(new Date()); Date nowdate = new
		 * Date(); nowdate.getHours(); String from_date1 =
		 * getParaValue("from_date1"); if (from_date1 == null) { from_date1 =
		 * timeFormatter.format(new Date()); request.setAttribute("from_date1",
		 * from_date1); } String to_date1 = getParaValue("to_date1"); if
		 * (to_date1 == null) { to_date1 = timeFormatter.format(new Date());
		 * request.setAttribute("to_date1", to_date1); } String from_hour =
		 * getParaValue("from_hour"); if (from_hour == null) { from_hour = "00";
		 * request.setAttribute("from_hour", from_hour); } String to_hour =
		 * getParaValue("to_hour"); if (to_hour == null) { to_hour =
		 * nowdate.getHours() + ""; request.setAttribute("to_hour", to_hour); }
		 * String starttime = from_date1 + " " + from_hour + ":00:00"; String
		 * totime = to_date1 + " " + to_hour + ":59:59"; int flag = 0; try {
		 * User operator = (User) session
		 * .getAttribute(SessionConstant.CURRENT_USER); String bids =
		 * operator.getBusinessids(); String bid[] = bids.split(","); Vector
		 * rbids = new Vector(); if (bid != null && bid.length > 0) { for (int i =
		 * 0; i < bid.length; i++) { if (bid[i] != null &&
		 * bid[i].trim().length() > 0) rbids.add(bid[i].trim()); } } configdao =
		 * new MQchannelConfigDao(); try { urllist =
		 * configdao.getMqChanelByBID(rbids); } catch (Exception e) {
		 *  }
		 * 
		 * Integer queryid = getParaIntValue("id");// .getUrl_id();
		 * 
		 * if (urllist.size() > 0 && queryid == null) { Object obj =
		 * urllist.get(0); } if (queryid != null) { // 如果是链接过来则取用查询条件 configdao =
		 * new MQchannelConfigDao(); initconf = (MQchannelConfig)
		 * configdao.findByID(queryid + ""); } queryid = initconf.getId();
		 * conn_name = queryid + "urlmonitor-conn"; valid_name = queryid +
		 * "urlmonitor-valid"; fresh_name = queryid + "urlmonitor-refresh";
		 * wave_name = queryid + "urlmonitor-rec"; delay_name = queryid +
		 * "urlmonitor-delay";
		 * 
		 * List urlList = realtimedao.getByUrlId(queryid);
		 * 
		 * Calendar last = null; if (urlList != null && urlList.size() > 0) {
		 * last = ((Urlmonitor_realtime) urlList.get(0)).getMon_time(); } int
		 * interval = 0; // TaskXmlUtil taskutil = new TaskXmlUtil("urltask");
		 * try { // Session session = this.beginTransaction(); List numList =
		 * new ArrayList(); TaskXml taskxml = new TaskXml(); numList =
		 * taskxml.ListXml(); for (int i = 0; i < numList.size(); i++) { Task
		 * task = new Task(); BeanUtils.copyProperties(task, numList.get(i)); if
		 * (task.getTaskname().equals("urltask")) { interval =
		 * task.getPolltime().intValue(); // numThreads =
		 * task.getPolltime().intValue(); } } } catch (Exception e) {
		 * e.printStackTrace(); } DateE de = new DateE(); if (last == null) {
		 * last = new GregorianCalendar(); flag = 1; } lasttime =
		 * de.getDateDetail(last); last.add(Calendar.MINUTE, interval); nexttime =
		 * de.getDateDetail(last);
		 * 
		 * int hour = 1; if (getParaValue("hour") != null) { hour =
		 * Integer.parseInt(getParaValue("hour")); } else {
		 * request.setAttribute("hour", "1"); // urlconfForm.setHour("1"); }
		 * 
		 * InitCoordinate initer = new InitCoordinate(new GregorianCalendar(),
		 * hour, 1); // Minute[] minutes=initer.getMinutes(); TimeSeries ss1 =
		 * new TimeSeries("", Minute.class); TimeSeries ss2 = new TimeSeries("",
		 * Minute.class);
		 *  // ss.add() TimeSeries[] s = new TimeSeries[1]; TimeSeries[] s_ =
		 * new TimeSeries[1]; // Vector wave_v =
		 * historyManager.getInfo(queryid,initer); Vector wave_v = historydao
		 * .getByUrlid(queryid, starttime, totime, 0);
		 * 
		 * for (int i = 0; i < wave_v.size(); i++) { Hashtable ht = (Hashtable)
		 * wave_v.get(i); double conn =
		 * Double.parseDouble(ht.get("conn").toString()); double fresh =
		 * Double.parseDouble(ht.get("refresh").toString()); double condelay =
		 * Double.parseDouble(ht.get("condelay") .toString()); String time =
		 * ht.get("mon_time").toString(); ss1.addOrUpdate(new
		 * Minute(sdf1.parse(time)), conn); ss2.addOrUpdate(new
		 * Minute(sdf1.parse(time)), condelay); } s[0] = ss1; s_[0] = ss2;
		 * ChartGraph cg = new ChartGraph(); cg.timewave(s, "时间", "连通", "",
		 * wave_name, 600, 120); cg.timewave(s_, "时间", "时延(ms)", "", delay_name,
		 * 600, 120); //
		 * p_draw_line(s,"","url"+queryid+"ConnectUtilization",740,120);
		 *  // 是否连通 String conn[] = new String[2]; if (flag == 0) // conn = //
		 * historyManager.getAvailability(queryid,initer,"is_canconnected");
		 * conn = historydao.getAvailability(queryid, starttime, totime,
		 * "is_canconnected"); else { // conn[0] = "0"; // conn[1] = "0"; //
		 * conn = //
		 * historyManager.getAvailability(queryid,initer,"is_canconnected");
		 * conn = historydao.getAvailability(queryid, starttime, totime,
		 * "is_canconnected"); } String[] key1 = { "连通", "未连通" };
		 * drawPiechart(key1, conn, "", conn_name); // drawchart(minutes,"连通",) //
		 * Vector conn_v = //
		 * historyManager.getInfo(queryid,initer,"is_canconnected"); // for(int
		 * i=0;i<conn) if (flag == 0) connrate =
		 * getF(String.valueOf(Float.parseFloat(conn[0]) /
		 * (Float.parseFloat(conn[0]) + Float .parseFloat(conn[1])) * 100)) +
		 * "%"; // 是否有效 String avail[] = new String[2]; if (flag == 0) avail =
		 * historydao.getAvailability(queryid, initer, "is_valid"); else {
		 * avail[0] = "0"; avail[1] = "0"; } String[] key2 = { "有效", "无效" };
		 * drawPiechart(key2, avail, "页面有效情况", valid_name); if (flag == 0)
		 * validrate = getF(String.valueOf(Float.parseFloat(avail[0]) /
		 * (Float.parseFloat(avail[0]) + Float .parseFloat(avail[1])) * 100)) +
		 * "%";
		 *  // 是否刷新 String refresh[] = new String[2]; if (flag == 0) refresh =
		 * historydao.getAvailability(queryid, initer, "is_refresh"); else {
		 * refresh[0] = "0"; refresh[1] = "0"; }
		 * 
		 * String[] key3 = { "刷新", "未刷新" }; drawPiechart(key3, refresh,
		 * "页面刷新情况", fresh_name); if (flag == 0) freshrate =
		 * getF(String.valueOf(Float.parseFloat(refresh[0]) /
		 * (Float.parseFloat(refresh[0]) + Float .parseFloat(refresh[1])) *
