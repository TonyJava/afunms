...
public class JBossManager extends BaseManager implements ManagerInterface {
	...
	public String detail() {
		...
		// service.getJBossData(nodeDTO.getNodeid());
	...
	private String isOK() {
		...
		NodeGatherIndicatorsDao indicatorsdao = new NodeGatherIndicatorsDao();
		List<NodeGatherIndicators> monitorItemList = new ArrayList<NodeGatherIndicators>();
		Hashtable<String, Hashtable<String, NodeGatherIndicators>> urlHash = new Hashtable<String, Hashtable<String, NodeGatherIndicators>>();// 存放需要监视的IIS指标
		...
		// try{
		// JBossPerformanceIndicatorGather jboss = new
		// JBossPerformanceIndicatorGather();
		// NodeGatherIndicators vo = new NodeGatherIndicators();
		// vo.setNodeid(queryid+"");
	...
	public String alarm() {
		...
		try {
			...
			try {
				...
				list = eventdao.getQuery(starttime1, totime1, "jboss", status + "", level1 + "", user.getBusinessids(), jbossconf.getId());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	private String showPingReport() {
		...
		try {
			...
			Hashtable ConnectUtilizationhash = jbossdao.getPingDataById(ip, queryid, starttime, totime);
			String curPing = "";
			String pingconavg = "";
			...
			pollMonitorManager.chooseDrawLineType(timeType, ConnectUtilizationhash, "连通率", newip + "jbosspingConnect", 740, 150);
	...
	// %H:%i:%s') as collecttime,h.unit from iisping"+allipstr+" h where ");
	// }else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
	// sb.append(" select h.thevalue,to_char(h.collecttime,'YYYY-MM-DD
	// HH24:MI:SS') as collecttime,h.unit from iisping"+allipstr+" h where ");
	// }
	//							
	// }
	...
	// if
	// (category.equals("IISPing")&&subentity.equalsIgnoreCase("ConnectUtilization")){
	...
	// }
	// // if
	// (category.equals("tomcat_jvm")&&subentity.equalsIgnoreCase("ConnectUtilization")){
	// // pingcon=pingcon+getfloat(thevalue);
	// // if(thevalue.equals("0")){
	// // downnum = downnum + 1;
	// // }
	...
	public String eventReport() {
		...
		try {
			...
			try {
				...
				list = dao.getQuery(starttime1, totime1, status + "", level1 + "", vo.getBusinessids(), Integer.parseInt(tmp), "jboss");
			...
			// try {
			// Hashtable hash1 =
			// getCategory(ip,"IISPing","ConnectUtilization",starttime1,totime1);
			// Hashtable hash =
	...
	private String changeMonflag() {
		...
		try {
			...
			// Node web =
			// (JBossConfig)PollingEngine.getInstance().getJBossByID(Integer.parseInt(id));
			// web.setFlag(monflag);
			JBossLoader loader = new JBossLoader();
			try {
