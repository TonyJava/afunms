...
public class MQManager extends BaseManager implements ManagerInterface {
	...
	private String add() {
		...
		// Hashtable<String,Hashtable<String,NodeGatherIndicators>> mqHash = new
		// Hashtable<String,Hashtable<String,NodeGatherIndicators>>();//存放需要监视的DB2指标
		...
		try {
			if (ids != null && ids.length > 0) {
				...
				for (int i = 0; i < ids.length; i++) {
					timeGratherConfigUtil.deleteTimeGratherConfig(ids[i], timeGratherConfigUtil.getObjectType("9")); // snow
					// add
					// at
					// 2010-5-20
	...
	public String alarm() {
		...
		try {
			...
			try {
				...
				list = eventdao.getQuery(starttime1, totime1, "mq", status + "", level1 + "", user.getBusinessids(), jbossconf.getId());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	public String eventReport() {
		...
		try {
			...
			try {
				...
				list = dao.getQuery(starttime1, totime1, status + "", level1 + "", vo.getBusinessids(), Integer.parseInt(tmp), "mq");
	...
	private String showPingReport() {
		...
		try {
			...
			Hashtable ConnectUtilizationhash = mq.getPingDataById(newip, queryid, starttime, totime);
			String curPing = "";
			String pingconavg = "";
			...
			pollMonitorManager.chooseDrawLineType(timeType, ConnectUtilizationhash, "连通率", newip + "mqpingConnect", 740, 150);
