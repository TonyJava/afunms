		...
		if (ids != null && ids.length > 0) {
			...
			try {
				...
				for (int i = 0; i < ids.length; i++) {
					...
					ctable.deleteTable("ping", id, "ping");// Ping
					// conn.close();
			...
			try {
				if (ShareData.getAlltracertsdata() != null && !"find".equals(find)) {
					...
					if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
						...
						where = "dotime >" + "to_date('" + starttime1 + "','YYYY-MM-DD HH24:MI:SS')" + " and dotime <" + "to_date('" + totime1 + "','YYYY-MM-DD HH24:MI:SS')"
								+ " and configid = " + tmp + " order by id desc";
					}
