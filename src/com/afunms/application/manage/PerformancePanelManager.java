...
public class PerformancePanelManager extends BaseManager implements ManagerInterface {
		...
		while (panelNamesIterator.hasNext()) {
			...
			if (performancePanelIndicatorsModelList != null) {
				for (int i = 0; i < performancePanelIndicatorsModelList.size(); i++) {// 增加表头
					...
					tableBuffer.append(pIndicators.getIndicatorDesc());// 根据指标的名称
																		// 得到指标的描述
			...
			for (int i = 0; i < performancePanelModelList.size(); i++) {// 增加表体
				...
				if (performancePanelIndicatorsModelList != null) {
					for (int j = 0; j < performancePanelIndicatorsModelList.size(); j++) {// 外层循环:单个面板的指标列表
						...
						if (nodeIndicatorAlarmList == null) {// 第一种情形:
																// 该设备无告警信息
						...
						for (int k = 0; k < nodeIndicatorAlarmList.size(); k++) {// 内层循环
																					// 告警列表
							...
							if (panelIndicatorsName.equalsIgnoreCase(nodeIndicatorAlarmName)) {// 告警指标名相同时
																								// （第二种情形：该设备有告警信息，并且告警指标名跟面板中的指标表头同名时）
								tableBuffer.append("<td align='center' class=\"data_content center\"><img src='");
	 ...
	 * @return
	 */
	private Hashtable<String, List<PerformancePanelIndicatorsModel>> getPanelIndicatorsHash(List<PerformancePanelIndicatorsModel> allPaneIndicatorsList) {
