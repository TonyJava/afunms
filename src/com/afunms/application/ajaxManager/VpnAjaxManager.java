...
public class VpnAjaxManager extends AjaxBaseManager implements AjaxManagerInterface {
	...
	public void exeVpnCmd() {
		...
		if (id != null) {
			...
			if (vo != null) {
				if (vo.getDeviceRender().equals("zte")) {
			...
			for (int k = 0; k < vpnlist.size(); k++) {
				...
				if (vpn != null) {
					...
					if (linkIdVector.contains(linkId)) {
							...
							for (int j = 0; j < linkList.size(); j++) {
								...
								if (model2 != null) {
									...
									if (model2.getId().equals(linkId)) {
										...
										info.append(model2.getLinkInfo()).append("<br><br>起点端口：").append(vpn.getSourcePortName()).append(sourceIfOperStatus).append("<br>终点端口：")
												.append(vpn.getDesPortName()).append(desIfOperStatus);
						...
						linkInfo.append("起点IP：" + sourceIp + "<br>终点IP：" + desIp + "<br><br>" + "起点端口：" + vpn.getSourcePortName()).append(sourceIfOperStatus).append(
								"<br>终点端口：" + vpn.getDesPortName()).append(desIfOperStatus);
