...
public class TrafficRxUtilization extends SnmpMonitor implements MonitorInterface {
	...
	public void collectData(Node node, MonitoredItem monitoredItem) {
		Host host = (Host) node;
		if (host.getInterfaceHash() == null || host.getInterfaceHash().size() == 0) {
			return;
		}

		SnmpItem item = (SnmpItem) monitoredItem;
		String[] oids = new String[] { "1.3.6.1.2.1.2.2.1.1", "1.3.6.1.2.1.2.2.1.10" };
		...
		for (int i = 0; i < valueArray.length; i++) {
			...
			if (ifEntity.getOperStatus() == 2 || ifEntity.getSpeed() == 0) {
				continue;
			}
			 ...
			 */
			long rxTraffic = (diffOctets * 8 / ((SysUtil.getCurrentLongTime() - item.getLastTime()) * 1024));
