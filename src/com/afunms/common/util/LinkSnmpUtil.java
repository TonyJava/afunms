...
public class LinkSnmpUtil {
	...
	public List getIfEntityList(String address, String community, int category) {
		...
		String[] oids1 = new String[] { "1.3.6.1.2.1.2.2.1.1", // index
	...
	public List getSubNetList(String address, String community) {
		...
		try {
			...
			for (int i = 0; i < ipArray.length; i++) {
				...
				if (ipArray[i][4].equals("255.255.255.255") || ipArray[i][4].equals("0.0.0.0") || ipArray[i][2].startsWith("127.0") || ipArray[i][1].startsWith("127.0")
						|| ipArray[i][2].equals("0.0.0.0")) {
					continue;
				}
		...
		try {
			...
			for (int i = 0; i < result.length; ++i) {
				...
				if (null == m_VlanIPAndCommunity.put(ip_port_comm[0] + ip_port_comm[1] + ip_port_comm[2], ip_port_comm)) {
					...
					System.out.println(address + " getVLanIPAndReadCommunity:" + result[i][0] + " " + result[i][1] + " to " + ip_port_comm[0] + " " + ip_port_comm[1] + " "
							+ ip_port_comm[2]);

				} else {

					System.out.println(address + " already getVLanIPAndReadCommunity:" + result[i][0] + " " + result[i][1] + " to " + ip_port_comm[0] + " " + ip_port_comm[1] + " "
							+ ip_port_comm[2]);
