...
public class CreateAmColumnPic {
	String[] colorStr = { "#0000FF", "#36DB43", "#3DA4D8", "#556B2F", "#8470F4", "#8A2BE2", "#23f266", "#FFA500", "#8B4513", "FFB6C1", "#FFF800", "#728699", "#9F005F", "#A52A2A",
			"#0000FF", "#36DB43", "#3DA4D8", "#556B2F", "#23f266" };
	...
	public String createAmMemoryChart(String ip, Hashtable memhash) {
		...
		long[] data = { curp, curv, 100 - curp, 100 - curv, avgp, avgv, 100 - avgp, 100 - avgv, maxp, maxv, 100 - maxp, 100 - maxv };
	...
	public String createCpuChartLastWeek(String ip) {
		...
		if (list != null && list.size() > 0) {
			...
			for (int i = 3; i < list.size(); i++) {
				...
				cpuStr.append("<value xid='").append(i).append("'>").append(name).append("</value>");
			}
			cpuStr.append("</series><graphs><graph>");
			for (int i = 3; i < list.size(); i++) {
				fVo = list.get(i);
				String value = fVo.getObjectNumber();
				cpuStr.append("<value xid='" + i).append("' color='").append(colorStr[i]);
				cpuStr.append("'>");
	 ...
	 * @return
	 */
	public String createCpuDetailAmChart(Hashtable cpuperfhash, Hashtable cpudetailhash) {
		...
		if (cpuperfhash != null && cpuperfhash.size() > 0) {
			...
			for (int i = 0; i < cpu_labels.length; i++) {
				xmlStr.append("<value xid='").append(i).append("'>").append(cpu_labels[i]).append("</value>");
			...
			for (int i = 0; i < title.length; i++) {
				...
				xmlStr.append("<graph gid='" + title[i] + "' title='").append(title[i]).append("'>").append("<value xid='0'> " + cpu_data1[i]).append("</value>");
				xmlStr.append("<value xid='1'>" + cpu_data2[i] + "</value>");
				xmlStr.append("</graph>");
	...
	public String createDiskChart(Hashtable diskhash) {
		...
		if (diskhash != null && diskhash.size() > 0) {
			...
			for (int i = 0; i < diskhash.size(); i++) {
				...
				dataStr.append("<value xid='").append(i).append("'>").append(name).append("</value>");
			...
			for (int i = 0, j = 0; i < diskhash.size(); i++, j++) {
				...
				dataStr.append("<value xid='" + i).append("' color='").append(colorStr[j]);
				dataStr.append("'>");
	...
	public String createDiskChartTop5(Hashtable diskhashtable) {
		...
		if (diskhashtable != null && diskhashtable.size() > 0) {
			...
			for (Iterator itr = diskhash.keySet().iterator(); itr.hasNext();) {
				...
				dataStr.append("<value xid='").append(k++).append("'>").append(name).append("</value>");
	...
	public String createDominoDiskPic(List diskList) {
		...
		if (diskList != null && diskList.size() > 0) {
			...
			for (int i = 0; i < diskList.size(); i++) {
				...
				xmlStr.append("<value xid='").append(i).append("'>").append(disk.getDiskname()).append("</value>");
	 ...
	 * @return
	 */
	public String createLinuxCpuDetailAmChart(Hashtable cpuperfhash, Hashtable cpudetailhash) {
		double[] cpu_data1 = null;
		double[] cpu_data2 = null;
		String dataStr = "";
		...
		if (cpuperfhash != null && cpuperfhash.size() > 0) {
			...
			if (cpudetailhash != null) {
				...
				if (cpudetailhash.containsKey("nice")) {
					Hashtable nicehash = (Hashtable) cpudetailhash.get("nice");
					if (nicehash != null) {
				...
				if (cpudetailhash.containsKey("steal")) {
					Hashtable stealhash = (Hashtable) cpudetailhash.get("steal");
					if (stealhash != null) {
	...
	/**
	 * @author wxy 创建sqlserver日志空间利用率率的柱状图
	 * @param oramem
	 * @return
	 */
		...
		try {
			...
			if (tableinfo != null && tableinfo.size() > 0) {
				...
				for (int i = 0; i < tablespaces.length; i++) {
					tablespace.append("<value xid='").append(i).append("'>").append(tablespaces[i]).append("</value>");
				}
				...
				for (int i = 0, j = 0; i < tspercents.length; i++, j++) {
					...
					tablespace.append("<value xid='").append(i).append("' color='").append(colorStr[j]).append("'>" + tspercents[i]).append("</value>");
	 ...
	 * @return
	 */
	public String createPGAChart(Hashtable oramem){
		String pgadata = "";
		try{
			...
			for (int k = 0; k < sysItem2.length; k++) {
				sb.append("<value xid='").append(k).append("'>").append(sysItemch2[k]).append("</value>");
			}
			sb.append("</series><graphs><graph gid='0'>");
			if (oramem != null) {
				...
				for (int i = 0; i < sysItem2.length; i++) {
					...
					sb.append("<value xid='").append(i).append("' color='").append(colorStr[i]).append("'>" + data).append("</value>");
				}
			} else {
				sb.append("</series><graphs><graph gid='0'>");
			}
	...
	public String createSGAChart(Hashtable oramem) {
		...
		try {
			...
			for (int k = 0; k < sysItem1.length; k++) {
				sga.append("<value xid='").append(k).append("'>").append(sysItemch1[k]).append("</value>");
			}
			...
			if (oramem != null) {
				...
				for (int i = 0; i < sysItem1.length; i++) {
					...
					int data = Math.round(Float.parseFloat(value));
					sga.append("<value xid='").append(i).append("' color='").append(colorStr[i]).append("'>" + data).append("</value>");
				}
			} else {
				sga.append("</series><graphs><graph gid='0'>");
			}
	...
	/**
	 * @author wxy 创建sqlserver各种命中率的柱状图
	 * @param oramem
	 * @return
	 */
	public String createSqlUtilChart(String[] data, String[] items) {
		...
		for (int k = 0; k < items.length; k++) {
			sb.append("<value xid='").append(k).append("'>").append(items[k]).append("</value>");
		}
		sb.append("</series><graphs><graph gid='0'>");
		for (int i = 0; i < data.length; i++) {
			sb.append("<value xid='").append(i).append("' color='").append(colorStr[i]).append("'>" + data[i]).append("</value>");
		}
		sb.append("</graph></graphs></chart>");
	...
	/**
	 * @author wxy 创建sqlserver表空间利用率率的柱状图
	 * @param oramem
	 * @return
	 */
	public String createTableSpaceUtilChart(Hashtable dataValue) {
		...
		if (database != null && database.size() > 0) {
			...
			String[] colorStr = new String[] { "#33FF33", "#FF0033", "#9900FF", "#FFFF00", "#33CCFF", "#003366", "#33FF33", "#FF0033", "#9900FF", "#FFFF00", "#333399", "#0000FF",
					"#A52A2A", "#800080" };
			sb.append("<chart><series>");
			if (dbname != null) {
	...
	public String createWinDiskChart(Hashtable diskhash) {
		...
		if (diskhash != null) {
			...
			for (int i = 0; i < disk_labels.length; i++) {
				xmlStr.append("<value xid='").append(i).append("'>").append(disk_labels[i]).append("</value>");
			...
			for (int i = 0; i < title.length; i++) {
				...
				if (i == 0) {
					for (int j = 0; j < disk_data1.length; j++) {
						xmlStr.append("<value xid='" + j + "'> " + disk_data1[j]).append("</value>");

					}
				} else if (i == 1) {
					for (int j = 0; j < disk_data2.length; j++) {
						xmlStr.append("<value xid='" + j + "'> " + disk_data2[j]).append("</value>");
	...
	public Map.Entry<String, String>[] getSortedHashtable(Hashtable<String, String> h) {

		Set set = h.entrySet();
		Map.Entry<String, String>[] entries = (Map.Entry[]) set.toArray(new Map.Entry[set.size()]);
		Arrays.sort(entries, new Comparator() {
			public int compare(Object arg0, Object arg1) {
				float key1 = Float.parseFloat(((Map.Entry) arg0).getValue().toString());
				float key2 = Float.parseFloat(((Map.Entry) arg1).getValue().toString());
				return ((Comparable) new Float(key2)).compareTo(new Float(key1)); // ---当然为了降序排只要把key1和key2互换一下
			}
		});
