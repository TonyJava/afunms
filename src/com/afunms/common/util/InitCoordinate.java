...
public class InitCoordinate {
	...
	public void computeScale() {
		if (0 < pollminute && pollminute <= 10) {
			scale = 10;
		} else if (10 < pollminute && pollminute <= 20) {
			scale = 20;
		} else if (20 < pollminute && pollminute <= 30) {
			scale = 30;
		} else if (30 < pollminute && pollminute <= 60) {
			scale = 60;
		} else {
			if (pollminute % 60 != 0) {
				scale = (pollminute / 60 + 1) * 60;
			} else {
				scale = pollminute;
			}
		}
	...
	public void InitCoor() {
		...
		for (int i = 0; i < count; i++) {
			...
			minutes[i] = new Minute(temp.get(Calendar.MINUTE), temp.get(Calendar.HOUR_OF_DAY), temp.get(Calendar.DAY_OF_MONTH), temp.get(Calendar.MONTH) + 1, temp
					.get(Calendar.YEAR));
		}
	}
