...
public class PollDataUtil {
			...
			public void run() {
				try {
					...
					if (alarmIndicatorsNode.getName().equalsIgnoreCase("cpu")) {
						...
						// Host node =
						// (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
						// if(node != null){
						// UtilHdxSnmp utilhdxsnmp = null;
						// try{
						// utilhdxsnmp =
						// (UtilHdxSnmp)Class.forName("com.afunms.polling.snmp.interfaces.UtilHdxSnmp").newInstance();
						// returnHash =
						// utilhdxsnmp.collect_Data(alarmIndicatorsNode);
						// //存入数据库
								...
								if (alarmIndicatorsNode.getSubtype().equalsIgnoreCase("h3c")) {
									...
									try {
										...
										for (int i = 0; i < list.size(); i++) {
											...
											if (alarmIndicatorsnode.getName().equalsIgnoreCase("AllInBandwidthUtilHdx")
													|| alarmIndicatorsnode.getName().equalsIgnoreCase("AllOutBandwidthUtilHdx")
	...
	/**
	 * 检查一个ip是否是一个有效的主机地址(是否在掩码的范围内) netAddress:网络IP地址 netMask:子网掩码
	 * ipAddress:主机IP地址
	 */
		...
		try {
			...
			if (nodeGatherIndicatorsNode.getName().equalsIgnoreCase("cpu")) {
				...
				if (node != null) {
					InterfaceSnmp interfacesnmp = null;
					try {
						// SysLogger.info("开始采集"+node.getIpAddress()+"
						// 接口信息...");
						interfacesnmp = (InterfaceSnmp) Class.forName("com.afunms.polling.snmp.interfaces.InterfaceSnmp").newInstance();
