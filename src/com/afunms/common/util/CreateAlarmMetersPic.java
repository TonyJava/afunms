...
/*******************************************************************************
 * Title:CreateAlarmMetersPic Description:生成告警关闭率表盘图片 Company dhcc
 * 
 * @author zhangcw Mar 28, 2011 3:02:48 PM
 *         *******************************************************
 */
public class CreateAlarmMetersPic {
	...
	public String createSimpleMeter(MeterModel mm) {
		...
		if (!mm.equals("")) {
			AngularMeter m = new AngularMeter(mm.getPicx(), mm.getPicy(), 0x80ff80, 0xffffff, -2);
			m.setBackground(mm.getBgColor());
			m.setMeter(mm.getMeterX(), mm.getMeterY(), mm.getMeterSize(), -135, 135);
			m.setScale(0, 100, 10, 5, 0);
			m.setLineWidth(0, 2, 1);
			m.setBgImage(ResourceCenter.getInstance().getSysPath() + "resource\\image\\dashboard2.png");
			...
			m.addText(mm.getTitleY(), mm.getTitleTop(), mm.getTitle(), "宋体", mm.getFontSize(), titlefontcolor, Chart.Center);
			m.addPointer(mm.getValue(), mm.getInPointerColor(), mm.getOutPointerColor()).setShape(Chart.ArrowPointer2);
			m.addText(mm.getValueY(), mm.getValueTop(), m.formatValue(mm.getValue(), "2"), "宋体", mm.getFontSize(), valuefontcolor, Chart.Center).setBackground(0xffffff, 0xffffff,
					-1);
