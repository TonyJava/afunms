...
public class CreateAmLinePic {
	public void addData(StringBuffer data, List<PolicyInterface> interfaceList, PolicyInterface vo, String title) {

		data.append("<graph title='" + vo.getInterfaceName() + ":" + vo.getClassName() + ":" + title + "' bullet='round_outlined' bullet_size='4'>");
	...
	public void addQueueData(StringBuffer data, List<QueueInfo> interfaceList, String title) {

		data.append("<graph title='" + title + "' bullet='round_outlined' bullet_size='4'>");
		for (int j = 0; j < interfaceList.size(); j++) {
	...
	public List getInterfaceList(String allipstr, String starttime, String totime) {
		List policyInterfaceList = null;
		PolicyInterfaceDao interfaceDao = null;
		try {
			interfaceDao = new PolicyInterfaceDao(allipstr);
			policyInterfaceList = interfaceDao.findByCondition(" where collecttime>='" + starttime + "' and collecttime<='" + totime
	 ...
	 * @return
	 */
	public List<List<PolicyInterface>> getSortClassData(List<PolicyInterface> list) {
		...
		if (list != null) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ((!vo.getInterfaceName().equals(tempInterface) || !vo.getPolicyName().equals(tempPolicy) || !vo.getClassName().equals(tempClass)) && i != 0) {
	...
	public HashMap<Integer, List<?>> getSortDetailData(List<AclDetail> list) {
		...
		if (list != null) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ((vo.getBaseId() != baseId || !vo.getName().equals(name)) && i != 0) {
			...
			for (int i = 0; i < alldata.size(); i++) {
				...
				if (interfaceList != null && interfaceList.size() > 0) {
					...
					for (int v = 0; v < 2; v++) {
						...
						data1.append("<graph title='" + vo.getInterfaceName() + ":" + vo.getClassName() + title + "' bullet='round_outlined' bullet_size='4'>");
						for (int j = 0; j < interfaceList.size(); j++) {
							vo = interfaceList.get(j);
							data1.append("<value xid='");
							data1.append(j);
		...
		if (alldata != null && alldata.size() > 0) {
			...
			for (int i = 0; i < alldata.size(); i++) {
				...
				if (interfaceList != null && interfaceList.size() > 0) {
					...
					data1.append("<graph title='" + vo.getName() + "' bullet='round_outlined' bullet_size='4'>");
					for (int j = 0; j < interfaceList.size(); j++) {
						vo = interfaceList.get(j);
						data1.append("<value xid='");
						data1.append(vo.getCollecttime());
