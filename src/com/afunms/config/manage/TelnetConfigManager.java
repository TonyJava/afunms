...
public class TelnetConfigManager extends BaseManager implements ManagerInterface {
	...
	public String delete() {
		...
		try {
			...
			List list = dao.findByCondition(" where id in(" + ids.substring(0, ids.length() - 1) + ")");
			dao = new GatherTelnetConfigDao();
			StringBuffer ips = new StringBuffer();
			if (list != null) {
				for (int i = 0; i < list.size(); i++) {
					GatherTelnetConfig config = (GatherTelnetConfig) list.get(i);
