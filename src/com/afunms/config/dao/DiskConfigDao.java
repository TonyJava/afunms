			...
			if (list != null && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {
					...
					if (!diskhash.containsKey(diskdata.getIpaddress() + ":" + diskdata.getSubentity())) {
						...
						diskconfig.setMonflag(1);// 0：不告警 1：告警，默认的情况是告警
						diskconfig.setSms(new Integer(1));// 0：不告警
						diskconfig.setReportflag(new Integer(0));// 0：不存在于报表
						diskconfig.setLimenvalue(new Integer(5));
						diskconfig.setLimenvalue1(new Integer(10));
						diskconfig.setLimenvalue2(new Integer(15));
						diskconfig.setSms1(new Integer(0));// 0：不告警
						diskconfig.setSms2(new Integer(0));// 0：不告警
						diskconfig.setSms3(new Integer(0));// 0：不告警
						try {
							conn = new DBManager();
							save(diskconfig);
						} catch (Exception ex) {
