...
public class BusinessSystemDao extends BaseDao implements DaoInterface {
	...
	public String getByKeySql(String key) {
		String sql = "where name like '%" + key + "%' or descr like '%" + key + "%' or contactname like '%" + key + "%' or contactemail like '%" + key + "%';";
		return sql;
	}
	...
	public List loadAll() {
		...
		try {
			rs = conn.executeQuery("select * from system_businesssystem order by id");
			while (rs.next()) {
	...
	public boolean save(BaseVo baseVo) {
		...
		sql.append("insert into system_businesssystem(id,name,descr,contactname,contactphone,contactemail)values(");
