...
public class VPNFileConfigDao extends BaseDao implements DaoInterface {
	...
	public boolean deleteFileByIps(String[] ips) {
		...
		try {
			for (int i = 0; i < ips.length; i++) {
				conn.addBatch("delete from nms_vpncfg where ipaddress='" + ips[i] + "'");
			}
			conn.executeBatch();
			result = true;
		} catch (Exception ex) {
	...
	public List getcfgListByIp(String ip) {
		List list = null;
		String sql = "select * from nms_vpncfg where ipaddress='" + ip + "' order by ipaddress,backup_time";
		list = this.findByCriteria(sql);
		return list;
	}
	...
	public List getDeviceByIps(List iplist, String type) {
		...
		String sql = "select max(id) 'id',timingId,ipaddress,fileName,content,max(backup_time) 'backup_time',file_size,bkp_type from nms_hua3vpncfg where ipaddress in('"
				+ sBuffer.toString() + ") and bkp_type='" + type + "' group by ipaddress";
		list = this.findByCriteria(sql);
		return list;
	}
	...
	public List listByPage(int curpage, String where, int perpage) {
		...
		try {
			rs = conn.executeQuery("select count(*) from (select max(id) 'id',ipaddress,fileName,content,backup_time,file_size,bkp_type from nms_hua3vpncfg group by ipaddress) t");
			if (rs.next()) {
				jspPage = new JspPage(perpage, curpage, rs.getInt(1));
			}

			rs = conn.executeQuery("select max(id) 'id',ipaddress,fileName,content,backup_time,file_size,bkp_type from nms_hua3vpncfg group by ipaddress;");
			int loop = 0;
			while (rs.next()) {
				loop++;
	...
	public List loadByIds(String[] ids) {
		...
		try {
			...
			sql.append("select * from nms_vpncfg where id in(" + split.substring(1) + ")");
	...
	public List loadByIp(String[] ipaddress) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select * from nms_vpncfg where ipaddress in('" + ips + "')");
		rs = conn.executeQuery(sql.toString());
		try {
			while (rs.next()) {
				list.add(loadFromRS(rs));
	...
	public List loadByIps(String[] ips) {
		...
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("select * from nms_vpncfg where ipaddress in(" + split.substring(0, split.length() - 1) + ")");
			rs = conn.executeQuery(sql.toString());
			while (rs.next()) {
				list.add(loadFromRS(rs));
			}
	...
	public boolean save(BaseVo vo) {
		...
		StringBuffer sql = new StringBuffer("insert into nms_vpncfg(id,timingId,ipaddress,fileName,content,backup_time,file_size,bkp_type) values(");
