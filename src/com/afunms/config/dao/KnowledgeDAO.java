...
public class KnowledgeDAO extends BaseDao implements DaoInterface {
	...
	public String findforevent(String eventid) {
		...
		int nodeid = 0;
		try {
			rs = conn.executeQuery("select * from system_eventlist where id='" + eventid + "';");
			while (rs.next()) {
				event_category = rs.getString("subtype");
				event_subentity = rs.getString("subentity");
				nodeid = rs.getInt("nodeid");
			}
			rs = conn.executeQuery("select * from topo_host_node where id='" + nodeid + "';");
			while (rs.next()) {
	...
	public List loadAll() {
		...
		try {
			rs = conn.executeQuery("select * from system_knowledge order by id");
			while (rs.next()) {
