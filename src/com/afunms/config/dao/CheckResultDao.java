...
public class CheckResultDao extends BaseDao implements DaoInterface {
	...
	public CheckResult getExtraCount(String id, String ip) {
		...
		try {
			String sql = "select count(ISVIOLATION) as countX ,CHECK_TIME from nms_comp_check_results  where ISVIOLATION=0 and STRATEGY_ID=" + id + " and ip='" + ip
					+ "' group by ISVIOLATION,check_time";
			rs = conn.executeQuery(sql);
			while (rs.next()) {
			...
			if (vector.size() > 0) {
				...
				sql = "select count(ISVIOLATION) as countX ,CHECK_TIME from nms_comp_check_results  where ISVIOLATION=1 and STRATEGY_ID=" + id + " and ip='" + ip
						+ "' group by ISVIOLATION,check_time";
	...
	public CheckResult getExtraCount1(String id, String ip) {
		...
		CheckResult result = new CheckResult();
		try {
			String sql = "select count(ISVIOLATION) as countX ,CHECK_TIME from nms_comp_check_results  where ISVIOLATION=1 and STRATEGY_ID=" + id + " and ip='" + ip
					+ "' group by ISVIOLATION,check_time";
	...
	public List<CompCheckResultModel> getReslutByIdAndIp(String id, String ip) {
		String sql = "select * from nms_comp_check_results  where STRATEGY_ID=" + id + " and ip='" + ip + "' order by ISVIOLATION ,VIOLATION_SEVERITY desc";
