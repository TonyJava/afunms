...
public class MQchannelDao extends BaseDao implements DaoInterface {
	...
	public List loadAll() {
		...
		try {
			rs = conn.executeQuery("select t1.*,t2.NAME as mqname from nms_mqchannel t1,nms_mqconfig t2 " + "where t1.IPADDRESS=t2.IPADDRESS " + "order by t1.ipaddress,t1.name");
			while (rs.next()) {
	...
	public List loadAllByBID(Vector bids) {
		...
		List list = new ArrayList();
		try {
			rs = conn.executeQuery("select t1.*,t2.NAME as mqname from nms_mqchannel t1,nms_mqconfig t2 " + "where t1.ipaddress=t2.ipaddress " + wstr
	...
	public List loadById(String id) {
		List list = new ArrayList();
		try {
			rs = conn.executeQuery("select t1.*,t2.NAME as mqname from nms_mqchannel t1,nms_mqconfig t2 " + "where t1.IPADDRESS=t2.IPADDRESS  and t2.id='" + id + "' "
					+ "order by t1.ipaddress,t1.name");
			while (rs.next()) {
				list.add(loadFromRS(rs));
			}
