...
public class NodeUtil {
	...
	public static synchronized boolean isOnlyCollectPing(Host host) {
		...
		boolean flag = false;
		if (host.getCollecttype() == SystemConstant.COLLECTTYPE_PING || host.getCollecttype() == SystemConstant.COLLECTTYPE_TELNETCONNECT
	...
	public NodeDTO creatNodeDTOByNode(HostNode hostNode) {
		...
		if (hostNode.getCategory() == 4) {
			...
			if (hostNode.getSysOid() != null && hostNode.getSysOid().trim().length() > 0) {
				...
				if (hostNode.getSysOid().startsWith("1.3.6.1.4.1.476.1.42")) {// 艾默生空调
			...
			if (hostNode.getSysOid() != null && hostNode.getSysOid().trim().length() > 0) {
				...
				if (hostNode.getSysOid().startsWith("1.3.6.1.4.1.13400.2.1")) {// 艾默生UPS
				...
				if (nodestr.indexOf(nodeTag) != -1 && !"bus".equalsIgnoreCase(nodeTag)) {
					...
					if (cate != null && cate.length > 0) {
						for (int j = 0; j < cate.length; j++) {
							if (cate[j] != null && !"null".equalsIgnoreCase(cate[j]) && !"".equalsIgnoreCase(cate[j]) && node.getCategory() == Integer.parseInt(cate[j])
									&& node.isManaged()) {
	...
	public String getDB_OracleSql() {
		...
		DBTypeDao dBTypeDao = new DBTypeDao();
		try {
			dBTypeVo = dBTypeDao.findByDbtype("oracle");
		} catch (Exception e) {
