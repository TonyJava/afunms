...
public class NodeGatherIndicatorsDao extends BaseDao implements DaoInterface {
	...
	public List findByNodeIdAndTypeAndSubtype(String nodeid, String type, String subtype, String isDefault) {
		if (subtype == null || "null".equalsIgnoreCase(subtype) || subtype.trim().length() == 0) {
			...
			if ("-1".equalsIgnoreCase(isDefault)) {
				...
				SysLogger.info("select * from nms_gather_indicators_node where nodeid ='" + nodeid + "' and type='" + type + "' and subtype='" + subtype + "' and isDefault='"
						+ isDefault + "'");
	...
	public List getByInterval( int enabled, String type, String subtype) {
		String sql = "select * from nms_gather_indicators_node where  isCollection='" + enabled
				+ "' and type='" + type + "' and subtype='" + subtype + "'";
		List list = findByCriteria(sql);
		return list;
	}
	...
	public List getByIntervalAndType(String nodeid, String interval, String unit, int enabled, String type) {
		String sql = "select * from nms_gather_indicators_node where nodeid ='" + nodeid + "'and poll_interval='" + interval + "' and interval_unit='" + unit
				+ "' and isCollection='" + enabled + "' and type='" + type + "'";
		List list = findByCriteria(sql);
		return list;
	}
	...
	public boolean save(BaseVo vo) {
		...
		sql
				.append("insert into nms_gather_indicators_node(nodeid, name, type, subtype, alias, description, category, isDefault, isCollection, poll_interval, interval_unit, classpath)values('");
