						...
						if (addindilist != null && addindilist.size() > 0) {
							try {
								for (int k = 0; k < addindilist.size(); k++) {
									...
									if (nodeindihash
											.containsKey(_nodeGatherIndicators.getName() + ":" + _nodeGatherIndicators.getType() + ":" + _nodeGatherIndicators.getSubtype())) {
										// 若存在,则修改
										NodeGatherIndicators nodeGatherIndicators_update = (NodeGatherIndicators) nodeindihash.get(_nodeGatherIndicators.getName() + ":"
												+ _nodeGatherIndicators.getType() + ":" + _nodeGatherIndicators.getSubtype());
		...
		if (nodeDTOlist != null) {
			for (int i = 0; i < nodeDTOlist.size(); i++) {
				...
				if ("-1".equals(nodeid) || nodeDTO.getNodeid().equals(nodeid)) {
					List<NodeGatherIndicators> list2 = nodeGatherIndicatorsUtil
							.getGatherIndicatorsForNode(String.valueOf(nodeDTO.getId()), nodeDTO.getType(), nodeDTO.getSubtype());
							...
							try {
								for (int k = 0; k < addindilist.size(); k++) {
									...
									if (nodeindihash.containsKey(gatherIndicators.getName() + ":" + gatherIndicators.getType() + ":" + gatherIndicators.getSubtype())) {
										// 若存在,则修改
										NodeGatherIndicators nodeGatherIndicators_update = (NodeGatherIndicators) nodeindihash.get(gatherIndicators.getName() + ":"
												+ gatherIndicators.getType() + ":" + gatherIndicators.getSubtype());
