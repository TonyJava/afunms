...
public class PortScanDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		sb.append("insert into nms_portscan_config(ipaddress,port,portName,desc,type,timeout,status,isScanned,scantime)values(");
	...
	public boolean saveBatch(List list) {
		...
		try {
			for (int i = 0; i < list.size(); i++) {
				...
				StringBuffer sb = new StringBuffer(200);
				sb.append("insert into nms_portscan_config(ipaddress,port,portName,description,type,timeout,status,isScanned,scantime)values('");
				sb.append(portConfig.getIpaddress());
				sb.append("','");
				sb.append(portConfig.getPort());
				sb.append("','");
	...
	public boolean deleteByIpaddress(List ipaddresslist) {
		...
		try {
			for (int i = 0; i < ipaddresslist.size(); i++) {
				...
				String sql = "delete from nms_portscan_config where ipaddress='" + ipaddress + "'";
				conn.addBatch(sql);
			}
			conn.executeBatch();
			result = true;
	...
	public List findByIpaddress(String ipaddress) {
		String sql = "select * from nms_portscan_config where ipaddress='" + ipaddress + "'";
		return findByCriteria(sql);
	}

	public Hashtable getPortStatusByIp(String ip, String starttime, String endtime) {
		Hashtable hash = new Hashtable();
		if (!starttime.equals("") && !endtime.equals("")) {
			List list1 = new ArrayList();
			String allip = ip;
			String sql = "select a.THEVALUE,a.COLLECTTIME from portstatus" + allip + " a where  a.COLLECTTIME >= '" + starttime + "' and  a.COLLECTTIME <= '" + endtime
					+ "' order by id";
	...
	public String getHourData(String allip, String index, String starttime, String totime) {
		int value = 0;
		...
		try {
			int temp = 0;
			boolean flag = false;
			rs = conn.executeQuery("select HOUR(COLLECTTIME)  as h,THEVALUE as value from portstatus" + allip + " where  COLLECTTIME >= '" + starttime + "' and  COLLECTTIME <= '"
					+ totime + "'  and SUBENTITY='" + index + "' group by h");
	...
	public Vector getCurrentStatus(String ip) {
		...
		try {
			if ("0".equals(runmodel)) {
				...
				String[] netInterfaceItem = { "index", "ifDescr", "ifSpeed", "ifOperStatus", "OutBandwidthUtilHdxPerc", "InBandwidthUtilHdxPerc", "OutBandwidthUtilHdx",
						"InBandwidthUtilHdx" };
				...
				// 采集与访问是分离模式
				rs = conn
						.executeQuery("select p.thevalue value , s.name n,p.collecttime from nms_interface_data_temp  p ,system_portconfig s where p.ip=s.ipaddress and p.sindex=s.portindex and s.sms=1 and  p.subentity='ifOperStatus' and p.ip='"
								+ ip + "'");
