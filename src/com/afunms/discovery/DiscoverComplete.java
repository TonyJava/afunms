			...
			try {
				...
				while (sub_ite.hasNext()) {
					try {
						...
						logger.info("运行: 解析 CDP link: nodeid=" + curCdpNodeId + " ifindex=" + cdpIfIndex + " nodeparentid=" + targetCdpNodeId + " parentifindex="
								+ cdpDestIfindex);
					...
					while (sub_ite.hasNext()) {
						try {
							...
							if ("10.10.0.6".equalsIgnoreCase(host.getIpAddress()) || "10.10.0.38".equalsIgnoreCase(host.getIpAddress())) {
								logger.info("--------------------------------");
								logger.info(host.getIpAddress() + " endndpMac:" + endndpMac + " endndpDescr:" + endndpDescr + "  存在这样的NDP数据");
								logger.info("--------------------------------");
							}
							Host endNode = getNodeFromMacIdentifierOfNdpNode(endndpMac);
							if (endNode == null) {
								logger.info("--------------------------------");
								logger.info(host.getIpAddress() + " endndpMac:" + endndpMac + " endndpDescr:" + endndpDescr + "  找不到MAC地址");
								logger.info("--------------------------------");
							...
							if (endIfEntity == null) {
								if ("10.10.0.6".equalsIgnoreCase(host.getIpAddress()) || "10.10.0.38".equalsIgnoreCase(host.getIpAddress())) {
									logger.info("--------------------------------");
									logger.info(endNode.getIpAddress() + " endndpMac:" + endndpMac + " endndpDescr:" + endndpDescr + " 找不到端口@@@ :");
									logger.info("--------------------------------");
								}
							...
							logger.info("运行: 解析 NDP link: nodeid=" + host.getId() + " ifindex=" + startIfEntity.getIndex() + " nodeparentid=" + endNode.getId()
