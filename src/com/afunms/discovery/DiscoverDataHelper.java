		...
		try {
			...
			while (rs.next()) {
				try {
					...
					if (discoverstatus > 0) {
						xmlOpr.addNode("net" + rsId, rsCategory, NodeHelper.getLostImage(rsCategory), rsIpAddress, rsAlias, x, y);// yangjun
																																	// xiugai
					} else {
						xmlOpr.addNode("net" + rsId, rsCategory, NodeHelper.getTopoImage(rsCategory), rsIpAddress, rsAlias, x, y);// yangjun
																																	// xiugai
					}
				} catch (Exception e) {
					e.printStackTrace();
			...
			while (rs.next()) {
				try {
					xmlOpr.addLine(rs.getString("link_name"), rs.getString("id"), "net" + rs.getString("start_id"), "net" + rs.getString("end_id"));// yangjun
																																					// xiugai
				} catch (Exception e) {
			...
			while (rs.next()) {
				try {
					xmlOpr.addAssistantLine(rs.getString("link_name"), rs.getString("id"), "net" + rs.getString("start_id"), "net" + rs.getString("end_id"));// yangjun
																																								// xiugai
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
	...
	public void DB2ServerXml() {
		...
		for (int i = 0; i < list.size(); i++) {
			try {
				...
				xmlOpr.addNode("net" + nodeId, 4, img, host.getIpAddress(), host.getAlias(), x, y);// yangjun
																									// xiugai
			} catch (Exception e) {
				e.printStackTrace();
			}
