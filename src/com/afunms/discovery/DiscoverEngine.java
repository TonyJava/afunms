...
public class DiscoverEngine {
	...
	public void addHost(Host newNode, Link link) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			Host tmpNode = (Host) hostList.get(i);
			// SysLogger.info("&&&&&#####"+newNode.getIpAddress()+"----BRIDGE
			// "+newNode.getBridgeAddress()+"======="+tmpNode.getIpAddress()+"===BRIDGE
			// "+tmpNode.getBridgeAddress());
			if (tmpNode.getBridgeAddress() != null && tmpNode.getBridgeAddress().equalsIgnoreCase(newNode.getBridgeAddress())) {
				// SysLogger.info("&&&&&******"+node.getIpAddress()+"----别名
				// "+node_ipalias.get(m)+"======="+tmpNode.getIpAddress()+"===temp_ip:"+temp_ip);
				SysLogger.info("类型为" + newNode.getCategory() + "的设备" + newNode.getIpAddress() + "已经存在");
				// exist = true;
				flag = 1;
				break;
	...
	public synchronized void addLinks(List temporaryLinks) {
		...
		for (int i = 0; i < temporaryLinks.size(); i++) {
			...
			for (int j = 0; j < linkList.size(); j++) {
				...
				if (tempLink.getStartId() == link.getStartId() && tempLink.getEndId() == link.getEndId()) {
	...
	private synchronized void addHost(Host node) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			Host tmpNode = (Host) hostList.get(i);
			// SysLogger.info("&&**"+node.getIpAddress()+"--BRIDGE
			// "+node.getBridgeAddress()+" "+tmpNode.getIpAddress()+" BRIDGE
			// "+tmpNode.getBridgeAddress());
	...
	private boolean isHostExist(Host node) {
		...
		for (int i = 0; i < hostList.size(); i++) {
			...
			// "+tmpNode.getIpAddress()+"===Bridge:"+tmpNode.getBridgeAddress());
						...
						if (node_ipalias != null && node_ipalias.size() > 0) {
							for (int m = 0; m < node_ipalias.size(); m++) {
								if (temp_ip.equalsIgnoreCase((String) node_ipalias.get(m))) {
									// SysLogger.info("@*"+node.getIpAddress()+"-别名
									// "+node_ipalias.get(m)+"="+tmpNode.getIpAddress()+"=temp_ip:"+temp_ip+"
									// nodeBRIDGE:"+node.getBridgeAddress()+"
									// tmpNodeBRIDGE:"+tmpNode.getBridgeAddress());
									// 同时判断BRIDGE
	...
	public boolean isDiscovered() {
		 ...
		 * //Runnable test = (Runnable)threadList.get(0); if(this.discoverdcount ==
		 * this.threads){ //threadPool.join(); //threadPool.interrupt();
	...
	public void dealLink(Host node, Link link) {
		...
		if (!hostidHash.containsKey(node.getId() + "") || !hostidHash.containsKey(link.getStartId() + ""))
			return;
		// hostList
	...
	public synchronized void checkAssistantLink(List linkList, Link link, int linktype) {
		if (linkList != null && linkList.size() > 0) {
			for (int i = 0; i < linkList.size(); i++) {
				...
				if ((tempLink.getStartId() == link.getStartId() && tempLink.getEndId() == link.getEndId())
						|| (tempLink.getStartId() == link.getEndId() && tempLink.getEndId() == link.getStartId())) {
					if ((link.getStartIp().equalsIgnoreCase(tempLink.getStartIp()) && link.getStartIndex().equalsIgnoreCase(tempLink.getStartIndex()))
							|| (link.getEndIp().equalsIgnoreCase(tempLink.getEndIp()) && link.getEndIndex().equalsIgnoreCase(tempLink.getEndIndex())))
						continue;
	...
	/*
	 * 判断两个连接是否相同 @param (Link)sourceLink @param (Link)destLink
	 */
	public boolean isSameLink(Link sourceLink, Link destLink) {
		if ((destLink.getStartIp().equalsIgnoreCase(sourceLink.getStartIp()) && destLink.getStartIndex().equalsIgnoreCase(sourceLink.getStartIndex())
				&& destLink.getEndIp().equalsIgnoreCase(sourceLink.getEndIp()) && destLink.getEndIndex().equalsIgnoreCase(sourceLink.getEndIndex()))
				|| (destLink.getStartIp().equalsIgnoreCase(sourceLink.getEndIp()) && destLink.getStartIndex().equalsIgnoreCase(sourceLink.getEndIndex())
						&& destLink.getEndIp().equalsIgnoreCase(sourceLink.getStartIp()) && destLink.getEndIndex().equalsIgnoreCase(sourceLink.getStartIndex())))
			return true;
		return false;
	}
