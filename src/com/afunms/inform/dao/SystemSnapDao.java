...
public class SystemSnapDao {
	...
	private String getNetworkStatus() {
		...
		for (int i = 0; i < netList.size(); i++) {
			if (!(netList.get(i) instanceof Host)) {
				continue;
			}
			Host host = (Host) netList.get(i);
			if (host.getCategory() > 3) {
				continue;
			}
	...
	private String getVirusStatus() {
		...
		ResultSet rs = null;
		try {
			rs = conn.executeQuery("select machine_ip,count(virus_file) files from nms_symantec" + " where substring(begintime,1,10)='" + SysUtil.getCurrentDate()
					+ "' group by machine_ip" + " order by files desc");
			while (rs.next()) {
