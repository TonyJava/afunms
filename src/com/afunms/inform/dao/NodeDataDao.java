...
public class NodeDataDao extends BaseDao {
	...
	public String[][] diskStat(String day, int nodeId) {
		String[][] result = null;
		try {
			String sql1 = "select entity from topo_node_multi_data where moid='001003'" + " and node_id=" + nodeId + " and substring(log_time,1,7)='" + day.substring(0, 7)
					+ "' group by entity ";

			rs = conn.executeQuery("select count(*) from (" + sql1 + ") t");
			int entityTotal = 0;
			if (rs.next()) {
				entityTotal = rs.getInt(1);
			}
			if (entityTotal == 0) {
				return null;
			}
			...
			String sql2 = "select entity,substring(log_time,9,3) logday,round(avg(percentage),1) avgvalue" + " from topo_node_multi_data where moid='001003' and node_id=" + nodeId
					+ " and " + " substring(log_time,1,7)='" + day.substring(0, 7) + "' group by node_id,entity,substring(log_time,1,10)";
	...
	public String[][] multiStat(String day, int nodeId, String moid) {
		String[][] result = null;
		try {
			String sql1 = "select entity from topo_node_multi_data where moid='" + moid + "'" + " and node_id=" + nodeId + " and substring(log_time,1,10)='" + day
					+ "' group by entity ";
			...
			String sql2 = "select entity,substring(log_time,11,3) loghour,round(avg(percentage),1) avgvalue" + " from topo_node_multi_data where moid='" + moid + "' and node_id="
					+ nodeId + " and " + " substring(log_time,1,10)='" + day + "' group by node_id,entity,substring(log_time,1,13)";
	...
	public String[][] portUtilStat(int nodeId, String day, String index) {
		...
		try {
			...
			String sqlIn = "select round(AVG(percentage),1) value1,SUBSTRING(log_time,12,2) value2 from topo_interface_data" + " where ((node_id=" + nodeId + ") and (entity='"
					+ index + "') and (SUBSTRING(log_time,1,10)='" + day + "') and (moid='003002'))" + " group by value2 order by log_time";
			String sqlOut = "select round(AVG(percentage),1) value1,SUBSTRING(log_time,12,2) value2 from topo_interface_data" + " where ((node_id=" + nodeId + ") and (entity='"
					+ index + "') and (SUBSTRING(log_time,1,10)='" + day + "') and (moid='003003'))" + " group by value2 order by log_time";
	...
	public float[] singleStat(String day, int nodeId, String moid) {
		String sql = "select substring(log_time,11,3) loghour,round(avg(value),1) avgvalue from" + " topo_node_single_data where substring(log_time,1,10)='" + day + " 'and moid='"
	...
	public String[][] trafficStat(String day, int nodeId, String moid, boolean isValue) {
		String[][] result = null;
		try {
			String sql1 = "select entity from topo_interface_data where moid='" + moid + "'" + " and node_id=" + nodeId + " and substring(log_time,1,10)='" + day
					+ "' group by entity ";
			rs = conn.executeQuery("select count(*) from (" + sql1 + ") t");
			int entityTotal = 0;
			if (rs.next()) {
			...
			String sql2 = "select entity,substring(log_time,11,3) loghour," + temp + " from topo_interface_data where moid='" + moid + "' and node_id=" + nodeId + " and "
