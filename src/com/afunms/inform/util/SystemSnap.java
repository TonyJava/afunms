...
public class SystemSnap {
	public static int getAirConditionStatus(String nodeid) {
		...
		if (nodeList != null && nodeList.size() > 0) {
			for (int i = 0; i < nodeList.size(); i++) {
				...
				if (node != null) {
					if (node.getAlarmlevel() > status) {
						status = node.getAlarmlevel();
					}
				}
			}
		}
	 ...
	 * 数据库的状态
	 * 
	 * @auth 胡可磊---wxy update date 2010-01-11
	 */
	...
	public static int getFirewallStatus(String bids) {
		...
		if (nodeList != null && nodeList.size() > 0) {
			for (int i = 0; i < nodeList.size(); i++) {
				CheckEvent node = (CheckEvent) nodeList.get(i);
				if (node != null) {
					Node node_ = PollingEngine.getInstance().getNodeByID(Integer.parseInt(node.getNodeid()));
	...
	public static int getFtpStatus(String bids) {
		...
		if (nodeList != null && nodeList.size() > 0) {
			for (int i = 0; i < nodeList.size(); i++) {
				CheckEvent node = (CheckEvent) nodeList.get(i);
				if (node != null) {
					Node node_ = PollingEngine.getInstance().getFtpByID(Integer.parseInt(node.getNodeid()));
					if (node_ != null) {
						if (node.getAlarmlevel() > status) {
							status = node.getAlarmlevel();
						}
