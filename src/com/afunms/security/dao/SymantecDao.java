...
public class SymantecDao extends BaseDao {
	...
	public void finish(SymantecLog slvo) {
		try {
			String sql = null;
			sql = "update nms_symantec_log set log_file='" + slvo.getLogFile() + "',log_row=" + slvo.getLogRow() + ",log_time='" + SysUtil.getCurrentTime() + "',info='"
					+ slvo.getInfo() + "' where id=" + slvo.getId();
	...
	public List machineStat(String begindate, int topn) {
		...
		try {
			// 先计算总共有多少种病毒
			String sql = "select machine_ip content,count(machine_ip) total from nms_symantec where " + " substring(begintime,1,10)='" + begindate + "' group by machine_ip";
	...
	public List selectByContent(Symantec vo) {
		...
		try {
			...
			if (vo.getMachineIp() != null) {
				sql = "select * from nms_symantec where machine_ip='" + vo.getMachineIp() + "' and substring(begintime,1,10)='" + vo.getBeginDate() + "'";
			} else {
				sql = "select * from nms_symantec where virus='" + vo.getVirus() + "' and substring(begintime,1,10)='" + vo.getBeginDate() + "'";
			}
	...
	public List virusStat(String begindate, int topn) {
		...
		try {
			// 先计算总共有多少种病毒
			String sql = "select virus content,count(virus) total from nms_symantec where " + " substring(begintime,1,10)='" + begindate + "' group by virus";
			rs = conn.executeQuery("select count(*) from (" + sql + ") t1");
