...
public class DetailService {
	...
	/**
	 * 用于解析 详细信息的 tab 页 该方法应由子类来调用
	 * 
	 * @param file
	 *            tab.xml 文件 格式为 应用路径下 /.../...
	 * @return
	 */
	protected List<DetailTabRemote> praseDetailTabXML(String file) {
		...
		try {
			...
			while (tabIterator.hasNext()) {
				...
				// 标签页 name
				String tabName = tabElement.getAttributeValue("title");
				// 标签页 action
				String tabAction = tabElement.getAttributeValue("action");
				...
				while (titleTrIterator.hasNext()) {
					Element titleTrElement = titleTrIterator.next();
					...
					while (titleTr_TitleIterator.hasNext()) {
						// 循环每一个 title-tr
						Element titleElement = titleTr_TitleIterator.next();
