...
public class AccreditDao extends BaseDao {
	...
	public List loadRoleMenu(int role_id) {
		...
		try {
			if (role_id == 0) {
				sql = "select b.id,b.title,c.operate from system_role a,system_menu b,system_role_menu c " + "where a.id=c.role_id and b.id=c.menu_id and a.id=0 order by b.id";
			} else {
				sql = "select b.id,b.title,c.operate from system_role a,system_menu b," + "system_role_menu c where a.id=c.role_id and b.id=c.menu_id and a.id=" + role_id
						+ " and c.operate>0 order by b.id";
			}
			rs = conn.executeQuery(sql);
			while (rs.next()) {
