...
/**
 * 此Functiondao类为对表"nms_func"操作 在使用此类时 在最后一定要调用close方法来关闭connection释放资源
 * 
 * @author hkmw
 * 
 */
...
public class FunctionDao extends BaseDao implements DaoInterface {
	...
	public boolean save(BaseVo vo) {
		...
		boolean result = false;
		try {
			String sql = "insert into nms_func(id,func_desc,ch_desc,level_desc,father_node,url,img_url," + "is_current_window,width,height,clientX,clientY)values('" + getNextID()
					+ "','" + function.getFunc_desc() + "','" + function.getCh_desc() + "','" + function.getLevel_desc() + "','" + function.getFather_node() + "','"
					+ function.getUrl() + "','" + function.getImg_url() + "','" + function.getIsCurrentWindow() + "','" + function.getWidth() + "','" + function.getHeight()
	...
	public boolean update(BaseVo vo) {
		...
		boolean result = false;
		try {
			String sql = "update nms_func set func_desc='" + function.getFunc_desc() + "',ch_desc='" + function.getCh_desc() + "',level_desc='" + function.getLevel_desc()
					+ "',father_node='" + function.getFather_node() + "',url='" + function.getUrl() + "',img_url='" + function.getImg_url() + "',is_current_window='"
					+ function.getIsCurrentWindow() + "',width='" + function.getWidth() + "',height='" + function.getHeight() + "',clientX='" + function.getClientX()
					+ "',clientY='" + function.getClientY() + "'" + "where id ='" + function.getId() + "'";
			conn.executeUpdate(sql);
			conn.commit();
			result = true;
		} catch (Exception e) {
