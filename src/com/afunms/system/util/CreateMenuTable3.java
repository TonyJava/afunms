...
public class CreateMenuTable3 {
	...
	private StringBuffer createThirdMenuTable(List<Function> menuThird_list, List<Function> menu_list) {
		...
		for (int i = 0; i < menuThird_list.size(); i++) {
			table.append("<li><table><tr><td style=\"height:22px;width:16px;padding-top:3px;\"><img src=" + rootPath + menuThird_list.get(i).getImg_url()
			...
			if (menuThird_list.get(i).getIsCurrentWindow() == 1) {
				table.append("<td style=\"padding-left:5px;padding-top:3px;\"><a href=\"javascript:void(null)\"" + "onClick='window.open(\"" + rootPath
						+ menuThird_list.get(i).getUrl() + "\",\"fullScreenWindow\", \"toolbar=no,height=\"+" + height + " + \",\"+\"width=\" + " + width
						+ "+ \",scrollbars=yes,resizable=yes\"+\"screenX=0,screenY=0\")'>" + menuThird_list.get(i).getCh_desc() + "</a></td>");
			} else {
				table.append("<td style=\"padding-left:5px;padding-top:3px;\"><a href=" + rootPath + menuThird_list.get(i).getUrl() + ">" + menuThird_list.get(i).getCh_desc()
						+ "</a></td>");
			}
