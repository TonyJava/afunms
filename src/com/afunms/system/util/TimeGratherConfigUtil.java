...
public class TimeGratherConfigUtil {
	 ...
	 * @return
	 */
	public List<TimeGratherConfig> getTimeGratherConfig(String objectId, String objectType) {
		validate(objectId, objectType);
		TimeGratherConfigDao dao = new TimeGratherConfigDao();
	...
	public int isBetween(List timeGratherConfigList) {
		...
		for (TimeGratherConfig tg : timeGratherConfig) {
			tg.setHourAndMin();
			if (hour >= Integer.parseInt(tg.getStartHour()) && hour < Integer.parseInt(tg.getEndHour())) {
				if (min >= Integer.parseInt(tg.getStartMin())) {
					return 1;
				}
			} else if (hour == Integer.parseInt(tg.getEndHour()) && tg.getStartHour().equals(tg.getEndHour())) {
				if (min >= Integer.parseInt(tg.getStartMin()) && min < Integer.parseInt(tg.getEndMin())) {
					return 1;
				}
	...
	public int isBetween(String objectId, String objectType) {
		TimeGratherConfigDao dao = new TimeGratherConfigDao();
		List<TimeGratherConfig> timeGratherConfig = dao.findTimeGratherConfigByObject(objectId, objectType);
	 ...
	 * @return
	 */
	public boolean saveTimeGratherConfigList(HttpServletRequest request, String objectId, String objectType) {
		if (request == null) {
		...
		TimeGratherConfigDao dao = new TimeGratherConfigDao();
		try {
			result = dao.saveTimeGratherConfigList(objectId, objectType, timeGratherConfigList);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			dao.close();
