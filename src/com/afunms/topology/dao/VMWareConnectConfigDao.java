	...
	public Hashtable getPingDataById(String ip, Integer id, String starttime, String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue,a.collecttime from ping" + ip + " a where " + "(a.collecttime >= '" + starttime + "' and  a.collecttime <= '" + endtime
						+ "') and subentity = 'ConnectUtilization' order by id";
				;
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "select a.thevalue from ping" + ip + " a where " + " (a.collecttime >= " + "to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS')" + " and  a.collecttime <= "
						+ "to_date('" + endtime + "','YYYY-MM-DD HH24:MI:SS')" + ") and subentity = 'ConnectUtilization' order by id";
				;
			}
			int i = 0;
			double curPing = 0;
