...
public class DiscoverConfigDao extends BaseDao implements DaoInterface {
	...
	public void saveCore(String ip, String community) {
		try {
			...
			if (rs.next()) // 如果有则更新
				...
				conn.executeUpdate("insert into topo_discover_config(id,address,community,flag)values(" + getNextID() + ",'" + ip + "','" + community + "','core')");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void saveOtherCore(String ip, String community) {
		try {
			...
			if (rs.next()) // 如果没有则更新
				...
				conn.executeUpdate("insert into topo_discover_config(id,address,community,flag)values(" + getNextID() + ",'" + ip + "','" + community + "','othercore')");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/*
	 * 清除拓扑发现表，以便重新发现 目前需要修改成对上次发现的设备进行保存,以便对新发现的设备进行比较
	 */
	...
	static public String ip2String(long ip) {
		String result = "";
		try {
			result = "" + (int) (ip >> 24) + "." + (((int) (ip >> 16)) & 0xff) + "." + (((int) (ip >> 8)) & 0xff) + "." + (((int) ip) & 0xff);
		} catch (Exception e) {
			e.printStackTrace();
			result = "";
		}
