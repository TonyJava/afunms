...
public class ChartGraphics {
	...
	public void graphicsGeneration(String flag) {
		...
		try {
			fis = new FileInputStream(ResourceCenter.getInstance().getSysPath() + "resource/xml/network.jsp");
			...
			for (int i = 0; i < list.size(); i++) {
				...
				y = y.replace("px", "");
				if (imageWidth < Integer.parseInt(x))
					imageWidth = Integer.parseInt(x);
				if (imageHeight < Integer.parseInt(y))
					imageHeight = Integer.parseInt(y);
