...
public class IpMacManager extends BaseManager implements ManagerInterface {
						...
						if (id != null && id.length() > 0) {
							...
							if (paras != null && paras.length == 4) {
								...
								if (!allmacbase.containsKey(relateipaddr + ":" + macipaddress + ":" + mac)) {
									...
									vo.setMac(mac);
									vo.setIfband(1);// 0:设为基准表 1:IP-MAC 2;端口-MAC
													// 3:IP-端口-MAC
									vo.setIfsms("0");// 不发信息
									vo.setIftel("0");// 不打电话
									vo.setIfemail("0");// 不发邮件
	...
	public String ipmacbaseInsertSQL(IpMacBase ipmacband) {
		...
		sql = sql + ipmacband.getMac() + "','" + time + "','" + ipmacband.getIfband() + "','" + ipmacband.getIfsms() + "','" + ipmacband.getIftel() + "','"
				+ ipmacband.getIfemail() + "'," + ipmacband.getEmployee_id() + ")";
		return sql;
	}
