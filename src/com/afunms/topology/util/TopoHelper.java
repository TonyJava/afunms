...
public class TopoHelper extends BaseManager implements ManagerInterface {
	 ...
	 *            ip,,读共同体,写共同体, 增加一台主机
	 */
	public int addHost(String assetid, String location, String ipAddress, String alias, int snmpversion, String community, String writecommunity, int transfer, int category, int ostype, int collecttype, String bid, String sendmobiles, String sendemail, String sendphone) {
		HostNodeDao tmpDao = new HostNodeDao();
	 ...
	 *            ip,,读共同体,写共同体, 增加一台主机
	 */
	public int addHost(String assetid, String location, String ipAddress, String alias, int snmpversion, String community, String writecommunity, int transfer, int category, int ostype, int collecttype, String bid, String sendmobiles, String sendemail, String sendphone, boolean ismanaged) {
		HostNodeDao tmpDao = new HostNodeDao();
		List tmpList = tmpDao.findBynode("ip_address", ipAddress);
		if (tmpList.size() > 0)
			return -1; // IP已经存在
		...
		try // 以下把新增加的主机的数据加入数据库
		{
			...
			if (collecttype == SystemConstant.COLLECTTYPE_SNMP) {
				...
				if (host.getCategory() == 4) {
					host.setBridgeAddress(SnmpUtil.getInstance().getHostBridgeAddress(ipAddress, community));
				} else {
				...
				if (host.getSysOid().startsWith("1.3.6.1.4.1.1588.2")) {// 博科的网络设备
					host.setIfEntityList(SnmpUtil.getInstance().getIfEntityList_brocade(ipAddress, community, category));
				} else {
					host.setIfEntityList(SnmpUtil.getInstance().getIfEntityList(ipAddress, community, category));
				}
	 ...
	 * @date 2010-05-18
	 */
	public int addHost(String assetid, String location, String ipAddress, String alias, int snmpversion, String community, String writecommunity, int transfer, int category, int ostype, int collecttype, String bid, String sendmobiles, String sendemail, String sendphone, int supperid, boolean managed, int securityLevel, String securityName, int v3_ap, String authPassPhrase, int v3_privacy, String privacyPassPhrase) {
		// emc location待别的参数
		...
		if (ipaliasHash != null && ipaliasHash.size() > 0) {
			if (ipaliasHash.containsKey(ipAddress))
				return -1;
		}
		...
		if (collecttype == SystemConstant.COLLECTTYPE_SNMP) {
			...
			if (snmpversion == 0 || snmpversion == 1) {
				...
				sysOID = SnmpUtil.getInstance().getSysOid(ipAddress, snmpversion, community, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase);
			}
		...
		try // 以下把新增加的主机的数据加入数据库
		{
			...
			if (collecttype == SystemConstant.COLLECTTYPE_SNMP) {
				...
				if (snmpversion == 0 || snmpversion == 1) {
					...
					// SNMP V3
					host.setMac(SnmpUtils.getMacAddress(ipAddress, community, snmpversion, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase, 3, 1000 * 30));
					if (host.getCategory() == 4) {
						host.setBridgeAddress(SnmpUtils.getHostBridgeAddress(ipAddress, community, snmpversion, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase, 3, 1000 * 30));
					} else {
						host.setBridgeAddress(SnmpUtil.getInstance().getBridgeAddress(ipAddress, snmpversion, community, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase));
					}
					if (host.getSysOid().startsWith("1.3.6.1.4.1.1588.2")) {// 博科的网络设备
						host.setIfEntityList(SnmpUtils.getIfEntityList_brocade(ipAddress, snmpversion, community, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase, category));
					} else {
						host.setIfEntityList(SnmpUtils.getIfEntityList(ipAddress, snmpversion, community, securityLevel, securityName, v3_ap, authPassPhrase, v3_privacy, privacyPassPhrase, category));
					}
				}
			...
			} else if (collecttype == SystemConstant.COLLECTTYPE_PING || collecttype == SystemConstant.COLLECTTYPE_TELNETCONNECT || collecttype == SystemConstant.COLLECTTYPE_SSHCONNECT || collecttype == SystemConstant.COLLECTTYPE_DATAINTERFACE) {
