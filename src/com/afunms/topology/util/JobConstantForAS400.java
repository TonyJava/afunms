...
public class JobConstantForAS400 {
	 ...
	 * 
	 * ASJ: 自动启动 BCH: 批量 BCI: 批量即时 EVK: 由程序启动请求启动 INT: 交互 o M36: 高级36服务器作业 MRT:
	 * 多个请求终端 PJ: 启动前作业 PDJ: 打印驱动作业 RDR: 读取程序 SBS: 子系统监视器 SYS: 系统 WTR: 写入程序状态
	 * 
	 * 上述类型为 IBM PCOMM 中所定义的类型
	 */

	/**
	 * Constant indicating that a job was disconnected form a work station
	 * display.
	 */
	public static final String ACTIVE_JOB_STATUS_DISCONNECTED = "DSC";

	/**
	 * Constant indicating that a job has been ended with the *IMMED option, or
	...
	/**
	 * Constant indicating that a job is ineligible and not currently in a pool
	 * activity level.
	 */
	...
	/**
	 * Constant indicating that a job is waiting for the completion of a CPI
	 * communications call.
	 */
	...
	/**
	 * Constant indicating that a job is waiting in a pool activity level to try
	 * a read operation again on a database file after the end-of-file (EOF) has
	 * been reached.
	 */
	 ...
	 * 正在等待延迟
	 * 
	 * Constant indicating that a job is delayed for a time interval to end, or
	 * for a specific delay end time, by the Delay Job (DLYJOB) command.
	 */
	 ...
	 * 正在等待已延迟
	 * 
	 * Constant indicating that a job is waiting for a specified time interval
	 * to end, or for a specific delay end time, as specified on the Delay Job
	 * (DLYJOB) command.
	 */
	 ...
	 * 正在等待请求
	 * 
	 * Constant indicating that a prestart job is waiting for a program start
	 * request.
	 */
	 ...
	 * 正在等待事件间隔
	 * 
	 * Constant indicating that a job is waiting in a pool activity level for a
	 ...
	 * 设置所有子类型的 hashtable
	 * 
	 * @param subtypehashtable
	 *            the subtypehashtable to set
	 */
	public static void setSubtypehashtable(Hashtable<String, String> subtypehashtable) {
	 ...
	 * 获取设置状态的 hashtable
	 * 
	 * @param activeStatushashtable
	 *            the activeStatushashtable to set
	 */
