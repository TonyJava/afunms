...
public class PXmlOperator {
	...
	public void updateInfo(boolean isCustom) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			com.afunms.polling.base.Node node = PollingEngine.getInstance().getNodeByID(id);
			if (node == null) {
		...
		for (int i = 0; i < linkList.size(); i++) {
			...
			com.afunms.polling.base.LinkRoad lr = PollingEngine.getInstance().getLinkByID(id);
			// 这里主要用TRAP进行告警确定
			if (lr.isAlarm()) {
				if (lr.getLevels() > 0) {
