...
public class SysInitialize {
	...
	private void loadActionXml() {
		...
		try {
			...
			while (it.hasNext()) {
				...
				actionMap.put(element.getAttributeValue("tag"), new Integer(element.getAttributeValue("operate")));
			}
	...
	private void loadMenuXml() {
		...
		try {
			...
			while (it.hasNext()) {
				Element element = (Element) it.next();
				menuMap.put(element.getChild("filename").getText(), element.getChild("menuId").getText());
			}
