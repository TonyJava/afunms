...
public class NetSyslogImpEventDao extends BaseDao implements DaoInterface {
	...
	/*
	 * public int getCountByEventid(String tablename, String startTime, String
	 * toTime, String eventids){ String where = " where recordtime >= '" +
	 * startTime + "' and recordtime <= '" + toTime + "'"; if
	 * (tablename.startsWith("log")) { where = " where eventid in (" + eventids + ")
	 * and recordtime >= '" + startTime + "' and recordtime <= '" + toTime +
	 * "'";
	 *  } String sql = "select count(*) from " + tablename + where; rs =
	 * conn.executeQuery(sql); System.out.println("getCountByEventid sql = " +
	 * sql); try { while(rs.next()){ return rs.getInt(1); } } catch
	...
	public int getCountByEventid(String tablename, String startTime, String toTime, String eventids, String loginType) {
		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			...
			where = " where eventid in (" + eventids + ") and recordtime >= to_date('" + startTime + "','YYYY-MM-DD HH24:MI:SS') and recordtime <= to_date('" + toTime
	...
	private int getCountByEventNetwork(String ipaddress, String startTime, String toTime, String regexp) {
		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			...
			where = " where ipaddress = '" + ipaddress + "' and message like '%" + regexp + "%' and recordtime >= to_date('" + startTime
	...
	public int getCountByPriority(String ipaddress, String startTime, String toTime, String priority) {
		...
		String sql = "select count(*) from nms_netsyslog " + where;
		// String sql = "select count(*) from " + SysUtil.doip("log" +
		// ipaddress) + where;
		rs = conn.executeQuery(sql);
		// System.out.println("getCountByPriority sql = " + sql);
		try {
	...
	/*
	 * public static String getTableName(String ipaddress){ String table =
	 * "nms_netsyslog"; boolean isNetworkDev = isNetworkDev(ipaddress); if
	 * (!isNetworkDev) { table = "log" + SysUtil.doip(ipaddress); } return
	 * table; }
	 * 
	 * public static boolean isNetworkDev(String ipaddress){ boolean
	 * isNetworkDev = false; HostNodeDao dao = new HostNodeDao(); List nodeList =
	 * dao.loadNetwork(1); if (null != nodeList && nodeList.size() > 0) { for
	 * (int i = 0; i < nodeList.size(); i++) { HostNode node =
	 * (HostNode)nodeList.get(i); if (node.getIpAddress().equals(ipaddress)) {
	 * isNetworkDev = true; break; } } } return isNetworkDev; }
	 */
}
