...
public class SyslogDao extends BaseDao implements DaoInterface {
	...
	public synchronized boolean createSyslogData(Syslog syslog) {
		...
		try {
			...
			if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "insert into " + tablename + "(ipaddress,hostname,message,eventid,facility,priority,priorityName,facilityName," + "processId,processName,processIdStr,recordtime,username) " + "values(\"" + ip + "\",\"" + syslog.getHostname() + "\",\"" + syslog.getMessage() + "\"," + syslog.getEventid() + "," + syslog.getFacility() + "," + syslog.getPriority() + ",\"" + syslog.getPriorityName() + "\",\"" + syslog.getFacilityName() + "\"," + syslog.getProcessid() + ",\"" + syslog.getProcessname() + "\",\"" + syslog.getProcessidstr() + "\",\"" + time + "\",\"" + syslog.getUsername() + "\")";
			} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
				sql = "insert into " + tablename + "(ipaddress,hostname,message,eventid,facility,priority,priorityName,facilityName," + "processId,processName,processIdStr,recordtime,username) " + "values('" + ip + "','" + syslog.getHostname() + "','" + syslog.getMessage() + "'," + syslog.getEventid() + "," + syslog.getFacility() + "," + syslog.getPriority() + ",'" + syslog.getPriorityName() + "','" + syslog.getFacilityName() + "'," + syslog.getProcessid() + ",'" + syslog.getProcessname() + "','" + syslog.getProcessidstr() + "',to_date('" + time + "','YYYY-MM-DD HH24:MI:SS'),'" + syslog.getUsername() + "')";
			}
	...
	public List getQuery(String ipaddress, String starttime, String totime, String priorityname) throws Exception {
		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			s.append("select * from log" + doip(ipaddress) + " e where e.recordtime>= '" + starttime + "' " + "and e.recordtime<='" + totime + "'");
		} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
