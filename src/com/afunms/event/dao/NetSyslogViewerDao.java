		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			if (null == priorityname) {// 全部
				if (NetSyslogManager.isNetworkDev(ipaddress)) {
					...
					sql = "select count(*) from " + SysUtil.doip("log" + ipaddress) + " where ipaddress = '" + ipaddress.trim() + "' and recordtime >= '" + starttime
							+ "' and recordtime <= '" + totime + "'";
				}
			} else if (null != priorityname && "others".equals(priorityname)) {
				if (NetSyslogManager.isNetworkDev(ipaddress)) {
					sql = "select count(*) from nms_netsyslog where trim(priorityname) != 'error' and trim(priorityname) != 'warning' and ipaddress = '" + ipaddress.trim()
							+ "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "'";
				} else {
					sql = "select count(*) from " + SysUtil.doip("log" + ipaddress) + " where trim(priorityname) != 'error' and trim(priorityname) != 'warning' and ipaddress = '"
							+ ipaddress.trim() + "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "'";
				}

			} else {
				if (NetSyslogManager.isNetworkDev(ipaddress)) {
					sql = "select count(*) from nms_netsyslog where trim(priorityname)= '" + priorityname.trim() + "' and ipaddress = '" + ipaddress.trim()
							+ "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "'";
				} else {
					sql = "select count(*) from " + SysUtil.doip("log" + ipaddress) + " where trim(priorityname)= '" + priorityname.trim() + "' and ipaddress = '"
							+ ipaddress.trim() + "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "'";
				}
			}
			...
			if (null == priorityname) {// 全部
				if (NetSyslogManager.isNetworkDev(ipaddress)) {
					sql = "select count(*) from nms_netsyslog where ipaddress = '" + ipaddress.trim() + "' and recordtime >= to_date('" + starttime
							+ "','YYYY-MM-DD HH24:MI:SS') and recordtime <= to_date('" + totime + "','YYYY-MM-DD HH24:MI:SS')";
				} else {
					sql = "select count(*) from " + SysUtil.doip("log" + ipaddress) + " where ipaddress = '" + ipaddress.trim() + "' and recordtime >= to_date('" + starttime
							+ "','YYYY-MM-DD HH24:MI:SS') and recordtime <= to_date('" + totime + "','YYYY-MM-DD HH24:MI:SS')";
				}
			} else if (null != priorityname && "others".equals(priorityname)) {
				if (NetSyslogManager.isNetworkDev(ipaddress)) {
					sql = "select count(*) from nms_netsyslog where trim(priorityname) != 'error' and trim(priorityname) != 'warning' and ipaddress = '" + ipaddress.trim()
							+ "' and recordtime >= to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS') and recordtime <= to_date('" + totime + "','YYYY-MM-DD HH24:MI:SS')";
				} else {
					sql = "select count(*) from " + SysUtil.doip("log" + ipaddress) + " where trim(priorityname) != 'error' and trim(priorityname) != 'warning' and ipaddress = '"
							+ ipaddress.trim() + "' and recordtime >= to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS') and recordtime <= to_date('" + totime
