...
public class NetSyslogDao extends BaseDao implements DaoInterface {
	...
	public int getCountByEvent(String ipaddress, String starttime, String totime, String eventMsg) {
		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			sql = "select count(*) from nms_netsyslog where ipaddress = '" + ipaddress + "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "' and "
					+ eventMsg;
	...
	public Map getCountByUserAccout(String ipaddress, String starttime, String totime, String msgRegexp) {
		String where = "";
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			where = " where ipaddress = '" + ipaddress + "' and recordtime >= '" + starttime + "' and recordtime <= '" + totime + "' and " + msgRegexp;
		} else if ("oracle".equalsIgnoreCase(SystemConstant.DBType)) {
			where = " where ipaddress = '" + ipaddress + "' and e.recordtime>= to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS') " + "and e.recordtime<=to_date('" + totime
					+ "','YYYY-MM-DD HH24:MI:SS')  and " + msgRegexp;
		}
