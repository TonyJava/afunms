...
public class NetSyslogManager extends BaseManager implements ManagerInterface {
	...
	public String execute(String action) {
		...
		/*
		 * if (action.equals("netsyslogeventdetail")) return
		 * netsyslogeventdetail();
		 */
	...
	private String exportAllevtCatelistall() {
		NetSyslogEventDao dao = new NetSyslogEventDao();
		String ipaddress = getParaValue("ipaddress");
		// String where = getWhere(starttime, totime, priority, eventid,
		// processname, message, ipaddress);
		...
		try {
			...
			AbstractionReport1 report = new ExcelReport1(new IpResourceReport(), reporthash);
			// report.createReport_impevt("temp" + File.separator +
			// "syslogall.xls");
			request.setAttribute("filename", report.getFileName());
		} catch (Exception e) {
			e.printStackTrace();
	...
	private String exportCatelistall() {
		...
		/*
		 * String where = getWhere(starttime, totime, priority, null,
		 * processname, message, ipaddress, null, null); NetSyslogDao
		 * netSyslogDao = new NetSyslogDao(); // List list =
		 * netSyslogDao.loadByPriority(ipaddress, priority, starttime, totime,
		 * processname, message); List list =
		 * netSyslogDao.loadByPriority(where); Hashtable reporthash = new
		 * Hashtable(); if (list != null) { reporthash.put("list", list); } else {
		 * list = new ArrayList(); } // reporthash.put("startRow", startRow);
		 * AbstractionReport1 report = new ExcelReport1(new IpResourceReport(),
		 * reporthash);
		 * 
		 * report.createReport_syslogall("temp" + File.separator +
		 * "syslogall.xls"); request.setAttribute("filename",
		 * report.getFileName()); return "/alarm/syslog/download.jsp";
		 */
		List list = new ArrayList();
	...
	private String getWhere() {
		...
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			...
			where = " where recordtime>= to_date('" + starttime + "','YYYY-MM-DD HH24:MI:SS') " + "and recordtime<=to_date('" + totime + "','YYYY-MM-DD HH24:MI:SS')";
		}
	...
	private String getWhere(String starttime, String totime, String priority, String facility, String processname, String message, String ipaddress, String eventMsg,
			String strDevType) {
		// String where = " where recordtime >= '" + starttime + "' and
		// recordtime <= '" + totime + "'";
		String where = "";
		if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
			where = " where recordtime >= '" + starttime + "' and recordtime <= '" + totime + "'";
			...
			if (!"-1".equals(strclass) && strclass != null && !"".equals(strclass) && !"null".equals(strclass)) {
				if ("1".equals(strclass)) {
					if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
						...
						s.append(" where recordtime>= to_date('" + starttime1 + "','YYYY-MM-DD HH24:MI:SS') " + "and recordtime<=to_date('" + totime1
					...
					if ("mysql".equalsIgnoreCase(SystemConstant.DBType)) {
						...
						s.append(" where category <> 4 and recordtime>= to_date('" + starttime1 + "','YYYY-MM-DD HH24:MI:SS') " + "and recordtime<=to_date('" + totime1
								+ "','YYYY-MM-DD HH24:MI:SS')");
					}
