...
public class HomeUserManager extends BaseManager implements ManagerInterface {
	...
	private String save() {
		...
		try {
			...
			List homeRoleList = roleDao.findByCondition(" where role_id='" + uservo.getRole() + "'");// 查找角色名下所有模块
																										// 包含了不显示的功能
			sql = "delete from nms_home_module_user where user_id='" + uservo.getUserid() + "'";
			db.addBatch(sql);
	...
	private String update() {
		...
		HomeUserDao homeUserDao = new HomeUserDao();
		homeUserModelList = homeUserDao.findByCondition(" where user_id='" + uservo.getUserid() + "'");// 能控制的模块
		HomeRoleDao homeRoleDao = new HomeRoleDao();
		List homeRoleList = homeRoleDao.findByCondition(" where role_id='" + uservo.getRole() + "' and visible='1'");// 能控制的模块
