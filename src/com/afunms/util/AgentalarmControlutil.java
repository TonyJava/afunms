...
/**
 * 
 * 告警控制 与告警抑制控制实现类 实现不通agent之间告警数据共享与告警抑制的功能
 * 
 * @author konglq
 * 
 */
...
public class AgentalarmControlutil {
	...
	public static void delete_nms_send_alarm_time(AlarmIndicatorsNode alarmIndicatorsNode, String name) {
		if (!alarmIndicatorsNode.getName().equals("diskperc") || !alarmIndicatorsNode.getName().equals("proce") || !alarmIndicatorsNode.getName().equals("diskinc")
				|| !alarmIndicatorsNode.getName().equals("diskbusy")) {
	...
	public static void Getsystem_eventlistsql(BaseVo baseVo) {
		...
		sql
				.append("insert into system_eventlist(eventtype,eventlocation,content,level1,managesign,bak,recordtime,reportman,nodeid,businessid,oid,lasttime,subtype,managetime,subentity)values(");
		sql.append("'");
	...
	public static void Getnms_send_alarm_timetosql(BaseVo vo) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("insert into nms_send_alarm_time(name,alarm_way_detail_id,send_times," + "last_send_time" + ") values('");
	...
	public static void Getnms_send_alarm_timetoDeletesql(BaseVo vo) {
		SendAlarmTime sendAlarmTime = (SendAlarmTime) vo;
		GathersqlListManager.Addsql_alarm("delete from nms_send_alarm_time where name='" + sendAlarmTime.getName() + "' and alarm_way_detail_id='"
				+ sendAlarmTime.getAlarmWayDetailId() + "'");
	}
	...
	public static void deleteCheckEvent(String nodeId, String type, String subtype, String name) {
		GathersqlListManager.Addsql_alarm("delete from nms_checkevent where nodeid='" + nodeId + "' and type='" + type + "' and subtype='" + subtype + "' and indicators_name='"
				+ name + "'");
	}
